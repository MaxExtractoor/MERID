...
class FederatedMemoryBus:
    ...
    def publish(self, packet: FederatedMemoryPacket) -> None:
        # Evict oldest packets when at capacity to prevent unbounded growth
        if len(self._packets) >= self.MAX_PACKETS:
            oldest_id = min(self._packets, key=lambda k: self._packets[k].timestamp)
            del self._packets[oldest_id]
