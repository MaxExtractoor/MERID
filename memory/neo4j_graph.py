...
class Neo4jMemoryGraph:
    ...
    def connect(self) -> bool:
        ...
        # Skip Neo4j in kalshi-only profile (no graph memory needed)
        import os
        if os.getenv("MERID_PROFILE", "").lower() == "kalshi-only":
            logger.info("Neo4j skipped — MERID_PROFILE=kalshi-only")
        ...
        try:
            ...
            # Test connection with a hard 5-second wall-clock timeout
            # (driver-level routing retries can exceed connection_timeout)
            def _ping():
            ...
            logger.warning(f"Neo4j unavailable — using JSON-only memory storage: {e}")
            if self.driver:
                try:
                    self.driver.close()
                except Exception:
