...
class BacktestEngine:
    ...
    async def _fetch_historical_data(
        ...
        for symbol in symbols:
            try:
                ohlcv = await feed.fetch_historical_ohlcv(symbol, timeframe, limit=500)
                if ohlcv:
                    data[symbol] = ohlcv
    ...
    def _calculate_metrics(
        ...
        annualized_return = ((final_capital / config.initial_capital) ** (365 / duration_days) - 1) * 100 if duration_days > 0 else 0
