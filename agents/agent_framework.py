    ...
    async def start(self) -> None:
        ...
        def _task_done_cb(task: asyncio.Task) -> None:
            if not task.cancelled() and task.exception():
                logger.error("Agent %s task crashed: %s", self.agent_id, task.exception())
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    logger.debug("silent catch in agent_framework:202")
...
class AgentRegistry:
    ...
    def register(self, agent: Agent) -> None:
        ...
        if agent.agent_id in self._agents:
            ...
            # Remove old entry from role list to prevent duplicates
            old_agent = self._agents[agent.agent_id]
            if old_agent.role in self._agents_by_role:
                self._agents_by_role[old_agent.role] = [
                    a for a in self._agents_by_role[old_agent.role]
