...
class StreamingAgent(ABC):
    ...
    async def start(self):
        ...
        self.running = True
        self._task = asyncio.create_task(self._run_loop(), name=f"streaming-{self.agent_id}")
        def _task_done_cb(task: asyncio.Task) -> None:
            if not task.cancelled() and task.exception():
                self.logger.error("Streaming agent %s task crashed: %s", self.agent_id, task.exception())
    ...
    async def _run_loop(self):
        ...
        # Event-loop yielding: prevent blocking when queue has many events
        _events_since_yield = 0
        _YIELD_EVERY_N_EVENTS = 5  # Yield every 5 events processed (tightened from 10)
        
        while self.running:
            try:
                # Check compute budget
                if self._budget.is_exhausted():
                    self.logger.debug(f"{self.agent_id} compute budget exhausted, waiting for reset")
                    await asyncio.sleep(60)
                    self._optimizer.reset_budgets()
                    _events_since_yield = 0  # Reset counter after sleep
                    continue
                ...
                try:
                    ...
                    _events_since_yield = 0  # Reset counter on timeout
                    continue
                ...
                try:
                    ...
                    success = False
                    analysis = None
                analyze_duration_ms = (time.perf_counter() - analyze_start) * 1000
                
                # Log slow analyze calls for profiling
                if analyze_duration_ms > 50:  # Log if analyze takes >50ms
                    self.logger.debug(
                        f"{self.agent_id} SLOW analyze: {analyze_duration_ms:.1f}ms "
                        f"(event_type={event.type if hasattr(event, 'type') else 'unknown'})"
                    )
                ...
                if analysis:
                    ...
                    emit_duration_ms = (time.perf_counter() - emit_start) * 1000
                    
                    # Log slow emit calls
                    if emit_duration_ms > 20:  # Log if emit takes >20ms
                        self.logger.debug(
                            f"{self.agent_id} SLOW emit_output: {emit_duration_ms:.1f}ms"
                        )
                ...
                # EVENT-LOOP YIELD: Prevent blocking when processing high event volume
                _events_since_yield += 1
                if _events_since_yield >= _YIELD_EVERY_N_EVENTS:
                ...
                _events_since_yield = 0  # Reset counter after error sleep
