...
def _get_kb():
    try:
        from memory.long_term_knowledge_base import get_long_term_knowledge_base
        return get_long_term_knowledge_base()
    except Exception:
        return None
...
class ReflectionSystem:
    ...
    def validate_consensus_outcome(
        ...
        if success and self.auto_persist:
            ...
            self.persist_insights(reflection.agent_id)
    ...
    def persist_insights(self, agent_id: str) -> int:
        """Persist current LearningEngine insights for an agent to LongTermKnowledgeBase.

        Returns the number of insights persisted.
        """
        kb = _get_kb()
        if kb is None:
            return 0
        ...
        for insight in insights:
            try:
                kb.record_entry(
                    entry_type="lesson",
                    title=f"[{agent_id}] {getattr(insight, 'insight_type', 'insight')}",
                    summary=getattr(insight, 'description', str(insight)),
                    tags=["learning", agent_id, getattr(insight, 'insight_type', 'unknown')],
                    impact=getattr(insight, 'severity', 'medium'),
                    evidence={
                        "confidence": str(getattr(insight, 'confidence', '')),
                        "evidence_count": str(getattr(insight, 'evidence_count', '')),
                        "recommendation": getattr(insight, 'recommendation', ''),
                    },
                    recorded_by="reflection_system",
                )
                persisted += 1
            except Exception as exc:
                logger.debug("Failed to persist insight for %s: %s", agent_id, exc)
        return persisted
...
_reflection_system_lock = threading.Lock()
