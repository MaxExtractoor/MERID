...
class EmailChannel(NotificationChannel):
    ...
    _startup_warning_logged: bool = False  # Class-level flag for one-time log
    ...
    def __init__(self):
        ...
        # Check if email is explicitly disabled via env
        self._explicitly_disabled = os.getenv("SMTP_ENABLED", "").lower() in ("false", "0", "no", "")
        
        # Only log once at startup if SMTP not configured
        if not self.smtp_host or not self.smtp_user:
            if not EmailChannel._startup_warning_logged:
                logger.info(
                    "notifications.channels Email disabled (no SMTP configured). "
                    "Set SMTP_HOST, SMTP_USER env vars to enable email notifications."
                )
                EmailChannel._startup_warning_logged = True
            # Mark as disabled so we don't warn on every send
            self.enabled = False
    ...
    async def send(self, message: NotificationMessage) -> bool:
        ...
        if not self.enabled:
            # Silently drop - email is disabled by config, not an error
        ...
        if not self.smtp_host:
            # No SMTP configured - log once at debug level then silently no-op
            self.logger.debug("Email not configured - dropping message to %s", message.recipient)
            message.status = DeliveryStatus.FAILED
            return False
...
class SMSChannel(NotificationChannel):
    ...
    async def send(self, message: NotificationMessage) -> bool:
        ...
        try:
            ...
            message.status = DeliveryStatus.FAILED
            self.logger.error("SMS send failed: twilio package not installed")
            return False
        except Exception as e:
            message.status = DeliveryStatus.FAILED
...
class PushChannel(NotificationChannel):
    ...
    async def send(self, message: NotificationMessage) -> bool:
        ...
        if not self.firebase_credentials:
            self.logger.warning("Push not configured (no Firebase credentials) — message not sent")
            message.status = DeliveryStatus.FAILED
            return False
        ...
        try:
            ...
            message.status = DeliveryStatus.FAILED
            self.logger.error("Push send failed: firebase_admin package not installed")
            return False
        except Exception as e:
            message.status = DeliveryStatus.FAILED
