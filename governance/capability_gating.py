...
class CapabilityGatingService:
    ...
    def request_capability(self, agent_id: str, capability: CapabilityLevel) -> bool:
        """T-055: Enforced capability gating — returns True ONLY if granted."""
        ...
        if state.reliability_score < gate.min_reliability:
            logger.warning(
                "Agent %s reliability %.2f below requirement %.2f for %s — DENIED",
                agent_id,
        ...
        # T-055: Pending is NOT granted — queue for approval but return False
        state.pending_capabilities[capability] = datetime.utcnow()
        logger.info("Capability %s pending for agent %s — NOT YET GRANTED", capability.value, agent_id)
        return False

    def has_capability(self, agent_id: str, capability: CapabilityLevel) -> bool:
        """T-055: Check if agent has been explicitly granted a capability."""
        state = self._agents.get(agent_id)
        if not state:
            return False
        return capability in state.granted_capabilities
...
_capability_gating_service_lock = threading.Lock()
...
def get_capability_gating_service() -> CapabilityGatingService:
    ...
    if _capability_gating_service is None:
        with _capability_gating_service_lock:
            if _capability_gating_service is None:
