...
class IntentReviewEngine:
    ...
    async def _check_constitutional(
        ...
        try:
            ...
            # Max position size invariant (use constitutional limit, not hardcoded)
            max_notional = getattr(self._constitutional, "REQUIRE_HUMAN_ABOVE_USD", 500000)
            if notional > max_notional:
                result.constitutional_check_passed = False
                result.rejection_details = f"Position ${notional:,.0f} exceeds constitutional max ${max_notional:,.0f}"
                return False
            
            # Max leverage invariant (use constitutional limit, not hardcoded)
            max_leverage = getattr(self._constitutional, "MAX_LEVERAGE", 3.0)
            if leverage > max_leverage:
                result.constitutional_check_passed = False
                result.rejection_details = f"Leverage {leverage:.1f}x exceeds constitutional limit {max_leverage:.1f}x"
                return False
...
_intent_review_engine_lock = threading.Lock()
...
def get_intent_review_engine() -> IntentReviewEngine:
    ...
    if _intent_review_engine is None:
        with _intent_review_engine_lock:
            if _intent_review_engine is None:
