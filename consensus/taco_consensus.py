...
DEPRECATED (T-002 Zero-Trust Audit): This module is superseded by
consensus.consensus_coordinator.EnhancedConsensusCoordinator which
has stricter quorum (3 vs 2), configurable timeouts, and proper
failure handling.  The production loop (merid/loop.py) now uses
EnhancedConsensusCoordinator.  This file is retained only for its
data classes (AgentOpinion, TradePlan, AgentRole, etc.) which are
still imported by other modules.

Do NOT instantiate TaCoConsensusCoordinator for new code.
Use: from consensus.consensus_coordinator import get_consensus_coordinator
...
class TaCoConsensusCoordinator:
    ...
    async def _run_consensus_cycle(self, symbol: str) -> Optional[TradePlan]:
        ...
        for opinion in opinions:
            ...
            # T-039: Fix freshness default inversion — unknown freshness gets 0.5, not 1.0
            freshness = getattr(opinion, "signal_freshness", 0.5)
        ...
        # Prune expired plans to prevent unbounded dict growth
        expired = [pid for pid, p in self._active_plans.items() if p.is_expired()]
        for pid in expired:
    ...
    def _get_agent_weight(self, agent_id: str) -> float:
        ...
        try:
            ...
            logger.debug("Risk metrics tracker lookup failed for %s: %s", agent_id, _rmt_exc)
