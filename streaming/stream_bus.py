...
class InMemoryStreamBus(StreamBus):
    ...
    def __init__(
        ...
        # Consumer tasks for cleanup on unsubscribe
    ...
    async def subscribe(
        ...
        # Start consumer task
        self._consumer_tasks[sub_id] = asyncio.create_task(self._consumer_loop(sub_id))
    ...
    async def unsubscribe(self, subscription_id: str) -> bool:
        ...
        # Cancel the consumer task to prevent resource leak
        ...
        while self._running and sub_id in self._subscriptions:
            try:
                ...
                if group:
                    ...
                    # Limit processed ID tracking — sets are unordered so we
                    # cannot keep "newest"; clear entirely to bound memory.
                    # Brief reprocessing window is acceptable (handlers should
                    # be idempotent by design).
                    if len(self._processed_ids[group]) > 100000:
                        self._processed_ids[group].clear()
...
class KafkaStreamBus(StreamBus):
    ...
    async def publish(self, event: StreamEvent) -> bool:
        ...
        try:
            # Use partition_key for ordered processing; fall back to event_id
            key = event.partition_key or event.event_id
            value = event.to_dict()
