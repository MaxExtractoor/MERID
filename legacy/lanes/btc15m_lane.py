...
⚠️ DEPRECATED (2026-05-15): This legacy BTC-specific lane implementation is ANCIENT_EXPERIMENTAL.
The canonical implementation is Crypto15MLane in merid/lanes/crypto15m_lane.py, which supports
all crypto assets (BTC/ETH/SOL/XRP/DOGE) with a unified interface.

BTC15MLane was previously used in web/api/kalshi_api.py and web/startup_agents.py but has been
migrated to use Crypto15MLane. This file is kept for reference but should not be used in production.

Migration path: Kalshi API and startup code now use Crypto15MLane via registry.get_lane("BTC_15M").
"""
...
class BTC15MLane:
    ...
    async def _init_components(self) -> None:
        ...
        _lane_id = f"{self.config.asset}_{self.config.timeframe}"
        self._promotion_engine = get_promotion_engine(_lane_id)
        ...
        try:
            ...
            if _sp is not None:
                try:
                    ...
                    logger.debug("silent catch in btc15m_lane:679")
...
_btc15m_lane_lock = threading.Lock()
...
class LaneOrchestrator:
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            # Guard against interpreter shutdown nullifying globals
            try:
                logger.warning("LaneOrchestrator stop error: %s", exc)
            except Exception:
                pass
    ...
    async def _phase_monitor_loop(self) -> None:
        ...
        while self._running:
            try:
                engine = get_promotion_engine("__global__")
...
_lane_orchestrator_lock = threading.Lock()
