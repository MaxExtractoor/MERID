...
class SecretsManager:
    ...
    async def check_and_alert_rotation(self) -> Dict[str, Any]:
        """T-053: Automated rotation check — call periodically (e.g. daily).
        Returns report of secrets needing rotation and emits alerts.
        """
        stale = self.get_secrets_needing_rotation()
        if not stale:
            return {"needs_rotation": 0, "secrets": []}

        for s in stale:
            logger.warning(
                "SECRET ROTATION OVERDUE: %s (type=%s) — rotate immediately",
                s["name"], s["secret_type"],
            )

        # Emit operator alert if notification system available
        try:
            from notifications.channels import send_operator_alert
            await send_operator_alert(
                f"{len(stale)} secret(s) overdue for rotation: "
                + ", ".join(s["name"] for s in stale),
                severity="critical",
            )
        except Exception:
            logger.debug("silent catch in secrets_manager:401")

        return {"needs_rotation": len(stale), "secrets": stale}
