...
class HyperliquidAdapter(PerpVenueAdapterBase):
            ...
            if not symbols:
                if self._symbol_cache:
                    ...
                    try:
                        ...
                        symbols = [a.symbol.split("/")[0] for a in get_perp_universe()]
                    except Exception:
                        symbols = ["BTC", "ETH", "SOL"]
    ...
    def _fetch_funding_live(self, symbols: Optional[List[str]]) -> List[FundingRateSnapshot]:
        if not symbols:
            symbols = self._symbol_cache or self._fallback_symbols
            if not symbols:
                try:
                    from data.asset_universe import get_perp_universe
                    symbols = [a.symbol.split("/")[0] for a in get_perp_universe()]
...
class BybitPerpAdapter(PerpVenueAdapterBase):
    ...
    def _fetch_funding_live(self, symbols: Optional[List[str]]) -> List[FundingRateSnapshot]:
        if not symbols:
            try:
                from data.asset_universe import get_perp_universe
                symbols = [a.symbol.split("/")[0] + "USDT" for a in get_perp_universe()
                           if a.liquidity_tier <= 2]
            except Exception:
