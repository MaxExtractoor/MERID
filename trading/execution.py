...
class ExecutionEngine:
    ...
    async def submit_order(
        ...
        try:
            ...
            self._logger.warning("Risk module not installed — kill switch check unavailable")
        except OrderRejectedError:
            raise
        except Exception as _risk_exc:
            self._logger.error("Kill switch check failed: %s — blocking order (fail-closed)", _risk_exc)
            raise OrderRejectedError(f"Kill switch check error (fail-closed): {_risk_exc}") from _risk_exc
    ...
    async def _execute_live_order(
        ...
        try:
            ...
            self._logger.error(
                "Live order execution failed: %s. Order REJECTED (no silent paper fallback).",
                str(e)
            )
            order.status = OrderStatus.REJECTED
            order.updated_at = time.time()
            order.metadata["rejection_reason"] = f"Live execution failed: {e}"
            self._order_history.append(order)
            if len(self._order_history) > 10000:
                self._order_history = self._order_history[-5000:]
            raise ExecutionError(f"Live order execution failed: {e}") from e
    ...
    def _update_position_from_fill(
        ...
        if symbol in self._positions:
            ...
            ) / new_quantity if new_quantity > 0 else order.filled_price
    ...
    async def _position_monitor_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                # Periodically prune completed orders older than 1 hour
                now = time.time()
                if not hasattr(self, '_last_order_prune') or now - self._last_order_prune > 300:
                    stale_ids = [
                        oid for oid, o in self._orders.items()
                        if o.is_complete() and (now - o.updated_at) > 3600
                    ]
                    for oid in stale_ids:
                        del self._orders[oid]
                    if stale_ids:
                        self._logger.debug("Pruned %d completed orders from memory", len(stale_ids))
                    self._last_order_prune = now
