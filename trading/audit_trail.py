...
def record_operator_action(
    *,
    ...
    """Record a destructive operator action (emergency stop, kill-switch reset, mode change, etc.).

    Args:
        action:            Human-readable action name, e.g. "emergency_stop", "reset_kill_switch".
        reason:            Operator-supplied reason string.
        actor_token_hash:  SHA-256 prefix (first 16 hex chars) of the Bearer token used.
                           Never store the raw token.
        actor_ip:          Client IP from request.client.host, or "" if unavailable.
        previous_state:    Snapshot of relevant state before the action (e.g. kill_switch status).
        metadata:          Any additional key/value context.

    Returns:
        The event_id of the written audit entry.
    """
    entry = AuditEntry(
        agent_id=f"operator:{actor_token_hash or 'unknown'}",
        venue="operator",
        symbol="",
        side="",
        size=0.0,
    ...
    append_entry(entry)
    logger.warning(
        "Operator action: %s | reason=%r | actor_ip=%s | token_hash=%s | event_id=%s",
        action, reason, actor_ip, actor_token_hash, entry.event_id,
    )
    return entry.event_id
...
def get_audit_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("Audit trail file read failed: %s", _af_exc)
