...
class RiskGuard:
    ...
    async def reset_kill_switch(self, reset_by: str, confirmation_code: str) -> bool:
        ...
        # Confirm reset: accept either the static code or a time-based OTP.
        # The static code "CONFIRM_RESET_TRADING" is the documented reset phrase.
        # An env-seeded TOTP is also accepted for production hardening.
        import hashlib
        import os as _os
        _STATIC_CONFIRM = "CONFIRM_RESET_TRADING"
        seed = _os.environ.get("MERID_KILL_SWITCH_SEED", "merid-default-seed-CHANGE-ME")
        time_slot = str(int(now // 300))  # 5-minute window
        expected_totp = hashlib.sha256(f"{seed}:{time_slot}".encode()).hexdigest()[:12]
        if confirmation_code != _STATIC_CONFIRM and confirmation_code != expected_totp:
            logger.warning("Kill switch reset failed: invalid confirmation code by %s", reset_by)
            return False
    ...
    async def update_limits(
        ...
        """
        Propose (and optionally immediately apply) risk limit changes (T-012).
        When requires_dual_approval=False (default), changes are applied immediately
        by the proposer.  Pass True to require a second distinct approver via
        approve_limit_change(change_id).
        """
        ...
        if requires_dual_approval:
            ...
            logger.info(
                "Risk limit change applied by %s (change_id=%s, single-approval).",
                updated_by, change_id,
            )
    ...
    def record_trade(self, symbol: str, size_usd: float, pnl_usd: float = 0.0) -> None:
        ...
        self.portfolio.daily_pnl_usd += pnl_usd
