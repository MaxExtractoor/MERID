...
class RiskGuard:
    ...
    async def reset_kill_switch(self, reset_by: str, confirmation_code: str) -> bool:
        """
        Reset kill switch. Requires confirmation code for safety.
        This is a manual operation requiring human intervention.
        """
        # Simple confirmation - in production, use proper auth
        expected_code = "CONFIRM_RESET_TRADING"
        if confirmation_code != expected_code:
            logger.warning(f"Kill switch reset failed: invalid confirmation by {reset_by}")
            return False
    ...
    async def update_limits(
        ...
        async with self._lock:
            ...
            # Update metadata
            major, minor, patch = self.limits.version.split(".")
            self.limits.version = f"{major}.{int(minor) + 1}.0"
            self.limits.updated_at = time.time()
            self.limits.updated_by = updated_by
            
            logger.info(f"Risk limits updated by {updated_by}: {old_version} -> {self.limits.version}")
