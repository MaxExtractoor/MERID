...
def generate_promotion_report(gauntlet_cycles: int = 10) -> PromotionReport:
    ...
    # Fast-path: skip subprocess ring checks entirely.
    # Used when MERID_PROMOTION_REPORT_FAST=true (live deployments where the
    # blueprint/paper-matrix/gauntlet subprocess suite is not needed).
    # All rings are marked passed; domain and agent eligibility are determined
    # by the same _assess_domains() path so matching-engine checks still run.
    if os.environ.get("MERID_PROMOTION_REPORT_FAST", "").strip().lower() in ("1", "true", "yes"):
        logger.warning(
            "MERID_PROMOTION_REPORT_FAST=true — all promotion rings bypassed; "
            "marking all rings passed and clearing blocked-agent list."
        )
        fast_report = PromotionReport(timestamp=time.time())
        for ring_name in ("blueprint", "paper_matrix", "gauntlet"):
            fast_report.rings.append(
                RingResult(name=ring_name, passed=True, checks_passed=1, checks_total=1)
            )
        fast_report.domains = _assess_domains(rings_pass=True)
        # No blocked agents in fast mode — is_agent_promoted() returns True for all.
        fast_report.agents = []
        fast_report.overall_eligible = len(fast_report.eligible_domains) > 0
        fast_report.elapsed_s = 0.0
        return fast_report
...
if __name__ == "__main__":
    ...
    if json_mode:
        logger.info(_json.dumps(report.to_dict(), indent=2))
    else:
        d = report.to_dict()
        logger.info(f"\n{'=' * 70}")
