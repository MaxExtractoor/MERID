...
class HashtagMonitor:
    ...
    def _get_fg(self, asset: str = "BTC") -> Optional[int]:
        try:
    ...
    def _publish_signal_telegram(self, signals: List[Any]) -> None:
        ...
        try:
            ...
            for sig in strong[:3]:
                ...
                msg = (
                    f"{emoji} <b>Hashtag Signal</b> [{sig.category.upper()}]\n"
                    f"Asset/Event: <code>{sig.asset_or_event}</code>\n"
                    f"Direction: <b>{sig.direction}</b> (strength={sig.strength:.2f})\n"
                    f"Reason: {sig.reason}\n"
                    f"Tags: {', '.join(sig.tags[:4])}\n"
                    f"Score: {sig.score:+.3f} | Vol: {sig.volume}"
                )
                mgr.send_telegram(msg)
