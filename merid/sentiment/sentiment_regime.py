...
class SentimentRegimeEngine:
    ...
    # Rolling window for NOISY regime detection: keep states from the last N seconds.
    _NOISY_WINDOW_SECONDS: int = 1800  # 30 minutes

    def __init__(self):
        self._current_regime: Optional[RegimeState] = None
        self._history: list = []

    def _recent_sent_values(self, now: datetime) -> list:
        """Return sentiment values from the last _NOISY_WINDOW_SECONDS.

        Uses the engine's own internal history so callers do NOT need to pass
        sent_history — the NOISY branch was effectively dead when callers omitted it.
        """
        cutoff = now.timestamp() - self._NOISY_WINDOW_SECONDS
        return [
            s.sent_combined
    ...
    def detect_regime(
        ...
        Args:
            ...
            sent_history: Optional external history; internal rolling window
                          is used by default (preferred — do not pass None).
        ...
        # Use internal time-windowed history when caller omits sent_history.
        # This fixes the production bug where NOISY was never triggered because
        # all callers passed sent_history=None.
        effective_history = sent_history if sent_history is not None else self._recent_sent_values(now)
        ...
        # Check for noisy regime using time-windowed internal history
        elif len(effective_history) >= 5:
            recent_signs = [1 if s > 0 else -1 for s in effective_history[-5:]]
            ...
            if sign_changes >= 2 and abs(sent_combined) < 0.3:
                ...
                reasoning = (
                    f"Noisy: {sign_changes} sign changes in last 5 observations "
                    f"(30-min window), weak magnitude {abs(sent_combined):.2f}"
                )
            else:
        ...
        # Keep last 100 states (hard cap to bound memory)
