...
_risk_manager_instance: Optional[SentimentRiskManager] = None


def get_sentiment_risk_manager(config: Optional[SentimentRiskConfig] = None) -> SentimentRiskManager:
    """Return the module-level singleton SentimentRiskManager.

    Using a singleton ensures that daily_pnl, cooldown_until, and last_sentiment
    accumulate correctly across calls.  The previous convenience function
    instantiated a fresh manager on every call, making daily loss caps
    structurally unreachable.

    Call manager.record_trade(pnl, sentiment, timestamp) after each fill to
    keep state current, and manager.reset_daily() at the start of each session.
    """
    global _risk_manager_instance
    if _risk_manager_instance is None:
        _risk_manager_instance = SentimentRiskManager(config or SentimentRiskConfig())
...
def check_sentiment_trade_allowed(
    ...
    Uses the singleton SentimentRiskManager so daily_pnl and cooldown state
    are preserved across calls.  The caller should also call
    get_sentiment_risk_manager().record_trade(pnl, sentiment, timestamp)
    after each fill.

    Returns:
        (allowed, size, reason) tuple
    """
    manager = get_sentiment_risk_manager()
    # Sync equity from caller — equity may change with each fill
    manager.equity = equity
    # Sync daily_pnl if caller is authoritative (e.g. start-of-day bootstrap)
    if daily_pnl != 0.0:
    ...
    # Check daily stops against accumulated state
    if manager.daily_pnl <= -equity * manager.config.hard_stop_pct:
        return False, 0.0, "hard_stop"

    # Calculate size
    base_size = equity * manager.config.max_trade_pct
