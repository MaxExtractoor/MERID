...
class NewsIngestionAgent:
    ...
    def _score_headline(self, raw: RawHeadline) -> NewsSentiment:
    ...
    async def run_cycle(
        ...
        for raw in deduped:
            try:
                scored.append(self._score_headline(raw))
            except Exception as exc:
        ...
        logger.info(
            "[news-agent] cycle: %d raw → %d deduped → %d scored → %d aggregated in %.1fs",
    ...
    async def run_event_cycle(
        ...
        deduped = self._dedupe(all_raw)
        scored = [self._score_headline(r) for r in deduped]
        aggregated = self._aggregate(scored)
