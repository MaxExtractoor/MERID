...
_finbert_model = None
_finbert_lock = threading.Lock()  # FPA-02: prevent concurrent model loading
...
def _get_finbert():
    ...
    if _finbert_tokenizer is None or _finbert_model is None:
        with _finbert_lock:
            if _finbert_tokenizer is None or _finbert_model is None:  # double-checked
                try:
                    ...
                    logger.warning(f"Failed to load FinBERT: {exc}")
                    return None, None
...
class NewsSentimentResult:
    ...
    model_version: str = "unknown"  # e.g. "ProsusAI/finbert" or "vader_fallback"
...
class KalshiNewsSentiment:
    ...
    def fetch_crypto_news(
        ...
        if not self.api_key:
            logger.warning(
                "No NEWS_API_KEY configured for %s — returning empty article list. "
                "Synthetic news is disabled in live mode to prevent fake signal injection. "
                "Set NEWS_API_KEY to enable real news sentiment.",
                symbol,
            )
            return []
        ...
        try:
            ...
            logger.warning("News fetch failed for %s: %s — returning empty list.", symbol, exc)
            return []
