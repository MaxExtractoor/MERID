...
class SimpleFGMarkov:
    ...
    def transition(self, fg_index: int) -> RegimeState:
        ...
        # Sample next state
        # Row layout: probs[0] = P(current -> CALM), probs[1] = P(current -> TURBULENT)
        # For CALM row:      probs[0] = P(calm->calm),      probs[1] = P(calm->turbulent)
        # For TURBULENT row: probs[0] = P(turbulent->calm),  probs[1] = P(turbulent->turbulent)
        next_state = RegimeState.TURBULENT if np.random.rand() > probs[0] else RegimeState.CALM
        
        prev_state = self.state
        self.state = next_state
        
        # Log transition — use the probability of the transition that actually occurred
        actual_prob = probs[next_state.value]
        info = RegimeInfo(
            state=next_state,
            state_name=next_state.name,
            transition_prob=actual_prob,
            fg_index=fg_index,
