...
def combine_sentiment(asset: str) -> SentimentBundle:
    ...
    # Weights: Twitter 50%, Reddit 30% — normalised to their combined weight.
    # NOTE: hashtag and news overlays are intentionally excluded here; they
    # already incorporate the same Twitter/Reddit data via SentimentBusV2 and
    # adding them would double-count the social signal.  The full layered blend
    # (social + hashtag + news + FG) is available via
    # SentimentBusV2.get_asset_context() → AssetSentimentContext.
    w_tw, w_rd = 0.5, 0.3
    social_combined = (w_tw * tw + w_rd * rd) / (w_tw + w_rd)

    combined = social_combined
