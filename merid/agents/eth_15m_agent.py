"""
ETH 15m Agent — KalshiGrid integration for regime-aware ETH 15m trading.
"""
...
from config.eth_15m_agent_spec import (
    Eth15mInputs,
    Eth15mParams,
    should_trade_eth_15m,
)


class Eth15mAgent(BaseKalshiAgent):
    """ETH 15m Kalshi agent mirroring BTC 15m structure."""
    ...
    def __init__(
        ...
        self.params = params or Eth15mParams()
        ...
        log_risk_limits_for_agent("ETH", mode)
    ...
    async def _build_inputs(self) -> Optional[Eth15mInputs]:
        ...
        market = self.market_registry.get_active_eth_15m()
        if not market:
            self.logger.debug("ETH 15m: no active market, skipping.")
            return None

        rti = self.crypto_rti_monitor.get_current_metrics(symbol="ETH")
        if rti is None:
            self.logger.debug("ETH 15m: no RTI metrics, skipping.")
            return None

        vol_metrics = self.crypto_rti_monitor.get_vol_metrics(symbol="ETH")
        if vol_metrics is None:
            self.logger.debug("ETH 15m: no vol metrics, skipping.")
            return None

        is_vol_elevated = self.portfolio_risk_agent.is_crypto_vol_elevated("ETH")
    ...
    async def get_opinion(
        ...
        # WINNER ALIGNMENT FIX (2026-05-10): Check if ETH is an arbiter winner before generating opinion
        ...
        try:
            ...
            if market:
                ...
                if not is_winner:
                    self.logger.info(
                        "[ARBITERBLOCKED] ETH15M %s not in arbiter winners - skipping opinion generation",
                        market.ticker
                    )
                    return None
                
                self.logger.debug(
                    "[ARBITEROK] ETH15M %s is in arbiter winners - proceeding with opinion generation",
                    market.ticker
                )
        except Exception as e:
            self.logger.warning("[ARBITER] Winner check failed for ETH15M: %s - fail-open allowing", e)
            # Fail-open: if check fails, allow opinion generation to avoid blocking valid signals

        signal = should_trade_eth_15m(inputs, self.params)
        if signal is None:
        ...
        return AgentOpinion.for_series(
            ...
            symbol="ETH",
            timeframe="15m",
            market_id=self.market_registry.get_active_eth_15m().ticker,
            side=side,
...
_eth_15m_agent: Optional[Eth15mAgent] = None


def get_eth_15m_agent() -> Eth15mAgent:
    """Get global ETH 15m agent instance."""
    global _eth_15m_agent
    if _eth_15m_agent is None:
        _eth_15m_agent = Eth15mAgent()
    return _eth_15m_agent
