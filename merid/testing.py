...
def run_paper_scenario(
    ...
    from trading.reconciliation import run_reconciliation as _run_recon, ReconciliationReport
    ...
    try:
                ...
                try:
                    ...
                    if not report.all_ok:
                        ...
                        logger.warning(
                            ...
                            tick_idx,
                            [c.name for c in report.checks if c.status.value != "ok"],
                        )
                except Exception as exc:
        ...
        try:
            final_report = _run_recon()
            metrics.reconciliation_report = final_report.to_dict()
            if not final_report.all_ok:
                metrics.reconciliation_breaks += 1
        except Exception as exc:
