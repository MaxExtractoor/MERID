...
# Default DB path — co-located with other MERID data
_DEFAULT_DB_PATH = os.path.join("data", "calibration.db")

# EWMA decay factor for rolling Brier (0.95 = slow decay, recent matters more)
EWMA_ALPHA = 0.05

# Minimum forecasts before a weight is considered reliable
MIN_FORECASTS_FOR_WEIGHT = 10

# Default weight for forecasters with insufficient history
DEFAULT_WEIGHT = 1.0
...
class BrierStats:
    ...
    def std_brier(self) -> float:
        if self.resolved_count < 2:
            return 0.0
        var = (self.sum_brier_sq / self.resolved_count) - (self.mean_brier ** 2)
...
class CalibrationStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self._conn = sqlite3.connect(
            self._db_path,
            check_same_thread=False if in_memory else True,
        )
        ...
        with self._conn:
            self._conn.execute(
                """INSERT INTO forecasts
                   (forecaster_id, bucket, market_id, p_model, timestamp)
                   VALUES (?, ?, ?, ?, ?)""",
                (forecaster_id, bucket, market_id, p_model, ts),
            )
            # Upsert the count in brier_stats
            self._conn.execute(
    ...
    def resolve_outcome(self, market_id: str, outcome: int) -> int:
        ...
        rows = self._conn.execute(
            """SELECT id, forecaster_id, bucket, p_model
               FROM forecasts
