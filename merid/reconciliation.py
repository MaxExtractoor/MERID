...
.. warning::
    This **module file is shadowed** by the package :mod:`merid.reconciliation`
    (directory). ``import merid.reconciliation`` loads the package, **not** this
    file. Live API lives in :mod:`merid.reconciliation.venue_reconciler` and
    :mod:`merid.reconciliation.kalshi_reconciler`. Do not edit here expecting
    runtime changes — merge fixes into the package instead.
...
Usage:
    ...
    # Force-align paper engine to venue truth (Kalshi is the live PM venue in this stack)
    from merid.reconciliation import force_align_from_venue
    force_align_from_venue("kalshi")
"""
...
def _reconcile_kalshi_venue() -> List["PositionDiscrepancy"]:
    ...
    try:
        ...
        # reconcile_venue() MUST always be called from a background thread
        # (via asyncio.to_thread or run_in_executor), never from the event
        # loop thread directly — asyncio.run() below requires no running loop.
        #
        # If a running loop IS detected, we raise immediately rather than
        # falling into run_coroutine_threadsafe + Future.result() which
        # deadlocks the event loop for up to 30 s and defeats wait_for.
        try:
            _asyncio.get_running_loop()
            raise RuntimeError(
                "reconcile_venue() called from event loop thread — "
                "wrap with await asyncio.to_thread(reconcile_venue, venue_name) instead"
            )
        except RuntimeError as _loop_check_exc:
            if "reconcile_venue() called" in str(_loop_check_exc):
                raise
            # No running loop — safe to spin up a new one
            pass
    ...
    if discrepancies:
        ...
        # R3: Critical discrepancies trigger kill switch recommendations
        if n_crit > 0:
    ...
    # branches require merid_qty != 0 before firing settlement hooks.
    settled_symbols = [
    ...
    # Supplement with fills-ledger computed positions (catches CT trades that never
    # write to the paper engine).  Any ticker with fills-ledger open position but no
    # Kalshi REST position has settled.
    try:
        from merid.event_venues.kalshi.fills_ledger import get_fills_ledger
        _ledger = get_fills_ledger()
        _ledger_positions = _ledger.compute_net_positions()
        for _ticker, _net in _ledger_positions.items():
            if (
                _net.get("contracts", 0) > 0
                and _ticker not in venue_map
                and _ticker not in settled_symbols
            ):
                settled_symbols.append(_ticker)
    except Exception as _fl_exc:
        logger.debug("fills_ledger supplement for settled_symbols skipped: %s", _fl_exc)
    if settled_symbols:
...
def _fetch_kalshi_settlement(market_id: str) -> Optional[bool]:
    ...
    if loop is not None and loop.is_running():
        # Called from a background thread while the event loop is running.
        # Use run_coroutine_threadsafe so the coroutine executes on the loop thread.
        import concurrent.futures as _cf
        try:
            fut = _asyncio.run_coroutine_threadsafe(_fetch(), loop)
            return fut.result(timeout=5.0)
        except (_cf.TimeoutError, Exception) as _e:
            logger.debug("settlement fetch threadsafe timeout/error for %s: %s", market_id, _e)
            return None
...
def _fire_settlement_hooks(market_id: str) -> None:
    ...
    try:
        ...
        # Use composite-key scan to match — bare market_id lookup is always False.
        _matching_keys = [k for k in tracker._open_trades if k.endswith(f":{market_id}")]
        if _matching_keys:
            rec = tracker._open_trades[_matching_keys[0]]
...
def has_critical_discrepancies() -> bool:
    ...
    Returns False when reconciliation has never run — the startup task
    populates state within seconds; fail-closed blocked execution permanently
    on every restart even with a clean account.
    """
    with _recon_lock:
...
def force_align_from_venue(venue_name: str, user_id: str = "operator") -> Dict[str, Any]:
    ...
    if venue_name == "kalshi":
        # Match reconcile_venue("kalshi"): canonical KalshiVenueAdapter (cache-first
        # positions), not only KalshiVenueClient via trading.registry adapter.
        try:
            import asyncio as _asyncio
...
def _recommend_kill_switch_for_discrepancies(discrepancies: List[PositionDiscrepancy]) -> None:
    """Recommend kill switch activation for critical reconciliation discrepancies.
    
    R3 Integration: Reconciliation discrepancies above threshold trigger
    per-asset and per-market kill switch recommendations. This ensures
    trading halts when local state diverges from venue reality.
    """
    critical_discrepancies = [d for d in discrepancies if d.severity == "critical"]
    if not critical_discrepancies:
        return
    
    # Group by asset to recommend per-asset kills
    assets_affected = set()
    markets_affected = set()
    
    for disc in critical_discrepancies:
        symbol = disc.symbol
        markets_affected.add(symbol)
        
        # Extract asset from Kalshi ticker
        if symbol.startswith("KXBTC"):
            assets_affected.add("BTC")
        elif symbol.startswith("KXETH"):
            assets_affected.add("ETH")
        elif symbol.startswith("KXSOL"):
            assets_affected.add("SOL")
        elif symbol.startswith("KXXRP"):
            assets_affected.add("XRP")
        elif symbol.startswith("KXDOGE"):
            assets_affected.add("DOGE")
    
    # Log recommendations (operator must confirm actual kill)
    logger.critical(
        "KILL SWITCH RECOMMENDED due to critical reconciliation discrepancies",
        extra={
            "assets_affected": list(assets_affected),
            "markets_affected": list(markets_affected),
            "critical_count": len(critical_discrepancies),
            "action_required": "Review discrepancies and confirm kill switch activation",
        }
    )
    
    # Attempt to auto-block markets via AutoPromoter if available
    try:
        from merid.promotion.auto_promoter import get_auto_promoter
        promoter = get_auto_promoter()
        
        for market in markets_affected:
            for agent_id in list(promoter.get_all_statuses().keys()):
                status = promoter.get_status(agent_id)
                if status and market in status.blocked_markets:
                    continue
                promoter.block_market(agent_id, market, "critical_reconciliation_discrepancy")
                logger.warning(f"Auto-blocked market {market} for agent {agent_id}")
    except Exception as exc:
        logger.debug(f"Auto-promoter block failed: {exc}")
