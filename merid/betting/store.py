...
class BettingStore:
    ...
    def _recover_db(self) -> None:
        """Rename the corrupt DB file and re-initialise from scratch."""
        if self._db_path == ":memory:" or not os.path.exists(self._db_path):
            return
        backup = f"{self._db_path}{_CORRUPT_SUFFIX}"
        try:
            os.replace(self._db_path, backup)
            logger.warning("betting_db_corrupted", backup=backup, action="recreating")
        except OSError as exc:
            logger.error("betting_db_backup_failed", error=str(exc))
        self._init_db()
    ...
    def _init_db(self) -> None:
        ...
        with self._lock:
            ...
            conn.executescript("""
                ...
                """)
            # Detect pre-existing corruption early
            if self._db_path != ":memory:":
                result = conn.execute("PRAGMA quick_check").fetchone()
                if result and result[0] != "ok":
                    logger.error("betting_db_integrity_failed", result=result[0])
                    raise sqlite3.DatabaseError(f"quick_check: {result[0]}")
    ...
    def store_odds_snapshot(self, snap: OddsSnapshot) -> None:
        try:
            self._store_odds_snapshot_inner(snap)
        except sqlite3.DatabaseError as exc:
            logger.error("betting_db_write_failed", error=str(exc), action="recovering")
            self._recover_db()

    def _store_odds_snapshot_inner(self, snap: OddsSnapshot) -> None:
        with self._lock:
            with self._connect() as conn:
                conn.execute(
