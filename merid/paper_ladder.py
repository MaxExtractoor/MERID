...
class PaperLadder:
    ...
    def seed_portfolio(self, portfolio_id: str, tier_override: Optional[int] = None) -> LadderProgress:
        ...
        # Apply to paper trading engine and sync risk caps
        self._apply_to_paper_engine(portfolio_id, tier.seed_usd)
        self._apply_risk_caps(tier)
    ...
    def _promote(self, p: LadderProgress) -> str:
        ...
        self._apply_risk_caps(new_tier)
    ...
    def _apply_risk_caps(self, tier: "LadderTier") -> None:
        """Sync PredictionRiskConfig caps to the current ladder tier.

        Notional caps scale with tier seed capital:
          - max_total_notional_usd  = seed_usd * 0.50  (50% utilisation ceiling)
          - max_notional_per_market = seed_usd * 0.05  (5% per single market)
          - max_notional_per_event  = seed_usd * 0.10  (10% per event)
          - max_daily_loss_usd      = seed_usd * max_drawdown_pct / 100
        """
        from decimal import Decimal as _D
        try:
            from merid.prediction.risk import get_prediction_risk
            risk = get_prediction_risk()
            seed = _D(str(tier.seed_usd))
            risk.config.max_total_notional_usd = (seed * _D("0.50")).quantize(_D("0.01"))
            risk.config.max_notional_per_market_usd = (seed * _D("0.05")).quantize(_D("0.01"))
            risk.config.max_notional_per_event_usd = (seed * _D("0.10")).quantize(_D("0.01"))
            risk.config.max_daily_loss_usd = (
                seed * _D(str(tier.max_drawdown_pct)) / _D("100")
            ).quantize(_D("0.01"))
            logger.info(
                "[ladder] Risk caps updated for tier %s (%s): "
                "total=$%.0f market=$%.0f event=$%.0f daily_loss=$%.0f",
                tier.level, tier.name,
                float(risk.config.max_total_notional_usd),
                float(risk.config.max_notional_per_market_usd),
                float(risk.config.max_notional_per_event_usd),
                float(risk.config.max_daily_loss_usd),
            )
        except Exception as e:
            logger.warning("[ladder] Could not update risk caps: %s", e)
