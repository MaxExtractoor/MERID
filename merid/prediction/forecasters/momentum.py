...
class MomentumForecaster(Forecaster):
    ...
    def __init__(self) -> None:
        # Per-instance history keyed by market_id — avoids cross-agent
        # contamination when multiple MomentumForecaster instances run
        # concurrently for different asset/timeframe cells.
        self._momentum_history: Dict[str, list] = defaultdict(list)
    ...
    def _volume_momentum(self, market_id: str, volume: float) -> Optional[float]:
        ...
        """
        history = self._momentum_history.get(market_id, [])
        if len(history) < 3:
            return None
    ...
    def _record_observation(
        ...
        self._momentum_history[market_id].append({
            "implied_yes": implied_yes,
            "volume": volume,
            "oi": oi,
        })
        if len(self._momentum_history[market_id]) > _MAX_HISTORY:
            self._momentum_history[market_id] = self._momentum_history[market_id][-_MAX_HISTORY:]
