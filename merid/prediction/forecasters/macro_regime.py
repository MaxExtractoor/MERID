...
class MacroRegimeForecaster(Forecaster):
    ...
    def _get_xtf_signal(self, asset: str, timeframe: str, kwargs: Dict) -> float:
        ...
        If multiple timeframes agree on direction, signal is stronger.
        """
        xtf_agreement = kwargs.get("xtf_agreement", None)

        if xtf_agreement is None:
            try:
                from merid.prediction.xtf_aggregator import get_xtf_aggregator
                agg = get_xtf_aggregator()
                signal = agg.get_signal(asset.upper())
                if signal:
                    xtf_agreement = signal.get("agreement", 0.0)
            except Exception:
                pass
        ...
        # agreement is typically -1 to +1 (all bearish to all bullish)
    ...
    def _get_vol_regime(self, asset: str, kwargs: Dict) -> float:
        ...
        if vol is None:
            try:
                from merid.event_venues.kalshi.vol_surface import get_vol_surface
                surface = get_vol_surface()
                vol = surface.get_atm_vol(asset.upper())
            except Exception:
                pass
