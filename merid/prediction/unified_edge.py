...
class UnifiedEdgeComputer:
    ...
    def check_edge(
        ...
        min_price_cents = 20  # CRITICAL: Default fallback (20 cents / $0.20) - 10c allowed losing deep OTM trades
        try:
            from merid.risk.profiles.crypto_15m_profile import get_active_profile
            profile_adapter = get_active_profile()
        ...
        # Set to 50¢ to match profile guardrails_max_contract_price_cents (10-50c sweet spot)
        ...
        if spread_cents is not None:
            ...
            max_spread_for_edge = 20  # Default fallback
            try:
                from merid.risk.profiles.crypto_15m_profile import get_active_profile
                profile_adapter = get_active_profile()
                if profile_adapter and hasattr(profile_adapter.profile, 'guardrails_max_spread_for_edge'):
                    ...
                    if edge_pct < 1.0:
                        ...
                        max_spread_for_edge = edge_pct_map.get("default", 20)
            except Exception as e:
                logger.debug("[EDGE-CHECK] Failed to load max_spread_for_edge from profile: %s, using default 20c", e)
