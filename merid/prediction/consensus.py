...
class PredictionInstrument:
    ...
    def to_dict(self) -> Dict[str, Any]:
        d: Dict[str, Any] = {
            ...
            "title": self.title,
            "expiry": datetime.fromtimestamp(float(self.expiry), tz=timezone.utc).isoformat() if self.expiry is not None else None,
...
class PredictionOpinion:
    ...
    def to_dict(self, include_explanation: bool = True) -> Dict[str, Any]:
        d: Dict[str, Any] = {
            ...
            "timestamp": datetime.fromtimestamp(float(self.created_at), tz=timezone.utc).isoformat(),
        }
...
class ResolvedMarket:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "symbol": self.symbol,
            "outcome": self.outcome,
            "resolved_at": datetime.fromtimestamp(float(self.resolved_at), tz=timezone.utc).isoformat(),
...
class PredictionConsensusStore:
    ...
    def __init__(self, db_path: str = _DB_PATH):
        ...
        self._brier_weights_cache: Dict[str, float] = {}
        self._brier_weights_ts: float = 0.0
        self._brier_weights_ttl: float = 300.0  # 5-minute cache
        self._init_db()
    ...
    def _build_consensus_summary(self, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        brier_weights = self.get_agent_brier_weights()
        summaries: List[Dict[str, Any]] = []
        for inst in instruments:
            opinions = opinions_by_symbol.get(inst.symbol, [])
            plans = plans_by_symbol.get(inst.symbol, [])

            swarm_prob = self._aggregate_swarm_prob_brier_weighted(opinions, brier_weights)
            ...
            summaries.append({
                ...
                "title": inst.title,
                "expiry": datetime.fromtimestamp(float(inst.expiry), tz=timezone.utc).isoformat() if inst.expiry is not None else None,
        ...
        # B9: Publish significant-edge consensus decisions to event bus (→ Telegram)
        try:
            ...
            if loop.is_running():
                for _s in summaries:
                    if abs(_s.get("edge", 0.0)) >= 0.05:   # only meaningful edges
                        direction = (
                            "YES" if _s.get("stance", "neutral").endswith("yes")
                            else ("NO" if _s.get("stance", "neutral").endswith("no") else "NEUTRAL")
                        )
                        _asyncio.ensure_future(_es.publish("kalshi:consensus_decision", {
                            "ticker": _s.get("ticker", ""),
                            "direction": direction,
                            "consensus_prob": _s.get("swarm_prob", 0.0),
                            "ev": _s.get("edge", 0.0),
                            "confidence": _s.get("confidence", 0.0),
                            "n_agents": _s.get("opinion_count", 0),
                            "mode": "live",
                        }))
        except Exception as _evt_exc:
            logger.debug("Consensus summary event publish failed: %s", _evt_exc)
    ...
    # ── Brier-calibrated agent weights ───────────────────────────────────

    def get_agent_brier_weights(self, window: int = 50) -> Dict[str, float]:
        """Per-agent consensus weights derived from historical Brier scores.

        weight(a) = 1 / (brier(a) + 0.01) — lower Brier → higher weight.
        Normalised so weights sum to len(agents), keeping the scale neutral.
        Agents absent from the weight map fall back to self-reported
        confidence inside _aggregate_swarm_prob_brier_weighted.
        Result cached for _brier_weights_ttl seconds (default 5 min).
        """
        now = time.monotonic()
        if (
            self._brier_weights_cache
            and (now - self._brier_weights_ts) < self._brier_weights_ttl
        ):
            return self._brier_weights_cache

        brier_data = self.compute_brier_scores(window=window)
        agent_briers: Dict[str, float] = brier_data.get("agent_briers", {})
        if not agent_briers:
            return {}

        raw = {a: 1.0 / (b + 0.01) for a, b in agent_briers.items()}
        total = sum(raw.values())
        n = len(raw)
        normalised = {a: (w / total) * n for a, w in raw.items()}

        self._brier_weights_cache = normalised
        self._brier_weights_ts = now
        return normalised

    @staticmethod
    def _aggregate_swarm_prob_brier_weighted(
        opinions: List["PredictionOpinion"],
        agent_weights: Dict[str, float],
    ) -> float:
        """Brier-calibrated weighted average of agent probabilities.

        Falls back to self-reported confidence for agents absent from the
        weight map, and to plain confidence-weighting when no map exists.
        """
        if not opinions:
            return 0.5
        if not agent_weights:
            return PredictionConsensusStore._aggregate_swarm_prob(opinions)

        total_w = 0.0
        weighted_sum = 0.0
        for op in opinions:
            w = agent_weights.get(op.agent_id, op.confidence)
        ...
        for r in rows:
            ...
            if explanation_raw:
                try:
                    ...
                    logger.debug("silent catch in consensus:990")
    ...
    def _row_to_opinion(row: sqlite3.Row) -> PredictionOpinion:
        ...
        if explanation_raw:
            try:
                ...
                logger.debug("silent catch in consensus:1086")
