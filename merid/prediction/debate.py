...
class DebateStore:
    ...
    def has_arbiter_argument(self, debate_id: str) -> bool:
        """Return True if the debate has at least one argument with role='arbiter'."""
        with self._connect() as conn:
            row = conn.execute(
                "SELECT COUNT(*) as cnt FROM pred_debate_args "
                "WHERE debate_id = ? AND role = 'arbiter'",
                (debate_id,),
            ).fetchone()
    ...
    def close_debate(
        ...
        H3 fix: if ``require_arbiter`` is True (default), the close is rejected
        unless at least one argument with role='arbiter' exists.  Callers that
        want to close without an arbiter (e.g., test scaffolding) must explicitly
        pass ``require_arbiter=False``.
        """
        if require_arbiter and not self.has_arbiter_argument(debate_id):
            logger.warning(
                "close_debate rejected for %s: no arbiter-role argument present (H3). "
                "Post an arbiter argument before closing, or pass require_arbiter=False.",
                debate_id,
            )
            return None
        now = time.time()
    ...
    def compute_rewards_for_resolution(
        ...
        for deb in debates:
            ...
            # H3: detect same-model debate (all args share same rationale prefix)
            rationale_prefixes = {
                a.rationale.split("_")[0] if a.rationale else "unknown" for a in args
            }
            same_model = len(rationale_prefixes) == 1 and len(args) >= 2

            # H3: check whether any argument carries a numeric data reference
            def _has_data_reference(arg: DebateArgument) -> bool:
                """True if the argument's explanation contains a numeric data reference."""
                if not arg.explanation:
                    return False
                refs = arg.explanation.get("data_references", [])
                if refs:
                    return True
                # Fallback: check contributions dict for non-zero numeric values
                contribs = arg.explanation.get("contributions", {})
                return any(isinstance(v, numbers.Real) and not isinstance(v, bool) and v != 0 for v in contribs.values())

            debate_has_data_ref = any(_has_data_reference(a) for a in args)

            for arg in args:
                if deb.debate_lift is not None:
                    existing = agent_debate_lift.get(arg.agent_id, 0.0)
                    lift_value = deb.debate_lift
                    # H3: discount 20% if debate was same-model (no epistemic diversity)
                    if same_model:
                        lift_value = lift_value * 0.8
                    # H3: zero out lift reward if no argument provided a data reference
                    if not debate_has_data_ref:
                        lift_value = 0.0
                        logger.debug(
                            "Debate %s lift reward zeroed: no argument contained a "
                            "numeric data reference (H3 fact-check gate)",
                            deb.id,
                        )
                    agent_debate_lift[arg.agent_id] = max(existing, lift_value)
    ...
    def _row_to_argument(row: sqlite3.Row) -> DebateArgument:
        ...
        if explanation_raw:
            try:
                ...
                logger.debug("silent catch in debate:945")
