...
# ═══════════════════════════════════════════════════════════════════════════════
# FVG Production Configuration (from Profile YAML)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _load_fvg_config_from_profile():
    ...
    try:
        ...
        return {
            'enabled': True,  # FVG is always enabled in production
            'min_price_change_cents': 0.5,
            'base_confidence': 0.85,
            'proximity_boost': 0.15,
            'max_edge_pct': 0.08,
            'min_confluence_score': 0.30,
            'size_multiplier': 1.25,
            'max_position_boost': 1.5,
            'filled_zone_size_factor': 0.75,
            'entry_timing_enabled': True,
            'entry_proximity_cents': 3.0,
            'exit_timing_enabled': True,
            'exit_proximity_cents': 2.0,
            'hold_on_confluence_threshold': 0.60,
            'max_trades_per_hour': 4,
            'entry_debounce_seconds': 900,
            'opposing_fvg_cancel_cents': 5.0,
            'log_signals': True,
            'large_gap_alert_cents': 10.0,
            'min_gap_cents': getattr(momentum_fvg_config, 'fvg_min_gap_cents', 2.0),
            'fill_threshold': getattr(momentum_fvg_config, 'fvg_fill_threshold_cents', 5.0),
        }
    except Exception as e:
        logger.warning("[FVG-INT] Failed to load config from profile, using defaults: %s", e)
        return {
            'enabled': True,
            'min_price_change_cents': 0.5,
            'base_confidence': 0.85,
...
_FVG_ENABLED = _FVG_CONFIG['enabled']
_FVG_MIN_PRICE_CHANGE_CENTS = _FVG_CONFIG['min_price_change_cents']
_FVG_BASE_CONFIDENCE = _FVG_CONFIG['base_confidence']
_FVG_PROXIMITY_BOOST = _FVG_CONFIG['proximity_boost']
_FVG_MAX_EDGE_PCT = _FVG_CONFIG['max_edge_pct']
_FVG_MIN_CONFLUENCE_SCORE = _FVG_CONFIG['min_confluence_score']
_FVG_SIZE_MULTIPLIER = _FVG_CONFIG['size_multiplier']
_FVG_MAX_POSITION_BOOST = _FVG_CONFIG['max_position_boost']
_FVG_FILLED_ZONE_SIZE_FACTOR = _FVG_CONFIG['filled_zone_size_factor']
_FVG_ENTRY_TIMING_ENABLED = _FVG_CONFIG['entry_timing_enabled']
_FVG_ENTRY_PROXIMITY_CENTS = _FVG_CONFIG['entry_proximity_cents']
_FVG_EXIT_TIMING_ENABLED = _FVG_CONFIG['exit_timing_enabled']
_FVG_EXIT_PROXIMITY_CENTS = _FVG_CONFIG['exit_proximity_cents']
_FVG_HOLD_ON_CONFLUENCE_THRESHOLD = _FVG_CONFIG['hold_on_confluence_threshold']
_FVG_MAX_TRADES_PER_HOUR = _FVG_CONFIG['max_trades_per_hour']
_FVG_ENTRY_DEBOUNCE_SECONDS = _FVG_CONFIG['entry_debounce_seconds']
_FVG_OPPOSING_FVG_CANCEL_CENTS = _FVG_CONFIG['opposing_fvg_cancel_cents']
_FVG_LOG_SIGNALS = _FVG_CONFIG['log_signals']
_FVG_LARGE_GAP_ALERT_CENTS = _FVG_CONFIG['large_gap_alert_cents']


def _get_asset_config(asset: str) -> Dict[str, float]:
    """Get asset-specific FVG configuration from profile YAML."""
    asset = (asset or "BTC").upper()
    # CRITICAL FIX: 2026-07-06 - Use profile YAML values, not environment variables
    # All assets use the same FVG parameters from profile YAML
    return {
        "min_gap_cents": _FVG_CONFIG['min_gap_cents'],
        "fill_threshold": _FVG_CONFIG['fill_threshold'],
    }
...
class FVGIntegrator:
    ...
    def __init__(self) -> None:
        ...
        if self._enabled:
            ...
            logger.info("FVGIntegrator initialized - FVG analysis DISABLED")
