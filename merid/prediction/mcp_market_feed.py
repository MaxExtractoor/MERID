...
class MCPMarketFeed:
    ...
    async def _poll_loop(self) -> None:
        ...
        while not self._shutdown.is_set():
            ...
            try:
                ...
                logger.debug("silent catch in mcp_market_feed:172")
    ...
    async def fetch_markets(
        ...
        try:
            ...
            # aiohttp not available — use httpx async fallback (never sync urllib)
            try:
                import httpx
                async with httpx.AsyncClient(timeout=self._config.timeout_s) as client:
                    ...
                    if resp.status_code == 200:
                        ...
                        self._error_count += 1
                        logger.warning(f"MCP httpx fallback returned {resp.status_code}")
            except Exception as exc:
                self._error_count += 1
                logger.error(f"MCP async fallback fetch failed: {exc}")
        except Exception as exc:
            self._error_count += 1
