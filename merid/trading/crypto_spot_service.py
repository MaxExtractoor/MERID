                ...
                if api_timestamp:
                    try:
                        ...
                        if age_seconds > 30.0:
                            ...
                            # Reject stale data
                            return None
                        logger.debug("Coinbase v2 success: %s = %s (age=%.1fs)", asset, format_price(asset, price), age_seconds)
                        ...
                        logger.debug("Coinbase v2 success: %s = %s (timestamp unavailable)", asset, format_price(asset, price))
                else:
                    logger.debug("Coinbase v2 success: %s = %s (no timestamp in response)", asset, format_price(asset, price))
                ...
                if api_time:
                    try:
                        ...
                        if age_seconds > 30.0:
                            ...
                            # Reject stale data
                            return None
                        logger.debug("Coinbase Exchange success: %s = %s (age=%.1fs)", asset, format_price(asset, price), age_seconds)
                    except (ValueError, TypeError) as e:
                        logger.debug("Coinbase Exchange timestamp parse failed for %s: %s", asset, e)
                        # If timestamp parsing fails, accept price but log warning
                        logger.debug("Coinbase Exchange success: %s = %s (timestamp unavailable)", asset, format_price(asset, price))
                else:
                    logger.debug("Coinbase Exchange success: %s = %s (no timestamp in response)", asset, format_price(asset, price))
                self._clear_venue_error("coinbase")
                return price
        except Exception as e:
