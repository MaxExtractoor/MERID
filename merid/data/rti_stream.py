...
class RTIWindow:
    ...
    def realized_var(self) -> float:
        """Realized variance over 1s simple returns."""
    ...
    def realized_vol(self) -> float:
        """Realized volatility over the current window (not annualized)."""
        var = self.realized_var()
        return var ** 0.5 if var > 0 else 0.0

    def is_full_window(self) -> bool:
        """Check if window contains a full time range."""
        if len(self.samples) < 2:
            return False
        return self.samples[-1][0] - self.samples[0][0] >= self.window_sec
...
class RTIStream:
    ...
    def add_rti_tick(self, asset: str, price: float, ts: float | None = None):
        ...
        # Update per-minute SMA if we've accumulated a full minute
        if window.is_full_window():
            sma = window.sma()
    ...
    def get_current_metrics(self, asset: str) -> dict[str, float]:
        ...
        return {
            ...
            "rti_60s_sma": window.sma(),           # settlement proxy
            "rti_60s_vol": window.realized_vol(),  # short-horizon vol
            "rti_sma_trend": sma_window.realized_vol() if len(sma_window.samples) > 1 else 0.0,  # vol of 1-min averages
        }
