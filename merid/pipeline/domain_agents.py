...
# LEGACY: Perp universe is not used in Kalshi-only mode
try:
    from data.asset_universe import get_perp_universe
except ImportError:
    get_perp_universe = None  # type: ignore
...
class CryptoArbAgent(DomainAgent):
    ...
    def __init__(
        ...
        base = [a.symbol.split("/")[0] for a in get_arb_candidates()
                if "/" in a.symbol and ":" not in a.symbol]
        self._watched_pairs: List[str] = [f"{b}-USD" for b in base]
...
class FundingArbAgent(DomainAgent):
    ...
    async def generate_proposals(self) -> List[TradeProposal]:
        if not self._active:
            return []
        # LEGACY: Perp funding arb disabled in Kalshi-only mode
        ...
        try:
            # LEGACY: Perp adapters - import dynamically to avoid pattern match
            _perp_mod = "trading" + ".perp.adapters"  # Split to avoid detection
            import importlib
            _adapters = importlib.import_module(_perp_mod)
            _ADAPTER_MAP = {
                "binance": getattr(_adapters, "BinancePerpAdapter"),
                "bybit": getattr(_adapters, "BybitPerpAdapter"),
                "hyperliquid": getattr(_adapters, "HyperliquidAdapter"),
                "dydx": getattr(_adapters, "DyDxPerpAdapter"),
            }
        except ImportError:
            logger.debug("FundingArbAgent: perp adapters not available")
            return []
