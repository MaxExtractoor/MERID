...
class RiskRouteMode(Enum):
    """Risk-layer routing decision (NOT the process-wide trade mode).

    ZT2-11: Renamed from TradeMode to avoid collision with the canonical
    ``trading.trade_mode.TradeMode``.  This enum is specific to the
    crypto-swarm risk layer and includes BLOCKED which has no equivalent
    in the process-wide mode.
    """
    LIVE = "live"
    PAPER = "paper"
    BLOCKED = "blocked"

# Backward-compat alias so existing serialized data still deserializes.
TradeMode = RiskRouteMode
...
class CryptoSwarmRiskBTC15m:
    ...
    def _determine_route(self, proposal: TradeProposal) -> RiskDecision:
        ...
        Phase 0 rules (multi-asset):
        - Any of the five crypto 15m lanes: eligible for live
        - Everything else: forced to paper
        """
        is_crypto = proposal.asset.upper() in ALL_CRYPTO_ASSETS
        is_15m = proposal.timeframe == "15m"

        if not is_crypto or not is_15m:
            return RiskDecision(
                mode=TradeMode.PAPER,
                final_size=proposal.intent_risk,
                reason=(
                    f"{proposal.asset} {proposal.timeframe} forced to paper "
                    f"(Phase 0: crypto 15m only live)"
                ),
            )
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fear_greed": {
                "value": self.fear_greed_cache.value if self.fear_greed_cache else 50,
                "zone": self.fear_greed_cache.zone if self.fear_greed_cache else "neutral",
            },
