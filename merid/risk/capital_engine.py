"""CapitalEngine — Exponential money loop for the MERID ↔ Kalshi crypto system.
...
Drawdown guard:
  - If risk capital drops ≥ drawdown_stepdown_threshold from its peak,
...
# ── Per-asset capital configuration ──────────────────────────────────────
...
# ── Per-asset/timeframe risk budget ──────────────────────────────────────
...
# ── Capital snapshot (for telemetry / UI) ────────────────────────────────
...
def _default_risk_budgets() -> List[RiskBudget]:
    ...
    Intraday BTC/ETH allows up to 2–3% of risk capital per trade;
...
class CapitalEngine:
    ...
    def sizing_multiplier(self) -> float:
        """Current drawdown-based global sizing multiplier (0–1)."""
    ...
    def get_risk_capital(self, asset: str) -> float:
        ...
        if cfg is None:
            logger.warning("CapitalEngine: unknown asset %r — returning zero risk capital", asset)
            return 0.0
    ...
    def allocate_to_trade(
        ...
        *,
        ...
        Args:
            ...
            conviction:      Structural conviction score (0–1).  Values ≥ 0.8
                             unlock growth-bucket extra size.

        Returns:
            Final allocated size (≥ 0, never exceeds risk budgets).
        """
        cfg = self._asset_configs.get(asset)
        if cfg is None:
            return 0.0
    ...
    def record_trade_result(self, asset: str, realized_pnl: float) -> None:
        ...
        if realized_pnl > 0:
            ...
            logger.debug(
                "CapitalEngine [%s] profit +%.4f → core +%.4f growth +%.4f risk +%.4f",
            ...
            logger.debug(
                "CapitalEngine [%s] loss -%.4f → risk -%.4f growth -%.4f",
    ...
    def _update_drawdown_state(self, asset: str) -> None:
        ...
        if drawdown_frac >= cfg.drawdown_stepdown_threshold:
            if self._sizing_multiplier == 1.0:
                logger.warning(
                    "CapitalEngine [%s]: drawdown %.1f%% ≥ threshold %.1f%% — "
            ...
            if recovery_frac >= cfg.drawdown_recovery_threshold and self._sizing_multiplier < 1.0:
                ...
                logger.info(
                    "CapitalEngine [%s]: risk capital recovered to %.1f%% of peak — "
