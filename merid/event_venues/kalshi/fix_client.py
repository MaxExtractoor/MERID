        ...
        try:
            ...
            def _task_done_cb(task: asyncio.Task) -> None:
                if not task.cancelled() and task.exception():
                    logger.error("KalshiFIXClient task crashed: %s", task.exception())
            self._heartbeat_task = asyncio.create_task(self._heartbeat_loop(), name="fix-heartbeat")
            self._heartbeat_task.add_done_callback(_task_done_cb)
            self._listen_task = asyncio.create_task(self._listen_loop(), name="fix-listen")
            self._listen_task.add_done_callback(_task_done_cb)
        ...
        def get_price(key: str) -> Optional[int]:
            ...
            if val:
                try:
                    ...
                    logger.debug("silent catch in fix_client:411")
            return None
