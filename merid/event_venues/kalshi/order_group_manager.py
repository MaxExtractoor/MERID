...
class OrderGroupState:
    ...
    def utilization_pct(self) -> float:
        """Return used_contracts as a percentage of contracts_limit."""
        if not self.contracts_limit:
            return 0.0
        return 100.0 * self.used_contracts / self.contracts_limit
    ...
    def record_new_order(self, contracts: int) -> None:
        """Optimistically record a new order (increases used_contracts)."""
        self.used_contracts += contracts

    def record_fill(self, contracts: int) -> None:
        """Record a fill: decreases used_contracts (clamp at 0), increases matched."""
        self.used_contracts = max(0, self.used_contracts - contracts)
        self.matched_contracts += contracts

    def release_reservation(self, contracts: int) -> None:
        """Release reserved contracts on failure/rejection.

        This ONLY decrements used_contracts and NEVER touches matched_contracts.
        Use this (not record_fill) when an order fails or is rejected.

        Args:
            contracts: Number of reserved contracts to release
        """
        self.used_contracts = max(0, self.used_contracts - contracts)
        # Note: matched_contracts is NOT incremented - no match occurred
...
class OrderGroupRiskManager:
    ...
    def get_group(self, og_id: str) -> Optional[OrderGroupState]:
        """Get order group state from cache.

        Args:
            og_id: Group ID

        Returns:
            OrderGroupState or None if not in cache (call refresh_all to populate)
        """
        return self.groups.get(og_id)

    def can_add_contracts(self, og_id: str, new_contracts: int) -> bool:
        """Check if we can add contracts to this group.

        BUG-7 fix: on a cache miss we previously returned False unconditionally,
        blocking ALL group-assigned orders after a restart or after refresh_all()
        cleared the cache.  We now return True (permissive default) and schedule
        an async refresh so the next call will have real data.  This avoids a
        full minute of phantom rejections during the 60-second sync interval.
        ...
        Returns:
            True if within limit (or if group is not yet in cache)
        """
        state = self.groups.get(og_id)
        if not state:
            # Unknown group — schedule a background refresh and allow optimistically.
            # The next call (after refresh completes) will have accurate state.
            ...
            logger.debug(
                "[order-group-risk] group %s not in cache, scheduling refresh; "
                "allowing optimistically",
                og_id,
            )
            return True
    ...
    def record_fill(self, og_id: str, matched_contracts: int) -> None:
        """Record a fill (decreases used_contracts, increases matched_contracts).
        ...
        state = self.groups.get(og_id)
        if state:
            state.used_contracts = max(0, state.used_contracts - matched_contracts)
            state.matched_contracts += matched_contracts

    def release_reservation(self, og_id: str, contracts: int) -> None:
        """Release reserved contracts on failure/rejection.

        This ONLY decrements used_contracts and NEVER increments matched_contracts.
        Use this (not record_fill) when an order fails or is rejected.
        ...
        state = self.groups.get(og_id)
        if state:
            state.release_reservation(contracts)
            logger.debug(
                "[order-group-risk] Released reservation for %s: %d contracts",
                og_id, contracts
            )
...
class KalshiOrderGroupManager:
    ...
    async def start_ws(self) -> None:
        ...
        self._ws_task = asyncio.create_task(self._ws_loop(), name="order-group-ws")
        def _task_done_cb(task: asyncio.Task) -> None:
            if not task.cancelled() and task.exception():
                logger.error("OrderGroupManager WS task crashed: %s", task.exception())
        self._ws_task.add_done_callback(_task_done_cb)
    ...
    async def _handle_ws_message(self, msg: Dict[str, Any]) -> None:
        ...
        # Update local state (lock protects against concurrent refresh_all)
        lock = self._ensure_lock()
        async with lock:
    ...
    async def update_from_ws(self, order_group_id: str, group_data: Dict[str, Any]) -> None:
