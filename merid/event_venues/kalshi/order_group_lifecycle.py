...
class OrderGroupLifecycleManager:
    ...
    async def start(self) -> None:
        ...
        def _task_done_cb(task: asyncio.Task) -> None:
            ...
            exc = task.exception()
            if exc is not None:
                logger.error("OrderGroupLifecycleManager task %s crashed: %s", task.get_name(), exc, exc_info=exc)

        self._sync_task = asyncio.create_task(self._sync_loop(), name="og-lifecycle-sync")
        self._sync_task.add_done_callback(_task_done_cb)
        self._health_task = asyncio.create_task(self._health_loop(), name="og-lifecycle-health")
        self._health_task.add_done_callback(_task_done_cb)
    ...
    async def _sync_with_rest(self) -> None:
        ...
        # is created and immediately garbage-collected without executing.
        await self._risk_manager.refresh_all()
