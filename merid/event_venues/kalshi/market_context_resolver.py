...
Built on top of the mapping builder to provide complete context
resolution for signal generation and execution.
"""
...
logger = get_logger("merid.event_venues.kalshi.market_context_resolver")


class MarketContextResolver:
    """Resolves complete market context using mapping builder and store"""
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        """Get coverage statistics for dashboards"""
        try:
            # Get mapping stats from registry
            mapping_stats = self._mapping_registry.build_all_mappings()
            
            # Get safe contexts count
            safe_contexts = self.get_safe_contexts()
            
            # Get context validation stats
            all_mappings = self._mapping_registry.get_enabled_mappings()
            validation_stats = {
                "total_enabled": len(all_mappings),
                "safe_to_trade": len(safe_contexts),
                "unsafe": len(all_mappings) - len(safe_contexts),
            }
            
            return {
                "mapping_stats": mapping_stats,
                "validation_stats": validation_stats,
                "safe_contexts_count": len(safe_contexts),
                "timestamp": time.time(),
            }
            ...
            return {
                "mapping_stats": {"total_markets": 0, "mapped_markets": 0, "enabled_markets": 0},
                "validation_stats": {"total_enabled": 0, "safe_to_trade": 0, "unsafe": 0},
                "safe_contexts_count": 0,
                "timestamp": time.time(),
                "error": str(e),
            }
