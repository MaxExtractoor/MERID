...
class KalshiMarketCatalog:
    ...
    async def refresh(self, force: bool = False) -> int:
        ...
        from merid.event_venues.kalshi.market_state import get_kalshi_market_state_store
        # CRITICAL FIX (2026-08-02): Skip state store feed during catalog refresh to prevent deadlock
        # The catalog refresh thread cannot safely call apply_rest_market due to threading issues
        # State store will be populated by other mechanisms (WS bridge, REST sync)
        logger.info("[CATALOG-FEED] Skipping state store feed during catalog refresh to prevent thread deadlock")
        store = None  # Skip state store feed during catalog refresh
        
        # CRITICAL FIX (2026-08-02): Skip entire feed loop to prevent deadlock
        # The catalog refresh thread cannot safely call apply_rest_market due to threading issues
        # State store will be populated by other mechanisms (WS bridge, REST sync)
        logger.info(f"[CATALOG-FEED] Skipping feed loop for {len(enriched)} markets - state store will be populated by WS bridge")
        feed_count = 0
