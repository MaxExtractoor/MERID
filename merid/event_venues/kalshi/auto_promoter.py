...
class AutoPromoter:
    ...
    # Minimum seconds since last rollback before re-promotion is allowed
    _ROLLBACK_COOLDOWN_S: float = 4 * 3600.0  # 4 hours
    ...
    async def _evaluate_all(self) -> None:
        ...
        for agent_name, metrics in agent_metrics.items():
            ...
            if mode == AgentMode.PAPER:
                ...
                if self._in_rollback_cooldown(agent_name):
                    logger.debug(
                        "[auto-promoter] %s: skipping promotion — in rollback cooldown",
                        agent_name,
                    )
                    continue
                ...
                if not metrics:
                    reason = "metrics_unavailable_for_shadow_agent"
                    logger.warning(
                        "[auto-promoter] %s: SHADOW → PAPER (metrics unavailable — fail-safe rollback)",
                        agent_name,
                    )
                    ctrl.rollback(agent_name, reason)
                    self._record_rollback_ts(agent_name)
                    self._fire_transition(agent_name, "SHADOW", "PAPER", reason)
                    continue
                ...
                if rollback_reason:
                    ...
                    if self._in_rollback_cooldown(agent_name):
                        logger.debug(
                            "[auto-promoter] %s: skipping SHADOW→LIVE — in rollback cooldown",
                            agent_name,
                        )
                        continue
                ...
                if not metrics:
                    reason = "metrics_unavailable_for_live_agent"
                    logger.warning(
                        "[auto-promoter] %s: LIVE → PAPER (metrics unavailable — fail-safe rollback)",
                        agent_name,
                    )
                    ctrl.rollback(agent_name, reason)
                    self._record_rollback_ts(agent_name)
                    self._fire_transition(agent_name, "LIVE", "PAPER", reason)
                    continue
    ...
    def _evaluate_shadow_to_live(
        ...
        gauntlet_ok, gauntlet_reason = self._check_gauntlet_verdict(agent_name)
        gates.append(GateResult(
            passed=gauntlet_ok,
            gate="gauntlet_verdict",
            actual=1.0 if gauntlet_ok else 0.0,
            required=1.0,
            detail=gauntlet_reason,
        ))

        # BUG-2 fix: require PromotionEngine can_go_live check before LIVE promotion
        live_ok, live_reason = self._check_promotion_engine_live(agent_name)
        gates.append(GateResult(
            passed=live_ok,
            gate="promotion_engine_live",
            actual=1.0 if live_ok else 0.0,
            required=1.0,
            detail=live_reason,
        ))
        ...
        if eval_.all_passed:
            readiness = {"ready": True, "checks": {g.gate: g.passed for g in gates}, "details": {g.gate: g.actual for g in gates}}
    ...
    def _check_gauntlet_verdict(self, agent_name: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if agent is None:
                return False, f"agent_{agent_name}_not_found_in_grid"
            config_hash = _compute_agent_config_hash(agent)
            valid, reason = has_valid_gauntlet_pass(agent_name, config_hash)
            return valid, reason
        except Exception as exc:
            logger.debug("[auto-promoter] gauntlet verdict check failed for %s: %s", agent_name, exc)
            return False, f"gauntlet_check_error: {exc}"

    def _check_promotion_engine_live(self, agent_name: str) -> tuple[bool, str]:
        """BUG-2 fix: check PromotionEngine.can_go_live() before LIVE promotion."""
        try:
            from merid.risk.promotion_engine import get_promotion_engine
            engine = get_promotion_engine(agent_name)
            can_live = engine.can_go_live() if hasattr(engine, "can_go_live") else True
            if not can_live:
                phase = engine.current_phase.name if hasattr(engine, "current_phase") else "unknown"
                return False, f"promotion_engine_phase_{phase}_does_not_allow_live"
            return True, "promotion_engine_ok"
        except Exception as exc:
            logger.debug("[auto-promoter] promotion engine check failed for %s: %s", agent_name, exc)
            # Fail closed: if we cannot verify, do not promote to LIVE
            return False, f"promotion_engine_unavailable: {exc}"

    def _in_rollback_cooldown(self, agent_name: str) -> bool:
        """BUG-m5: return True if the agent had a rollback within the cooldown window."""
        last_rb = self._last_rollback_ts.get(agent_name)
        if last_rb is None:
            return False
        return (time.time() - last_rb) < self._ROLLBACK_COOLDOWN_S

    def _record_rollback_ts(self, agent_name: str) -> None:
        """Record the timestamp of a rollback for cooldown tracking."""
        self._last_rollback_ts[agent_name] = time.time()
