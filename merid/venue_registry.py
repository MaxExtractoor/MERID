...
def _initialize_default_venues() -> None:
    ...
    if _is_validation:
        logger.info("[VALIDATION MODE] Venue registry default initialization skipped (prevents lag)")
        return
    ...
    try:
        ...
        # mode=None → singleton uses MERID_PM_LIVE_ENABLED (see get_kalshi_venue_adapter)
        kalshi = get_kalshi_venue_adapter()
