...
class MessageNormalizer:
    ...
    def _extract_assets(self, content: str) -> List[str]:
        ...
        try:
            ...
            common_tickers = [
                a.symbol.split("/")[0] for a in get_swarm_eligible()
                if "/" in a.symbol and ":" not in a.symbol
            ]
        except Exception:
            common_tickers = ["BTC", "ETH", "SOL", "AVAX", "ARB", "OP"]
...
_social_sentiment_pipeline_lock = threading.Lock()
...
def get_social_sentiment_pipeline() -> SocialSentimentPipeline:
    ...
    if _social_sentiment_pipeline is None:
        with _social_sentiment_pipeline_lock:
            if _social_sentiment_pipeline is None:
