...
_brier_db_singleton_lock = threading.Lock()

def get_brier_db() -> BrierMetricsDB:
    """Get the singleton BrierMetricsDB instance."""
    global _brier_db_singleton
    if _brier_db_singleton is None:
        with _brier_db_singleton_lock:
            if _brier_db_singleton is None:
                _brier_db_singleton = BrierMetricsDB()
    return _brier_db_singleton
