"""MERID Gate.io Adapter - Optional/Data Venue.
...
class GateIOAdapter(VenueAdapter):
    ...
    def __init__(
        ...
        super().__init__("gateio", paper=True)  # Default to paper for safety
        self.api_key = api_key or os.getenv("GATE_KEY")
        self.api_secret = api_secret or os.getenv("GATE_SECRET")
        self.client = None
        self.endpoint = "https://api.gateio.ws"
        self.logger = logger.bind(adapter="GateIO")
    ...
    async def connect(self) -> bool:
        ...
        try:
            if self.us_compliance_warning:
                self.logger.warning(
                    "gateio_us_compliance_warning",
                    message="Gate.io may not be available to US residents. Use for data/sim only unless legally cleared."
                )
            ...
            self.connected = True
            self.logger.info("gateio_connected")
            return True
        except Exception as e:
            self.logger.error("gateio_connect_error", error=str(e))
            return False
    ...
    async def disconnect(self) -> bool:
        ...
        self.client = None
        self.logger.info("gateio_disconnected")
        return True
    ...
    async def get_market_data(self, symbol: str) -> Optional[MarketData]:
        ...
        try:
            return MarketData(
                symbol=symbol,
                bid=Decimal("50015.00"),
                ask=Decimal("50025.00"),
                last=Decimal("50020.00"),
                volume_24h=Decimal("130000"),
    ...
    async def _place_order_impl(
        ...
        if self.us_compliance_warning and not self.paper:
            self.logger.error("gateio_live_trading_blocked_us")
            return {
                "status": "blocked",
                "reason": "Gate.io live trading blocked for US residents. Use paper mode or MERID_SIM."
            }
