                    ...
                    for range_str in ranges.split(','):
                        ...
                        if '-' in range_str:
                            ...
                            try:
                                ...
                                logger.debug("Failed to parse line number: %s", range_str)
...
class AutonomousCoverageFixer:
    ...
    HITL gate: ``require_approval=True`` (default) blocks file writes and
    subprocess test runs until an operator explicitly calls
    ``approve_pending()`` or constructs the fixer with
    ``require_approval=False`` (e.g. in CI where a human triggered the job).
    """

    def __init__(self, llm_client=None, *, require_approval: bool = True):
        self.analyzer = CoverageAnalyzer()
        self.llm_client = llm_client
        self.require_approval = require_approval
        self._approved = not require_approval
        self.logger = logger.bind(component="AutonomousCoverageFixer")

    def approve_pending(self) -> None:
        """Operator explicitly approves autonomous actions."""
        self._approved = True
        self.logger.info("hitl_approval_granted")

    def _assert_approved(self, action: str) -> None:
        if not self._approved:
            raise PermissionError(
                f"HITL gate: autonomous action '{action}' blocked — "
                "call approve_pending() or set require_approval=False"
            )
    ...
    def _append_test_to_file(self, test_file_path: str, test_code: str):
        ...
        self._assert_approved("write_test_file")
    ...
    def _run_tests(self, test_file_path: str) -> bool:
        ...
        self._assert_approved("run_tests")
        try:
            result = subprocess.run(
