...
from agents.trading_registry import load_trading_agents
...
def _vote_positive(vote) -> bool:
    """Check if a vote is positive — handles int, str, bool, None."""
    if vote is None:
        return False
...
class MeridCore:
    ...
    async def run_cycle(self, energy: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if vote_result["approved"]:
            ...
            reality_gap = (validation_result or {}).get("reality_gap")
            validated = (validation_result or {}).get("validated", True)
    ...
    async def _handle_approved_energy(
        ...
        await self.state.record_top_pick(
            ...
            curator=[summary["agent"] for summary in vote_result.get("summaries", []) if _vote_positive(summary.get("vote"))],
        )
...
_CORE_INSTANCE_lock = threading.Lock()


def get_core() -> MeridCore:
    global _CORE_INSTANCE
    if _CORE_INSTANCE is None:
        with _CORE_INSTANCE_lock:
            if _CORE_INSTANCE is None:
