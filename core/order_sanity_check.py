...
class OrderSanityChecker:
    ...
    def check(
        ...
        side: str = "buy",
        min_order_notional_usd: Optional[float] = None,
    ) -> SanityCheckResult:
        ...
        with self._lock:
            ...
            min_notional_floor = (
                float(min_order_notional_usd)
                if min_order_notional_usd is not None
                else float(self.config.min_order_notional_usd)
            )
            ...
            # 3. Min order notional (optional per-intent override, e.g. crypto matrix)
            if notional < min_notional_floor:
                violations.append({
                    "check": "min_order_notional_usd",
                    "limit": min_notional_floor,
                    "actual": round(notional, 4),
                    "message": (
                        f"Order notional ${notional:.4f} below "
                        f"minimum ${min_notional_floor:.2f}"
                    ),
                })
            ...
            if passed:
                # Update daily counters only on pass — atomic with the checks above
