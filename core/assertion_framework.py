    ...
    def _send_to_registry(self, action: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if action == "register":
                ...
                response = requests.put(f"{self.registry_url}/update", json=payload)
            elif action == "record":
                response = requests.post(f"{self.registry_url}/record", json=payload)
            elif action == "list":
                response = requests.get(f"{self.registry_url}/list", params=payload)
            elif action == "get_template":
                response = requests.get(f"{self.registry_url}/templates/{payload['template_id']}")
            else:
