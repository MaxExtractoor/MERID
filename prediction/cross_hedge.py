...
class CrossHedgeEngine:
    ...
    def _get_hedge_symbol(self, market_id: str) -> str:
        """Get hedge symbol from market ID using asset universe keyword map."""
        market_lower = market_id.lower()
        try:
            from monitoring.intelligence_layer import IntelligenceProcessor
            for keyword, tickers in IntelligenceProcessor.SYMBOL_KEYWORDS.items():
                if keyword in market_lower and tickers:
                    base = tickers[0]
                    from data.asset_universe import ASSET_UNIVERSE
                    asset = ASSET_UNIVERSE.get(base)
                    if asset and "/" in asset.symbol and ":" not in asset.symbol:
                        return asset.symbol
        except Exception:
            logger.debug("silent catch in cross_hedge:385")
...
_cross_hedge_engine_lock = threading.Lock()
