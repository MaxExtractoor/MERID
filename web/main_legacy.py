...
# Original Proactor issue with uvicorn may need to be addressed differently if it recurs.
...
async def _app_lifespan(application: FastAPI):
    ...
    # ── Phase 0.505: AgentGrid cycle runner for kalshi_crypto_15m_v2 ────────
    async def _agentgrid_cycle_runner():
        """Background task to run AgentGrid cycles for kalshi_crypto_15m_v2 profile."""
        import os
        profile = os.getenv("MERID_PROFILE", "").lower()
        if profile != "kalshi_crypto_15m_v2":
            logger.info("[AGENTGRID-CYCLE-RUNNER] Skipping - not kalshi_crypto_15m_v2 profile")
            return
        
        logger.info("[AGENTGRID-CYCLE-RUNNER] Starting cycle runner for kalshi_crypto_15m_v2")
        from merid.prediction.agent_grid import get_agent_grid
        agent_grid = get_agent_grid()
        
        # Wait for grid to be started before running cycles
        if not agent_grid.agents:
            logger.warning("[AGENTGRID-CYCLE-RUNNER] Grid not started yet, waiting 10s before retry")
            await asyncio.sleep(10)
            if not agent_grid.agents:
                logger.error("[AGENTGRID-CYCLE-RUNNER] Grid still not started, aborting cycle runner")
                return
        ...
        while True:
            try:
                tick += 1
                logger.info(f"[AGENTGRID-CYCLE-RUNNER] Running cycle {tick}")
                logger.info(f"[AGENTGRID-CYCLE-RUNNER] BEFORE agent_grid.run_cycle call - agent_grid={type(agent_grid).__name__}")
                # Add timeout to prevent indefinite blocking - 5 minute max per cycle
                try:
                    logger.info(f"[AGENTGRID-CYCLE-RUNNER] BEFORE asyncio.wait_for")
                    coroutine = agent_grid.run_cycle(tick)
                    logger.info(f"[AGENTGRID-CYCLE-RUNNER] AFTER coroutine creation - coroutine={type(coroutine).__name__}")
                    # Try calling coroutine directly without asyncio.wait_for to see if wait_for is the issue
                    result = await coroutine
                    logger.info(f"[AGENTGRID-CYCLE-RUNNER] AFTER coroutine await - result={type(result).__name__}")
                except asyncio.TimeoutError:
                    logger.error(f"[AGENTGRID-CYCLE-RUNNER] Cycle {tick} timed out after 5 minutes")
                # 15m cycle interval - run every 15 minutes
                await asyncio.sleep(15 * 60)
            except asyncio.CancelledError:
                logger.info("[AGENTGRID-CYCLE-RUNNER] Task cancelled, shutting down")
                break
            except Exception as e:
                logger.error(f"[AGENTGRID-CYCLE-RUNNER] Cycle {tick} failed: {e}", exc_info=True)
                await asyncio.sleep(60)  # Wait 1 minute before retry
    
    _cycle_runner_task = asyncio.create_task(_agentgrid_cycle_runner(), name="agentgrid-cycle-runner")
    _startup_state["background_tasks"].append(_cycle_runner_task)
    ...
    if _is_validation:
        ...
        try:
            ...
            async def _run_price_feed_streaming() -> None:
                logger.warning("[CCXT-STREAM] Legacy _run_price_feed_streaming() called")
                max_attempts = 4
                delay_s = 2.0
                try:
                    for attempt in range(1, max_attempts + 1):
                        try:
                            logger.warning("[CCXT-STREAM] Attempt %d/%d calling start_streaming()", attempt, max_attempts)
    ...
    logger.warning("[KALSHI-STARTUP] Attempting to start KalshiVenueClient")
    try:
    ...
    try:
        ...
        _catalog = build_kalshi_catalog(_settings)
        ...
        try:
            ...
            if _universe is None:
                ...
                if _catalog_assets != _expected_assets:
                    # NON-FATAL: Asset mismatch should not block startup - system can operate with partial coverage
                    logger.warning(
                        "[MARKET-UNIVERSE-VALIDATION] Catalog assets mismatch (non-fatal). "
                        "Expected %s, got %s. System will operate with available assets only.",
                        sorted(_expected_assets),
                        sorted(_catalog_assets)
                    )
                    # Do NOT set startup_success=False - allow startup with partial asset coverage
                if _catalog_count == 0:
                    logger.error(
        ...
        # NON-FATAL: Catalog failure should not block startup - system can operate with partial asset coverage
        # AgentGrid will still cycle for available assets, and risk vetoes will prevent trading on missing assets
        logger.warning(f"⚠️  KalshiMarketCatalog failed to start (non-fatal): {e}")
        logger.warning("⚠️  System will continue with partial asset coverage. AgentGrid will cycle for available assets only.")
        _startup_state["services"]["kalshi_market_catalog"] = {"status": "degraded", "error": str(e)}
    ...
    # LEAN 15m KALSHI STACK (2026-05-13): Skip when ENABLE_SENTIMENT_TRUTH=false
    _is_validation = __import__("os").environ.get("MERID_VALIDATION_MODE", "") == "1"
    ...
    if _is_validation:
        ...
        logger.info("[LEAN KALSHI] KalshiSentimentService skipped (ENABLE_SENTIMENT_TRUTH=false)")
        _startup_state["services"]["kalshi_sentiment_service"] = {"status": "skipped", "reason": "sentiment_disabled"}
    else:
        try:
            ...
            if _sentiment_svc is None:
                logger.info("[PROFILE-GUARD] KalshiSentimentService skipped (profile-gated)")
                _startup_state["services"]["kalshi_sentiment_service"] = {"status": "skipped", "reason": "profile_gated"}
            else:
    ...
    # LEAN 15m KALSHI STACK (2026-05-13): Skip when ENABLE_SENTIMENT_TRUTH=false
    _is_validation = os.environ.get("MERID_VALIDATION_MODE", "") == "1"
    _sentiment_disabled = os.environ.get("ENABLE_SENTIMENT_TRUTH", "true").lower() == "false"
    if _is_validation:
        ...
        logger.info("[LEAN KALSHI] MarketMoodBus skipped (ENABLE_SENTIMENT_TRUTH=false)")
        _startup_state["services"]["market_mood_bus"] = {"status": "skipped", "reason": "sentiment_disabled"}
    else:
        try:
            from merid.sentiment.market_mood_bus import get_market_mood_bus
            _mood_bus = get_market_mood_bus()
            if _mood_bus is None:
                logger.info("[PROFILE-GUARD] MarketMoodBus skipped (profile-gated)")
                _startup_state["services"]["market_mood_bus"] = {"status": "skipped", "reason": "profile_gated"}
            else:
    ...
    # [MODE-DUMP] Print all relevant mode flags before MeridLoop gating decision
    logger.info(
        "[MODE-DUMP] MERID_VALIDATION_MODE=%r MERID_PROFILE=%r MERID_PM_PROFILE=%r "
        "MERID_TRADING_MODE=%r KALSHI_ENV=%r KALSHI_USE_DEMO=%r MERID_ALLOW_LIVE_TRADES=%r "
        "MERID_ENABLE_KALSHI_CT=%r MERID_ENABLE_KALSHI_PM=%r startup_success=%s "
        "failed_services=%s",
        os.environ.get("MERID_VALIDATION_MODE", ""),
        os.environ.get("MERID_PROFILE", ""),
        os.environ.get("MERID_PM_PROFILE", ""),
        os.environ.get("MERID_TRADING_MODE", ""),
        os.environ.get("KALSHI_ENV", ""),
        os.environ.get("KALSHI_USE_DEMO", ""),
        os.environ.get("MERID_ALLOW_LIVE_TRADES", ""),
        os.environ.get("MERID_ENABLE_KALSHI_CT", ""),
        os.environ.get("MERID_ENABLE_KALSHI_PM", ""),
        startup_success,
        [
            name for name, info in _startup_state.get("services", {}).items()
            if isinstance(info, dict) and info.get("status") in ("failed", "blocked", "error")
        ],
    )

    if _is_validation:
        logger.warning("[MERID-LOOP SKIPPED] MERID_VALIDATION_MODE=1 (prevents 5s+ startup lag)")
        ...
        logger.error(
            "[MERID-LOOP SKIPPED] startup_success=False — critical startup failures detected. "
            "Failed services: %s. Resolve them before enabling live trading.",
            _failed_services,
        )
        _startup_state["services"]["merid_loop"] = {
            "status": "blocked",
            "reason": f"startup_success=False — failed services: {_failed_services}",
        }
    else:
        # MeridLoop moved to standalone worker process to bypass Windows event-loop starvation
        # Run with: python -m merid.merid_loop_worker
        logger.info("ℹ️  MeridLoop disabled in uvicorn - running as standalone worker process")
        _startup_state["services"]["merid_loop"] = {"status": "disabled", "reason": "moved to standalone worker"}

    # Sleep probe task to test if blocking time.sleep works in executor
    import time as _time
    def sleep_probe_blocking():
        """Blocking version using time.sleep in executor thread."""
        i = 0
        while True:
            _time.sleep(5.0)
            i += 1
            logger.info("[SLEEP-PROBE-BLOCKING] tick=%d, blocking sleep works", i)
    
    async def sleep_probe():
        """Async wrapper that runs blocking sleep in executor."""
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, sleep_probe_blocking)
    
    try:
        probe_task = asyncio.create_task(sleep_probe(), name="SleepProbe")
        _startup_state["background_tasks"].append(probe_task)
        logger.info("✅ SleepProbe started (blocking version in executor)")
    except Exception as e:
        logger.warning(f"⚠️  SleepProbe failed to start: {e}")
...
assert app is not None, "FastAPI app must not be None"
...
if __name__ == "__main__":
    ...
    # Exception handler will be set on uvicorn's loop in _app_lifespan
    # Do NOT create a new loop here - let uvicorn manage the loop
