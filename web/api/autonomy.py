...
router = APIRouter(prefix="/api/v1/swarm/prime-screen/autonomy", tags=["autonomy"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
async def update_autonomy_state(
    payload: AutonomyStateUpdateRequest,
...
async def upsert_autonomy_limits(
    payload: AutonomyPortfolioUpdateRequest,
    _session: dict = Depends(get_current_session),  # ZT4-02
) -> AutonomyPortfolioResponse:
    try:
