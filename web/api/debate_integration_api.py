...
router = APIRouter(prefix="/debates/integration", tags=["debate-integration"])
...
async def get_pnl_attribution_dashboard(days_back: int = 30):
    ...
    try:
        ...
        # get_attribution_dashboard() is sync and calls asyncio.run() internally —
        # must run in a thread to avoid RuntimeError (loop already running).
        dashboard = await asyncio.to_thread(attribution_engine.get_attribution_dashboard, days_back)
        return dashboard
