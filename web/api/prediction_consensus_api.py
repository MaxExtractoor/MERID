...
router = APIRouter(prefix="/api/v1/prediction", tags=["prediction-consensus"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
async def prediction_opinions(
    ...
    if store:
        try:
            ...
            serialized = []
            for o in opinions:
                od = o.to_dict(include_explanation=include_explanation)
                # Ensure agent_name is never empty string
                if not od.get("agent_name"):
                    od["agent_name"] = od.get("agent_id", "unknown")
                serialized.append(od)
            return {
                "opinions": serialized,
...
async def list_debates(
    ...
    try:
        ...
        for d in debates:
            dd = d.to_dict()
            # Ensure team_id is never empty string
            if not dd.get("team_id"):
                dd["team_id"] = dd.get("id", "unknown")[:12]
            serialized.append(dd)
        return {"debates": serialized, "count": len(serialized)}
    except Exception as exc:
...
async def list_teams(limit: int = 50):
    ...
    try:
        ...
        # Fallback: derive virtual teams from agent grid assets when no teams in DB
        if not team_dicts:
            try:
                ...
                for agent in grid.agents:
                    ...
                    asset = agent.config.category if hasattr(agent.config, "category") else name.split("_")[0]
                    if asset not in asset_groups:
                        asset_groups[asset] = []
                    asset_groups[asset].append(name)
                for asset, members in asset_groups.items():
                    team_dicts.append({
                        "id": f"team-{asset.lower()}",
                        "name": f"Team {asset}",
                        "members": members,
                        "created_at": "",
                        "source": "agent_grid",
        ...
        return {"teams": team_dicts, "count": len(team_dicts)}
    except Exception as exc:
...
async def leaderboard(limit: int = 20, decay_lambda: Optional[float] = None):
    ...
    def _compute():
        result = ds.compute_leaderboard(limit=limit, decay_lambda=decay_lambda)
        # If no reward data yet, derive rankings from debate arguments
        if not result.get("agents"):
            try:
                debates = ds.list_debates(limit=100)
                agent_stats: Dict[str, Dict[str, Any]] = {}
                for deb in debates:
                    args = ds.list_arguments(deb.id)
                    for arg in args:
                        aid = arg.agent_id or "unknown"
                        if aid not in agent_stats:
                            agent_stats[aid] = {"arguments": 0, "total_conf": 0.0}
                        agent_stats[aid]["arguments"] += 1
                        agent_stats[aid]["total_conf"] += arg.confidence
                for aid, s in agent_stats.items():
                    s["avg_confidence"] = s["total_conf"] / max(s["arguments"], 1)
                    s["total_points"] = round(s["arguments"] * s["avg_confidence"], 2)
                sorted_agents = sorted(agent_stats.items(), key=lambda x: x[1]["total_points"], reverse=True)[:limit]
                result["agents"] = [
                    {"agent_id": aid, "total_points": s["total_points"],
                     "reward_events": s["arguments"], "source": "debate_participation"}
                    for aid, s in sorted_agents
                ]
            except Exception as e:
                logger.debug(f"Silent error: {e}")
        # If still no agents, populate from agent grid
        if not result.get("agents"):
            try:
                from merid.prediction.agent_grid import get_agent_grid
                grid = get_agent_grid()
                result["agents"] = [
                    {"agent_id": a.config.name, "total_points": float(a.state.cycles_run),
                     "reward_events": a.state.orders_placed, "source": "agent_grid"}
                    for a in grid.agents if a.state.cycles_run > 0
                ]
            except Exception as e:
                logger.debug(f"Silent error: {e}")
        # Populate teams from debate store teams
        if not result.get("teams"):
            try:
                teams = ds.list_teams() if hasattr(ds, "list_teams") else []
                result["teams"] = [
                    {"team_id": getattr(t, "id", str(t)), "name": getattr(t, "name", ""),
                     "members": getattr(t, "members", []), "total_points": getattr(t, "total_points", 0)}
                    for t in teams
                ]
            except Exception as e:
                logger.debug(f"Silent error: {e}")
    ...
    try:
        return await asyncio.wait_for(asyncio.to_thread(_compute), timeout=5.0)
    except asyncio.TimeoutError:
        logger.warning("consensus/leaderboard timed out (5s)")
        # Return agent grid data directly as fast fallback
        try:
            from merid.prediction.agent_grid import get_agent_grid
            grid = get_agent_grid()
            agents = [
            ...
            return {"agents": agents, "teams": [], "debate_impact": []}
        except Exception:
            return {"agents": [], "teams": [], "debate_impact": [], "error": "Leaderboard timed out (5s)"}
    except Exception as exc:
