...
router = APIRouter(prefix="/api/risk", tags=["risk"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
_trading_guard_instance_lock = threading.Lock()
...
def _get_or_create_guard() -> TradingGuard:
    ...
    if _trading_guard_instance is None:
        with _trading_guard_instance_lock:
            if _trading_guard_instance is None:
...
async def get_risk_protections() -> RiskProtectionsResponse:
    ...
    if guard._error_timestamps:
        ...
        last_error = datetime.fromtimestamp(last_ts, tz=timezone.utc).isoformat()
    
    opened_at = None
    if guard._circuit_opened_at:
        opened_at = datetime.fromtimestamp(guard._circuit_opened_at, tz=timezone.utc).isoformat()
...
@router.get("/snapshot")
async def get_risk_snapshot_endpoint(
    include_verdicts: bool = True,
    _session: dict = Depends(get_current_session),
) -> Dict[str, Any]:
    """
    Get comprehensive risk state snapshot.
    
    Aggregates all risk controls: kill switches, caps, CQI, cooldown, promotion.
    This endpoint provides a single source of truth for operator visibility.
    ...
    """
    try:
        from merid.risk_state import get_risk_snapshot, RiskSnapshot
        
        snapshot = get_risk_snapshot(
        ...
        logger.error("Failed to generate risk snapshot: %s", exc)
        raise HTTPException(
            status_code=500,
            detail=f"Risk snapshot generation failed: {str(exc)}"
        )
