...
router = APIRouter(prefix="/api/v1/localvenue", tags=["local-venue"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
async def get_engine_status():
    ...
    try:
        ...
        return EngineStatus(
            ...
            dropped_messages=getattr(simulator, '_dropped_messages', 0),
            ...
            if book:
                ...
                for event in trade_events[-limit:]:  # Get recent trades
                    trade = Trade(
                        ...
                        pnl=float(event.metadata.get('pnl', 0.0)) if event.metadata else 0.0
                    )
