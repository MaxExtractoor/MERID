...
router = APIRouter(prefix="/api/v1/paper", tags=["paper_trading"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
async def close_paper_position_alt(  # renamed to avoid duplicate func name
    user_id: str,
    position_key: str,
    _session: dict = Depends(get_current_session),  # ZT5-01
):
...
async def get_portfolio_trade_history(user_id: str, limit: int = 50):
...
, dependencies=[Depends(get_current_session)]  # ZT6-01
)
