...
LEGACY: Perps trading endpoints moved to _legacy/
"""
...
router = APIRouter(prefix="/api/v1/trading", tags=["trading"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
_trading_agents_lock = threading.Lock()  # FPA-05: protects all 3 trading agent singletons
...
async def execute_arbitrage(
    ...
    _session: dict = Depends(get_current_session),  # ZT4-02: auth required
):
    """
...
async def list_prediction_markets(
    ...
    """List available prediction markets from Kalshi."""
    # LEGACY: Polymarket support moved to _legacy/
    if platform.lower() == "polymarket":
        return {"error": "Polymarket support is legacy-only. Use platform='kalshi'.", "platforms": ["kalshi"]}, 400
    
    try:
        from merid.kalshi import get_market_catalog
        
        catalog = get_market_catalog()
        markets = catalog.list_markets(category=category, limit=limit)
...
async def execute_prediction_market_trade(
    ...
    """Execute prediction market trade via paper trading (Kalshi-only)."""
    # LEGACY: Polymarket support moved to _legacy/
    if platform.lower() == "polymarket":
        return {"error": "Polymarket support is legacy-only. Use platform='kalshi'.", "platforms": ["kalshi"]}, 400
    import uuid
...
async def get_market_positions(platform: str = "kalshi", user_id: str = "default_user"):
    """Get prediction market positions from paper trading (Kalshi-only)."""
    # LEGACY: Polymarket support moved to _legacy/
    if platform.lower() == "polymarket":
        return {"error": "Polymarket support is legacy-only. Use platform='kalshi'.", "platforms": ["kalshi"]}, 400
