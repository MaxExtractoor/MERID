...
# ZT5-01: All wallet routes require authentication.
router = APIRouter(
...
@router.post("/walletconnect/disconnect/{wc_session_id}")
async def disconnect_wallet_connect(wc_session_id: str) -> Dict[str, Any]:
    """Disconnect a WalletConnect session."""
    success = get_wallet_manager().disconnect_wallet_connect(wc_session_id)
    if success:
