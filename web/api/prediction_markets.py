...
router = APIRouter(prefix="/api/v1/prediction-markets", tags=["prediction-markets"], dependencies=[Depends(get_current_session)]  # ZT6-01
)
...
_pm_lock = threading.Lock()  # FPA-05: protects all 4 PM singletons
...
async def pm_acknowledge_alert(alert_id: str):
    """Acknowledge a prediction-market alert by its alert_id."""
    mgr = _get_alerts()
    found = mgr.acknowledge(alert_id)
    if not found:
