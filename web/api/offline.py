...
@router.post("/replay/{session_id}/load")
async def load_replay_session(session_id: str) -> Dict[str, Any]:
    """Load events for a replay session."""
    success = await get_replay_engine().load_session(session_id)
    if success:
        return {"status": "loaded"}
    raise HTTPException(status_code=400, detail="Load failed")


@router.post("/replay/{session_id}/start")
async def start_replay(session_id: str) -> Dict[str, Any]:
    """Start replay."""
    success = await get_replay_engine().start(session_id)
    if success:
...
async def pause_replay(session_id: str) -> Dict[str, Any]:
    """Pause replay."""
    success = get_replay_engine().pause(session_id)
    if success:
...
async def resume_replay(session_id: str) -> Dict[str, Any]:
    """Resume replay."""
    success = get_replay_engine().resume(session_id)
    if success:
...
async def stop_replay(session_id: str) -> Dict[str, Any]:
    """Stop replay."""
    success = await get_replay_engine().stop(session_id)
    if success:
        return {"status": "stopped"}
    raise HTTPException(status_code=404, detail="Session not found")
...
async def seek_replay(session_id: str, timestamp: float) -> Dict[str, Any]:
    """Seek to timestamp."""
    success = get_replay_engine().seek(session_id, timestamp)
    if success:
...
async def set_replay_speed(session_id: str, speed: float) -> Dict[str, Any]:
    """Set replay speed."""
    ...
    success = get_replay_engine().set_speed(session_id, replay_speed)
    if success:
