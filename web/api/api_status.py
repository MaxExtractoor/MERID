...
def _coinbase_credentials_status() -> Dict[str, Any]:
    """Single row: any of MERID_/COINBASE_CLIENT_/COINBASE_ key + secret."""
    k = coinbase_api_key()
    s = coinbase_api_secret()
    is_configured = bool(
        k and s and k not in ("", "change_me") and s not in ("", "change_me")
    )
    key_preview = k[:8] + "..." if k and len(k) > 8 else (k if k else None)
    return {
        "name": "COINBASE_API_KEY (+ secret)",
        "configured": is_configured,
        "required": True,
        "status": "active" if is_configured else "missing",
        "value_preview": key_preview,
    }
