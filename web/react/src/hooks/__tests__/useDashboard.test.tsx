...
describe('useSystemHealth', () => {
  ...
  it('should handle fetch error', async () => {
    // retryOnce calls twice — both must reject for error to propagate
    mockApi.getSystemHealth.mockRejectedValue(new Error('API Error'));

    const { result } = renderHook(() => useSystemHealth());

    // Advance past retryOnce's 500ms delay
  ...
  it('should handle non-Error rejection', async () => {
    mockApi.getSystemHealth.mockRejectedValue('String error');
...
describe('usePnLSummary', () => {
  ...
  it('should fetch P&L data on mount', async () => {
    ...
    // usePnLSummary delays initial fetch by 2000ms
...
describe('useAgentsSummary', () => {
  ...
  it('should fetch agents data on mount', async () => {
    ...
    // useAgentsSummary delays initial fetch by 4000ms
...
describe('useTradingSummary', () => {
  ...
  it('should fetch trading data on mount', async () => {
    ...
    // useTradingSummary delays initial fetch by 6000ms + retryOnce 500ms
  ...
  it('should handle fetch error gracefully', async () => {
    // retryOnce calls twice — both must reject
    mockApi.getTradingSummary.mockRejectedValue(new Error('API Error'));

    const { result } = renderHook(() => useTradingSummary());

    // 6000ms delay + 500ms retry
