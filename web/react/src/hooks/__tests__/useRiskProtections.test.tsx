...
// Mock constants so we get predictable URLs in test
...
describe('useRiskProtections', () => {
  ...
  it('should reset circuit breaker', async () => {
    ...
    mockFetch.mockImplementation((url: string) => {
      return Promise.resolve({
        ok: true,
        json: async () => url.includes('circuit-breaker') ? {} : mockData,
      });
    });
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/v1/operator/reset-kill-switch', expect.any(Object));
  });
  ...
  it('should toggle kill switch enable', async () => {
    ...
    mockFetch.mockImplementation((url: string) => {
      return Promise.resolve({
        ok: true,
        json: async () => url.includes('kill-switch') ? {} : mockData,
      });
    });
    ...
    expect(success).toBe(true);
    expect(mockFetch).toHaveBeenCalledWith('/api/v1/operator/guard/kill', expect.any(Object));
  });
  ...
  it('should toggle kill switch disable', async () => {
    ...
    expect(success).toBe(true);
    expect(mockFetch).toHaveBeenCalledWith('/api/v1/operator/guard/unkill', expect.any(Object));
  });
