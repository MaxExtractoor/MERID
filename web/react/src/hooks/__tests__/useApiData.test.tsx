...
describe('useApiData', () => {
  ...
  it('appends query params to the fetch URL when query option is provided', async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ ok: true }),
    });

    render(<TestComponent endpoint="/api/test" options={{ query: { foo: 'bar', baz: 'qux' } }} />);
  ...
  it('omits undefined query param values', async () => {
    ...
    render(<TestComponent endpoint="/api/test" options={{ query: { present: 'yes', absent: undefined } }} />);
