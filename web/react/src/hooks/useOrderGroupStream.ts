 ...
 *     autoConnect: true,
 *     onTriggered: (groupId, data) => console.debug('Triggered:', groupId),
 *   });
 */
...
export function useOrderGroupStream(
  ...
  // Ref so the error handler always reads the current attempt count without stale closure
  ...
  const connect = useCallback(() => {
    ...
    es.addEventListener('error', (_event) => {
      ...
      // Attempt reconnection with exponential backoff (use ref to avoid stale closure)
  ...
  }, [buildUrl, onConnect, onError, onTriggered, clearAlertsForGroup, maxReconnectAttempts, reconnectDelayMs]);
