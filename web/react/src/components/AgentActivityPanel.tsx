    ...
    <div className="bg-slate-900/70 rounded-xl p-6 border border-slate-800">
      ...
      <div className="space-y-3">
        {agents.map((agent) => {
          ...
          const isError = agent.status === AGENT_STATUS.ERROR;
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-500">
                    ...
                    {agent.win_rate != null && agent.win_rate > 0 && (
                      <span className="ml-2 text-emerald-500">{(agent.win_rate ?? 0).toFixed(1)}% win</span>
                    )}
                  </div>
                </div>
...
export default React.memo(AgentActivityPanel);
