...
const operatorSection = [
  ...
  { name: 'Risk Alerts', href: 'kalshi-risk', icon: AlertTriangle, color: 'text-red-400' },
  { name: 'Kill Switch', href: 'kill-switch', icon: ShieldAlert, color: 'text-red-500' },
];
...
function NavItem({ item, current, onChange, collapsed, isLive }: { item: typeof tradingCore[0]; current: View; onChange: (v: View) => void; collapsed: boolean; isLive?: boolean }) {
  ...
  const showModeDot = [
    'kalshi-dashboard', 'kalshi-grid', 'kalshi-terminal',
    'kalshi-all-markets', 'kalshi-portfolio', 'positions', 'orders',
  ].includes(item.href);
  return (
    <button type="button"
