  ...
  return (
    <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        {[
          { label: 'Kalman', value: combined_sm, sub: kalman_gain != null ? `gain=${kalman_gain.toFixed(2)}` : 'primary' },
      ...
      <div className="flex items-center justify-between text-xs border-t border-slate-800 pt-2">
        <span className="text-slate-500">
          Conf: <span className="text-slate-300 font-mono">{(confidence * 100).toFixed(0)}%</span>
        </span>
        <span className="text-slate-500">
        ...
        {kalshi_adj !== 0 && (
          <span className="text-blue-400 font-mono">
            adj {kalshi_adj > 0 ? '+' : ''}{(kalshi_adj * 100).toFixed(2)}%
          </span>
        )}
      </div>
        ...
        <div className="bg-slate-800/50 rounded-lg p-2.5 space-y-1.5 text-xs">
          <div className="flex items-center justify-between">
            ...
            <span className={`font-mono font-bold ${clamp.sizing_multiplier < 0.8 ? 'text-amber-400' : 'text-green-400'}`}>
              {(clamp.sizing_multiplier * 100).toFixed(0)}% size
            </span>
          </div>
