  ...
  const fetchData = useCallback(async () => {
    try {
      const [matrixRes, clusterRes] = await Promise.all([
        fetch(`${API_BASE_URL}${API_ENDPOINTS.CORRELATION_MATRIX}`),
        fetch(`${API_BASE_URL}${API_ENDPOINTS.CORRELATION_CLUSTERS}`),
      ]);
      if (matrixRes.ok) {
                ...
                return (
                  ...
                  >
                    <span className="text-gray-400">
                      {p.asset_a}/{p.asset_b}: ρ={p.correlation.toFixed(3)}
                    </span>
                    <span className={`rounded px-1.5 py-0.5 ${badge.className}`}>
                      Cap ×{p.reduction_factor.toFixed(2)} — {badge.label}
                    </span>
                  </div>
                );
              })}
