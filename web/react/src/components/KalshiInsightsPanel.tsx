...
const KalshiInsightsPanel: React.FC<InsightsPanelProps> = ({ onNavigate }) => {
  const { data: swarmData } = useApiData<{ insights: Insight[] }>(
    API_ENDPOINTS.KALSHI_INSIGHTS,
    { pollingInterval: DEFAULTS.POLLING_INTERVALS.SLOW },
  );
  const { data: riskData } = useApiData<{ insights: Insight[] }>(
    API_ENDPOINTS.KALSHI_RISK_INSIGHTS,
    { pollingInterval: DEFAULTS.POLLING_INTERVALS.SLOW },
  );
  ...
  const rawInsights = useMemo(() => {
    ...
    for (const i of [...(riskData?.insights ?? []), ...(swarmData?.insights ?? [])]) {
