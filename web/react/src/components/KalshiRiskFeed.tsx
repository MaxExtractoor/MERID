...
const KalshiRiskFeed: React.FC<RiskFeedProps> = ({ maxItems = 50, onNavigate, onOpenMarket }) => {
  ...
  const handleDownsize = useCallback(async (asset?: string) => {
    ...
    try {
      ...
      setTimeout(() => setActionStatus(prev => { const n = { ...prev }; delete n[key]; return n; }), 3000);
    } catch {
  ...
  const handlePauseAgents = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => setActionStatus(prev => { const n = { ...prev }; delete n['pause-agents']; return n; }), 3000);
    } catch {
  ...
  const handleResetKillSwitch = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => setActionStatus(prev => { const n = { ...prev }; delete n['reset-ks']; return n; }), 3000);
    } catch {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <>
                          ...
                          {event.severity === 'critical' && (
                            ...
                            >
                              ...
                              {actionStatus['pause-agents'] === 'done' ? 'Paused ✓' : actionStatus['pause-agents'] === 'pending' ? 'Pausing...' : 'Pause venue'}
                            </button>
                          )}
                        </>
                      )}
                      ...
                      {event.category === 'rate_limit' && (
                        ...
                        >
                          <PauseCircle className="w-2.5 h-2.5" />
                          {actionStatus['pause-agents'] === 'done' ? 'Paused ✓' : actionStatus['pause-agents'] === 'pending' ? 'Pausing...' : 'Pause agents'}
                        </button>
                      )}
                      ...
                      {event.category === 'circuit_breaker' && (
                        ...
                        >
                          ...
                          {actionStatus['reset-ks'] === 'done' ? 'Reset ✓' : actionStatus['reset-ks'] === 'pending' ? 'Resetting...' : 'Reset kill switch'}
                        </button>
                      )}
                      ...
                      {event.category === 'liquidity' && (
                        <>
                          ...
                          >
                            ...
                            {actionStatus['downsize-all'] === 'done' ? 'Reduced ✓' : 'Reduce size'}
                          </button>
