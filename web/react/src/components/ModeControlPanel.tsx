...
export default function ModeControlPanel() {
  ...
  const { data: rawData, loading, error: fetchError, refetch } = useApiData<{ venues: ModePanelVenueMode[] }>(
  ...
  const doChangeMode = async (venue: string, newMode: string) => {
    ...
    // T-049: Block mode switch if orders are actively submitting
    try {
      const gateRes = await fetch(`${API_BASE_URL}${API_ENDPOINTS.SYSTEM_EXECUTION_GATE}`, {
        headers: authHeaders(),
      });
      if (gateRes.ok) {
        const gate = await gateRes.json();
        if (gate?.active_submissions > 0) {
          setActionError('Cannot switch mode while orders are being submitted. Wait for completion.');
          setActionLoading(false);
          return;
        }
      }
    } catch { /* gate check optional — proceed if unavailable */ }
