    ...
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm font-bold ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
            {isPositive ? '+' : ''}${(latestPnl ?? 0).toFixed(2)}
          </span>
        </div>
        <div className="flex items-center gap-2">
          {/* Chart mode toggle */}
      ...
      {stats && (
        <div className="flex items-center gap-4 px-4 py-1.5 border-b border-slate-800/50 text-[10px]">
          <div><span className="text-gray-500">Peak</span> <span className="text-green-400 font-mono">${(stats.peak ?? 0).toFixed(2)}</span></div>
          <div><span className="text-gray-500">Max DD</span> <span className="text-red-400 font-mono">${(stats.maxDD ?? 0).toFixed(2)}</span></div>
          <div><span className="text-gray-500">Avg Daily</span> <span className={`font-mono ${stats.avgDaily >= 0 ? 'text-green-400' : 'text-red-400'}`}>${(stats.avgDaily ?? 0).toFixed(2)}</span></div>
        ...
        {chartData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke={CHART_COLORS.BAR_BASE} />
              <XAxis
                dataKey="time"
                tick={{ fill: CHART_COLORS.AXIS_TICK, fontSize: 10 }}
                axisLine={{ stroke: CHART_COLORS.GRID_STROKE }}
                tickLine={false}
              />
              <YAxis
                yAxisId="left"
              ...
              <Tooltip
                contentStyle={{
                  ...
                  border: `1px solid ${CHART_COLORS.GRID_STROKE}`,
                  borderRadius: '8px',
                  fontSize: '11px',
                }}
                labelStyle={{ color: CHART_COLORS.SLATE_400 }}
              ...
              <Legend
                ...
                iconSize={8}
              />
              <ReferenceLine yAxisId="left" y={0} stroke={CHART_COLORS.SLATE_600} strokeDasharray="3 3" />
              ...
              {(() => {
                ...
                return (
                  <>
                    <ReferenceArea yAxisId="left" y1={dsLine} y2={warnLine} fill={CHART_COLORS.AMBER} fillOpacity={0.04} />
                    <ReferenceArea yAxisId="left" y1={Number.MIN_SAFE_INTEGER} y2={dsLine} fill={CHART_COLORS.RED} fillOpacity={0.04} />
                    <ReferenceLine yAxisId="left" y={warnLine} stroke={CHART_COLORS.AMBER} strokeDasharray="4 4" strokeOpacity={0.3}
                      label={{ value: 'Warn 5%', position: 'right', fill: CHART_COLORS.AMBER, fontSize: 8 }} />
                    <ReferenceLine yAxisId="left" y={dsLine} stroke={CHART_COLORS.RED} strokeDasharray="4 4" strokeOpacity={0.3}
                      label={{ value: 'Halt 10%', position: 'right', fill: CHART_COLORS.RED, fontSize: 8 }} />
                  </>
                );
              })()}
              ...
              {breachTimes.map((b, i) => (
                <ReferenceLine
                  ...
                  strokeDasharray="2 2"
                  label={{ value: b.label, position: 'top', fill: CHART_COLORS.RED, fontSize: 9 }}
                />
              ))}
              {/* Risk alert pins from WS */}
              {(riskAlerts ?? []).map((alert, i) => {
                const t = new Date(alert.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                const color = alert.severity === 'critical' ? CHART_COLORS.RED : CHART_COLORS.AMBER;
                return (
                  <ReferenceLine
                    key={`alert-${i}`}
                    yAxisId="left"
