...
function KalshiActivityLog({ ticker, maxItems = 12 }: KalshiActivityLogProps) {
  const { data: gridData } = useApiData<{ agents: ActivityGridAgent[] }>(
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-3">
      ...
      <div className="space-y-1 max-h-48 overflow-y-auto">
        {visible.map((item, i) => (
          <div key={`${item.ts}-${item.agent}-${item.type}-${i}`} className="flex items-center gap-2 text-[10px] py-1 px-1 rounded hover:bg-slate-700/50">
            {item.type === 'signal' ? (
