...
function KalshiRecentOrders({ orders, fills, isPortfolioEmpty }: { orders: KalshiOrder[]; fills: KalshiFill[]; isPortfolioEmpty?: boolean }) {
      ...
      <div className="space-y-1 max-h-64 overflow-y-auto">
        {recentFills.map(f => {
          ...
          return (
            <div key={f.trade_id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-slate-800/50">
              ...
              <div className="text-xs text-slate-500">{fmtTimestamp(f.timestamp, { timeOnly: true })}</div>
            </div>
          );
        })}
        {recentFills.length === 0 && recent.length === 0 && isPortfolioEmpty && (
          <div className="text-center py-6">
            <Activity className="w-5 h-5 mx-auto mb-1 opacity-50 text-slate-500" />
            <div className="text-slate-500 text-sm mb-2">No fills discovered yet</div>
            <div className="text-xs text-slate-600">Trading not yet active — discovery OK</div>
          </div>
        )}
        {recentFills.length === 0 && recent.length === 0 && !isPortfolioEmpty && (
        ...
        {recentFills.length === 0 && recent.length > 0 && recent.map(o => (
          <div key={o.order_id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-slate-800/50">
            ...
            <div className="text-xs text-slate-500">{fmtTimestamp(o.created_at, { timeOnly: true })}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    const srv: 'paper' | 'live' = venueModeData.mode.toLowerCase().includes('paper') ? 'paper' : 'live';
    setGridStartMode(prev => prev !== srv ? srv : prev);
  }, [venueModeData?.mode]);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* WS kill-switch propagation banner — shows for 15000ms after a kill_switch alert */}
      {wsKillActive && wsKillAt && (Date.now() - wsKillAt < 15000) && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-300 text-xs animate-pulse">
          <ShieldAlert className="w-4 h-4 shrink-0" />
          <span>Propagating kill-switch signal… allow up to {15000 / 1000}s for all agents to halt.</span>
        </div>
      )}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <AgentActivityPanel />
        <KalshiRecentOrders orders={ordData?.orders ?? []} fills={fillData?.fills ?? []} isPortfolioEmpty={isPortfolioEmpty} />
      </section>
