...
const TABS: Array<{ id: TabId; label: string; icon: any }> = [
  { id: "ticket", label: "Order Ticket", icon: Send },
...
export default function Trading() {
  ...
  const handleOrderSubmit = useCallback(async (side?: "BUY" | "SELL") => {
    ...
    try {
      ...
      if (!response.ok || result.success === false) {
        throw new Error(result.error || result.detail || "Order submission failed");
      }
      ...
      // Refresh data
      setTimeout(() => { refetchOrders(); refetchPositions(); refetchFills(); }, 500);
    } catch (error: any) {
  ...
  const positionColumns = useMemo(() => [
    ...
    {
      key: "side" as keyof Position, label: "Side",
      render: (value: string) => (
    ...
    { key: "size" as keyof Position, label: "Size", sortable: true, render: (v: number) => formatNumber(v, 4) },
    { key: "entryPrice" as keyof Position, label: "Entry", render: (v: number) => formatCurrency(v) },
    { key: "currentPrice" as keyof Position, label: "Current", render: (v: number) => formatCurrency(v) },
    {
      key: "pnl" as keyof Position, label: "P&L", sortable: true,
    ...
    { key: "venue" as keyof Position, label: "Venue" },
  ], []);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/70 rounded-xl border border-slate-800 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div>
                  <label htmlFor="order-type" className="block text-xs font-medium text-slate-500 mb-1">Order Type</label>
                  <select
                    id="order-type"
                    name="orderType"
                    title="Order Type"
