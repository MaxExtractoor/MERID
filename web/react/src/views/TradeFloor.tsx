...
export default function TradeFloor() {
  ...
  const [tradeWsStatus, setTradeWsStatus] = useState<'connecting' | 'connected' | 'disconnected' | 'unavailable'>('disconnected');
  const [riskWsStatus, setRiskWsStatus] = useState<'connecting' | 'connected' | 'disconnected' | 'unavailable'>('disconnected');
  ...
  useEffect(() => {
    const connectTradeWs = () => {
      ...
      ws.onopen = () => {
        console.log('[TradeFloor] Connected to trade events');
        setWsConnected(true);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.event_type === 'feature_unavailable') {
            console.log('[TradeFloor] Trade stream unavailable');
            setTradeWsStatus('unavailable');
            ws.close(1000);
            return;
          }
          ...
          console.error('[TradeFloor] Parse error:', err);
        }
      };
      ...
      ws.onclose = () => {
        ...
        if (tradeRetryCount.current < MAX_RETRIES) {
          ...
          if (tradeRetryCount.current <= 3) {
            console.log(`[TradeFloor] Reconnecting trades in ${delay}ms (${tradeRetryCount.current}/${MAX_RETRIES})`);
          }
          tradeReconnectTimeout.current = setTimeout(connectTradeWs, delay);
        } else {
          console.log('[TradeFloor] Trade stream max retries exceeded');
          setTradeWsStatus('unavailable');
        }
      };

      ws.onerror = () => {
        if (!hasLoggedTradeError.current) {
          hasLoggedTradeError.current = true;
          console.warn('[TradeFloor] Trade WebSocket connection failed');
        }
      };
    ...
    const connectRiskWs = () => {
      ...
      ws.onopen = () => {
        console.log('[TradeFloor] Connected to risk stream');
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'feature_unavailable' || data.event_type === 'feature_unavailable') {
            console.log('[TradeFloor] Risk stream unavailable');
            setRiskWsStatus('unavailable');
            ws.close(1000);
            return;
          }
          ...
          console.error('[TradeFloor] Risk parse error:', err);
        }
      };
      ...
      ws.onclose = () => {
        ...
        if (riskRetryCount.current < MAX_RETRIES) {
          ...
          if (riskRetryCount.current <= 3) {
            console.log(`[TradeFloor] Reconnecting risk in ${delay}ms (${riskRetryCount.current}/${MAX_RETRIES})`);
          }
          riskReconnectTimeout.current = setTimeout(connectRiskWs, delay);
        } else {
          console.log('[TradeFloor] Risk stream max retries exceeded');
          setRiskWsStatus('unavailable');
        }
      };

      ws.onerror = () => {
        if (!hasLoggedRiskError.current) {
          hasLoggedRiskError.current = true;
          console.warn('[TradeFloor] Risk WebSocket connection failed');
        }
      };
    ...
    const fetchRestFallback = async () => {
      ...
      } catch { /* WS is primary, REST is fallback */ }
    };
    fetchRestFallback();
  ...
  const getEventColor = (event: TradeEvent) => {
    switch (event.event_type) {
      ...
      case 'pnl_update': return event.realized_pnl! >= 0 ? 'text-green-400' : 'text-red-400';
