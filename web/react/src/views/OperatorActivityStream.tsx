...
export function OperatorActivityStream() {
  ...
  const fetchTab = useCallback(async (t: Tab) => {
    ...
    try {
      ...
      const fetchOperator = (endpoint: string) => fetch(endpoint, { headers });

      if (t === 'orders') {
        const res = await fetchOperator(`${API_BASE_URL}${API_ENDPOINTS.OPERATOR_ORDERS}?limit=20`);
        if (res.ok) {
          const data = await res.json();
          setOrders(data.orders || data || []);
        }
      } else if (t === 'decisions') {
        const res = await fetchOperator(`${API_BASE_URL}${API_ENDPOINTS.SYSTEM_DECISIONS}?limit=10`);
        if (res.ok) {
          const data = await res.json();
          setDecisions(data.decisions || []);
        }
      } else if (t === 'audit') {
        const res = await fetchOperator(`${API_BASE_URL}${API_ENDPOINTS.OPERATOR_AUDIT_TRAIL}?limit=20`);
        if (res.ok) {
          const data = await res.json();
