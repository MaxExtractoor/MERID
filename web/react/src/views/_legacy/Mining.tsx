...
export default function Mining() {
  ...
  const { data: rawData, loading, error, rawResponse, refetch } = useApiData<MiningData>(
    API_ENDPOINTS.MINING_OVERVIEW,
    { pollingInterval: DEFAULTS.POLLING_INTERVALS.SLOW },
  );
  const rigs = rawData?.rigs ?? [];
  const pools = rawData?.pools ?? [];
          ...
          {rigs.map(rig => (
            <div key={rig.id} className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-6">
              ...
              <div className="mt-4 flex gap-2">
                <button type="button" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition-colors" title="Configure">
                  Configure
                </button>
                {rig.status === 'active' ? (
                  <button type="button" className="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg text-sm transition-colors" title="Stop">
                    Stop
                  </button>
                ) : (
                  <button type="button" className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm transition-colors" title="Start">
                    Start
                  </button>
                )}
                <button type="button" className="px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg text-sm transition-colors" title="Restart">
                  Restart
                </button>
              </div>
            </div>
          ...
          {pools.map(pool => (
            <div key={pool.id} className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-6">
              ...
              <div className="flex gap-2">
                <button type="button" className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm transition-colors" title="Request Payout">
                  Request Payout
                </button>
                <button type="button" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition-colors" title="View Stats">
                  View Stats
                </button>
              </div>
            </div>
