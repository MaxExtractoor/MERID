...
export default function Plugins() {
  ...
  const { data: rawData, loading, error, refetch } = useApiData<PluginData>(
    API_ENDPOINTS.PLUGINS_LIST,
    { pollingInterval: DEFAULTS.POLLING_INTERVALS.BACKGROUND },
  );
  const plugins = rawData?.plugins ?? [];
  const stats = rawData?.stats ?? null;

  const installPlugin = async (pluginId: string) => {
    try {
      await fetch(API_ENDPOINTS.PLUGINS_INSTALL(pluginId), { method: 'POST' });
      refetch();
    } catch {
      // Install failed — UI state unchanged
    }
  };

  const uninstallPlugin = async (pluginId: string) => {
    try {
      await fetch(API_ENDPOINTS.PLUGINS_UNINSTALL(pluginId), { method: 'DELETE' });
      refetch();
    } catch {
      // Uninstall failed — UI state unchanged
    }
  };

  const togglePlugin = async (pluginId: string) => {
    try {
      await fetch(API_ENDPOINTS.PLUGINS_TOGGLE(pluginId), { method: 'POST' });
      refetch();
    } catch {
      // Toggle failed — UI state unchanged
    }
  };
            ...
            <div className="flex gap-2">
              {plugin.status === 'installed' ? (
                ...
                <>
                  ...
                  <button type="button" className="px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg text-sm transition-colors flex items-center gap-2">
