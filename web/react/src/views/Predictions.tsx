...
function useMarkets() {
  ...
  const [meta, setMeta] = useState({ total: 0, open: 0, totalVolume: 0, totalPnl: 0 });
  const [loading, setLoading] = useState(true);

  const fetch_ = useCallback(async () => {
    try {
      const res = await fetch('/api/v1/us-compliant/prediction-markets');
      if (res.ok) {
  ...
  useEffect(() => { fetch_(); const i = setInterval(fetch_, 15_000); return () => clearInterval(i); }, [fetch_]);
  return { markets, meta, loading, refresh: fetch_ };
}
...
function usePMSummary() {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const r = await fetch('/api/v1/prediction-markets/summary');
        if (r.ok) setSummary(await r.json());
      } catch { /* silent */ }
    };
    f(); const i = setInterval(f, 15_000); return () => clearInterval(i);
  }, []);
...
function useDriftSignals() {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const r = await fetch('/api/v1/us-compliant/drift-signals?limit=30');
        if (r.ok) { const d = await r.json(); setSignals(d.signals || []); }
      } catch { /* silent */ }
      finally { setLoading(false); }
    };
    f(); const i = setInterval(f, 30_000); return () => clearInterval(i);
  }, []);
...
function MarketDetail({ market, onClose }: { market: Market; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch('/api/v1/trading-mode/spectator/record', {
        method: 'POST',
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-lg bg-slate-900 border-l border-slate-700 overflow-y-auto"
...
function AgentActivityPanel() {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const r = await fetch('/api/v1/trading-mode/spectator/live?limit=15');
        if (r.ok) {
        ...
        {activity.map((t: any, i: number) => (
          <div key={i} className="p-3 hover:bg-slate-800/30 transition-colors">
            ...
            <div className="flex items-center gap-2 text-[10px] text-slate-500">
              ...
              {t.confidence > 0 && <span>{Math.round(t.confidence * 100)}% conf</span>}
            </div>
          </div>
        ))}
      </div>
...
export default function Predictions() {
  ...
  const _handlePMAction = async (action: 'pause' | 'resume') => {
    try {
      await fetch(`/api/v1/prediction-markets/${action}`, { method: 'POST' });
      refresh();
    } catch { /* silent */ }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={refresh} className="p-2 rounded-lg hover:bg-slate-800 transition-colors" title="Refresh">
