...
import { API_BASE_URL, API_ENDPOINTS, DEFAULTS, CHART_COLORS, AUTH_TOKEN_KEY} from '../config/constants';
import KalshiTradeTicket from '../components/KalshiTradeTicket';
// ExecutionGateStrip import moved to OperatorHeader
...
import { SkeletonMetricRow, SkeletonTable } from '../components/SkeletonLoader';
import OperatorHeader from '../components/OperatorHeader';

// Import debate status badge component
import DebateStatusBadge from '../components/DebateStatusBadge';
import DebateContextPanel from '../components/DebateContextPanel';
...
type QuickTab = 'all' | 'trending' | 'live' | 'cross-category' | 'sports' | 'crypto-hourly' | 'crypto-15m' | 'top-volume' | 'new-markets' | 'my-positions' | 'favorites';
...
const QUICK_TABS: { id: QuickTab; label: string; icon: string }[] = [
  ...
  { id: 'cross-category', label: 'Multi-Event',     icon: 'layers' },
  ...
  { id: 'new-markets',    label: 'Expiring Soon',   icon: 'star' },
...
interface DebateBadge { label: string; color: string; tooltip?: string }
...
function debateBadge(hasCritical: boolean, hasWarnings: boolean, totalAlerts: number, topAlert: any): DebateBadge | null {
  ...
  if (hasCritical) {
    return {
      ...
      tooltip: `Debate system critical: ${totalAlerts} alerts (including critical). Latest: ${topAlert?.message || 'System degraded'}`
    };
  }
  ...
  if (hasWarnings) {
    return {
      ...
      tooltip: `Debate system degraded: ${totalAlerts} warnings. Latest: ${topAlert?.message || 'System warnings active'}`
    };
  }
  ...
  return {
    ...
    tooltip: `Debate system active: ${totalAlerts} alerts. Latest: ${topAlert?.message || 'System monitoring active'}`
  };
}
...
function getDataFreshness(timestamp: string | null | undefined, now: Date = new Date()): FreshnessStatus {
  if (!timestamp) return { level: 'old', color: 'text-red-400', label: 'no data' };
  
  const ts = new Date(timestamp);
  const diffMs = now.getTime() - ts.getTime();
  const diffSec = diffMs / 1000;
  
  if (diffSec < 5) return { level: 'fresh', color: 'text-green-400', label: '<5s' };
  if (diffSec < 30) return { level: 'stale', color: 'text-yellow-400', label: `${Math.round(diffSec)}s` };
  return { level: 'old', color: 'text-red-400', label: `${Math.round(diffSec / 60)}m` };
}

function DataFreshnessBadge({ timestamp, label }: { timestamp: string | null | undefined; label: string }) {
  const freshness = useMemo(() => getDataFreshness(timestamp), [timestamp]);
  return (
    <span className={`text-[10px] ${freshness.color}`} title={`${label} updated ${freshness.label} ago`}>
      {freshness.label}
    </span>
  );
}
// Fixed: Added missing localStorage helper functions

// LocalStorage-only helpers for favorites
function loadFavoritesLocal(): Set<string> {
  try {
    const raw = localStorage.getItem('kalshi_favorites');
...
function saveFavoritesLocal(favs: Set<string>): void {
  try {
...
// Server-side favorites load/save available but currently using localStorage only
// See loadFavoritesLocal / saveFavoritesLocal above
        ...
        <div className="space-y-0.5">
          {data.yes_bids.slice(0, 5).map((l, i) => (
            <div key={i} className="relative flex justify-between text-[10px] px-1">
              ...
              <span className="relative text-green-400 font-mono">{((l.price ?? 0) * 100).toFixed(0)}¢</span>
        ...
        <div className="space-y-0.5">
          {data.yes_asks.slice(0, 5).map((l, i) => (
            <div key={i} className="relative flex justify-between text-[10px] px-1">
              ...
              <span className="relative text-red-400 font-mono">{((l.price ?? 0) * 100).toFixed(0)}¢</span>
              <span className="relative text-gray-400">{l.quantity}</span>
            </div>
          ))}
        </div>
...
const KalshiDashboardView: React.FC = () => {
  ...
  // Debate context for market-level awareness
  const { 
    getDebateStatus: _getDebateStatus, 
    hasCriticalAlerts, 
    hasWarnings, 
    getTotalAlerts, 
    getTopAlert 
  ...
  // Auto-refresh catalog on mount — rate-limited to once per 60s across navigations.
  // M06 fix: autoRefreshedRef only guards within a single mount lifecycle; navigation
  // resets it. Use localStorage timestamp to enforce cross-navigation rate limit.
  const CATALOG_REFRESH_INTERVAL_MS = 60_000;
  const CATALOG_REFRESH_TS_KEY = 'merid:catalog_last_refresh';
  useEffect(() => {
    if (autoRefreshedRef.current) return;
    autoRefreshedRef.current = true;
    const lastRefresh = parseInt(localStorage.getItem(CATALOG_REFRESH_TS_KEY) ?? '0', 10);
    ...
    (async () => {
      ...
      try {
        ...
        localStorage.setItem(CATALOG_REFRESH_TS_KEY, String(Date.now()));
        await Promise.all([mktsRefetchRef.current(), catRefetchRef.current()]);
  ...
  const effectiveCategory =
    (quickTab === 'crypto-hourly' || quickTab === 'crypto-15m') ? 'crypto' :
    quickTab === 'cross-category' ? 'cross_category' :
    quickTab === 'sports' ? 'sports' :
    filterCategory;
  ...
  // Keep ref current so the auto-refresh effect (empty deps) always calls the latest refetch
  const mktsRefetchRef = useRef(mktsResult.refetch);
  mktsRefetchRef.current = mktsResult.refetch;
  ...
  const catRefetchRef = useRef(catResult.refetch);
  catRefetchRef.current = catResult.refetch;
  const healthResult = useApiData<HealthStatus>(API_ENDPOINTS.KALSHI_HEALTH, {
    pollingInterval: DEFAULTS.POLLING_INTERVALS.SLOW,
  });
  const posResult = useApiData<{ positions: { ticker: string }[]; reconciliation_status?: string | { status: string; last_run?: string; divergence_count?: number }; timestamp?: string }>(API_ENDPOINTS.KALSHI_POSITIONS, {
    pollingInterval: DEFAULTS.POLLING_INTERVALS.STANDARD,
  });
  ...
  const balResult = useApiData<{ usd: number; locked: number; available: number }>(API_ENDPOINTS.KALSHI_BALANCE, {
    pollingInterval: DEFAULTS.POLLING_INTERVALS.SLOW,
  });
  ...
  // Add risk result for unified PnL display
  const riskResult = useApiData<{
    daily_pnl_usd: number;
    drawdown_pct: number;
    timestamp?: string;
    kill_switch_active?: boolean;
    limits?: { orders_this_minute?: number; max_orders_per_minute?: number };
  ...
  // Unified position count from positions endpoint (not health.risk)
  const positionCount = useMemo(() => {
    // Count only non-synthetic positions from the positions endpoint
    const positions = posResult.data?.positions ?? [];
    return positions.filter((p: any) => !p.synthetic).length;
  }, [posResult.data]);

  // Position tickers for filtering
  const positionTickers = useMemo(() => {
    const positions = posResult.data?.positions ?? [];
    return new Set(positions.filter((p: any) => !p.synthetic).map((p: any) => p.ticker));
  }, [posResult.data]);
  ...
  const markets = useMemo(() => {
    ...
    // Expiring Soon: sort by nearest expiry (soonest first)
  ...
  return (
    <div className="space-y-4">
      {/* Global Operator Header — always visible */}
      <OperatorHeader />
      ...
      {isLive && (
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500/10 border border-green-500/30 text-green-400 text-xs font-medium">
          ...
          LIVE TRADING ACTIVE — real orders on Kalshi. Kill switch is {riskResult.data?.kill_switch_active ? <span className="text-red-400 font-bold ml-1">ENGAGED</span> : <span className="text-green-300 ml-1">clear</span>}.
        </div>
      )}

      {/* Reconciliation Status Indicator */}
      {(() => {
        const recon = posResult.data?.reconciliation_status;
        if (!recon) return null;
        // Handle both string format (legacy) and object format {status, last_run, divergence_count, ...}
        const reconStatus = typeof recon === 'string' ? recon : recon.status;
        if (!reconStatus || reconStatus === 'ok') return null;
        return (
          <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
            ...
            <span>Reconciliation {reconStatus} — positions may not match fills</span>
          </div>
        );
      })()}
      ...
      {refreshError && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm">
          ...
          <button type="button" onClick={() => setRefreshError(null)} className="ml-auto text-red-400 hover:text-red-300" title="Dismiss" aria-label="Dismiss error"><Icon name="close" size={16} className="w-4 h-4" /></button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Icon name="barChart" size={24} className="w-6 h-6 text-orange-400" />
          <div>
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              Kalshi Markets
              <KalshiModeBadge />
              {catalogRefreshing && <Icon name="refresh" size={14} className="w-3.5 h-3.5 text-gray-500 animate-spin" />}
            </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          {balResult.data && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800">
              <Icon name="dollarSign" size={14} className="w-3.5 h-3.5 text-green-400" />
          ...
          {health && (
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg ${healthBg}`}>
              <Icon name="heart" size={14} className={`w-3.5 h-3.5 ${healthColor}`} />
          ...
          >
            <Icon name="briefcase" size={14} className="w-3.5 h-3.5" />
            {positionCount > 0 && <span className="font-bold">{positionCount}</span>}
          </button>
          <button
            type="button"
          ...
          >
            <Icon name="refresh" size={14} className={`w-3.5 h-3.5 ${catalogRefreshing ? 'animate-spin' : ''}`} />
            Refresh
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
        ...
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
          <div className="flex items-center gap-1.5 mb-1">
            <Icon name="activity" size={14} className="w-3.5 h-3.5 text-blue-400" />
        ...
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
          ...
          <div className="flex items-baseline gap-2">
            <p className={`text-lg font-bold ${(riskResult.data?.daily_pnl_usd ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              ${riskResult.data?.daily_pnl_usd?.toFixed(2) ?? '0.00'}
            </p>
            {riskResult.data?.timestamp && (
              <DataFreshnessBadge timestamp={riskResult.data.timestamp} label="Risk" />
            )}
          </div>
          <p className="text-[10px] text-gray-600">DD: {riskResult.data?.drawdown_pct?.toFixed(1) ?? 0}%</p>
        </div>
        {/* Kelly */}
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
          <div className="flex items-center gap-1.5 mb-1">
            <Icon name="gauge" size={14} className="w-3.5 h-3.5 text-orange-400" />
        ...
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
          <div className="flex items-center gap-1.5">
            <Icon name="briefcase" size={14} className="w-3.5 h-3.5 text-blue-400" />
          ...
          <div className="flex items-baseline gap-2">
            ...
            {posResult.data?.timestamp && (
              <DataFreshnessBadge timestamp={posResult.data.timestamp} label="Positions" />
            )}
          </div>
        ...
        <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
          <div className="flex items-center gap-1.5 mb-1">
            {health?.ws.running ? <Icon name="wifi" size={14} className="w-3.5 h-3.5 text-cyan-400" /> : <Icon name="wifiOff" size={14} className="w-3.5 h-3.5 text-red-400" />}
        ...
        {/* Kill Switch — now from canonical /risk endpoint */}
        <div className={`rounded-xl p-3 border ${riskResult.data?.kill_switch_active ? 'bg-red-500/10 border-red-500/30' : 'bg-slate-900 border-slate-800'}`}>
          <div className="flex items-center gap-1.5 mb-1">
            <Icon name="shield" size={14} className={`w-3.5 h-3.5 ${riskResult.data?.kill_switch_active ? 'text-red-400' : 'text-green-400'}`} />
            <span className="text-[10px] text-gray-500 uppercase tracking-wider">Kill Switch</span>
          </div>
          <p className={`text-lg font-bold ${riskResult.data?.kill_switch_active ? 'text-red-400' : 'text-green-400'}`}>
            {riskResult.data?.kill_switch_active ? 'ACTIVE' : 'OFF'}
          </p>
          <p className="text-[10px] text-gray-600">{riskResult.data?.limits?.orders_this_minute ?? 0}/{riskResult.data?.limits?.max_orders_per_minute ?? 30}/min</p>
        </div>
        {/* Consensus Signals */}
        <div className={`rounded-xl p-3 border ${consensusResult.data?.engine_running ? 'bg-slate-900 border-slate-800' : 'bg-slate-900/50 border-slate-800/50'}`}>
          <div className="flex items-center gap-1.5 mb-1">
            <Icon name="radio" size={14} className={`w-3.5 h-3.5 ${consensusResult.data?.engine_running ? 'text-purple-400' : 'text-gray-600'}`} />
          ...
          <p className="text-[10px] text-gray-600">{consensusResult.data ? `${((consensusResult.data.consensus_rate ?? 0) * 100).toFixed(0)}% rate` : 'engine off'}</p>
        </div>
        {/* Debate Context */}
        <DebateContextPanel />
      </div>
      ...
      {newsResult.data && newsResult.data.count > 0 && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-3">
          <div className="flex items-center gap-2 mb-2">
            <Icon name="radio" size={14} className="w-3.5 h-3.5 text-blue-400" />
          ...
          <div className="flex flex-col gap-1.5 max-h-32 overflow-y-auto">
            {newsResult.data.signals.slice(0, 5).map((s, i) => (
              <div key={`${s.title}-${s.source}-${i}`} className="flex items-start gap-2 text-xs">
      ...
      {/* Search */}
      <div className="flex items-center gap-2">
        <Icon name="search" size={16} className="w-4 h-4 text-gray-400 shrink-0" />
        <input
          type="text"
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Icon name="filter" size={14} className="w-3.5 h-3.5 text-gray-500 shrink-0" />
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            >
              <Icon name="menu" size={12} className="w-3 h-3" /> OB
            </button>
          </div>
          <div className="flex gap-1.5">
            {(['volume', 'expiry', 'spread'] as SortKey[]).map((key) => (
              ...
              >
                {key === 'volume' && <Icon name="trendingUp" size={12} className="w-3 h-3" />}
                {key === 'expiry' && <Icon name="clock" size={12} className="w-3 h-3" />}
                {key === 'spread' && <Icon name={"arrowUpDown" as any} size={12} className="w-3 h-3" />}
      ...
      <div className="flex items-center gap-4 text-[10px] text-gray-500 px-2">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-green-400" /> {'<5s fresh'}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-yellow-400" /> {'5-30s stale'}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-red-400" /> {'>30s old'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          <>
            <SkeletonMetricRow count={7} />
            <SkeletonTable rows={6} columns={4} />
          </>
        ) : markets.length === 0 ? (
          <div className="col-span-full text-center py-12 text-gray-500">
            {(quickTab === 'crypto-hourly' || quickTab === 'crypto-15m') ? (
              <div className="space-y-2">
                <p className="text-amber-400 font-medium">No active crypto prediction markets on Kalshi right now.</p>
                <p className="text-xs text-gray-600">Crypto markets (BTC/ETH/SOL/XRP/DOGE) are not currently listed. Try the <button onClick={() => setQuickTab('cross-category')} className="text-violet-400 underline">Multi-Event</button> or <button onClick={() => setQuickTab('all')} className="text-blue-400 underline">All Markets</button> tabs.</p>
              </div>
            ) : (
              <p>No markets found. Try adjusting filters or refreshing the catalog.</p>
            )}
          </div>
        ) : (
          // UI-008 fix: compute global debate badge once outside the per-market loop
          (() => {
          const globalDebateBadge = debateBadge(hasCriticalAlerts(), hasWarnings(), getTotalAlerts(), getTopAlert());
            ...
            >
              ...
              <div className="relative flex items-start justify-between mb-2">
                ...
                <div className="relative z-10 flex items-center gap-1 shrink-0 ml-2">
                  ...
                  >
                    <Icon name="star" size={14} className={`w-3.5 h-3.5 ${favorites.has(m.ticker) ? 'fill-current' : ''}`} />
                  </button>
                  <Icon name="chevronRight" size={16} className="w-4 h-4 text-gray-600 group-hover:text-orange-400" />
                </div>
              </div>
              ...
              <div className="text-xs text-gray-500 mb-2 space-y-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {(m as any).tick_size && (
                    <span className="flex items-center gap-1">
                      <Icon name="trendingUp" size={10} className="w-2.5 h-2.5" />
                      Tick: {(m as any).tick_size}¢
                    </span>
                  )}
                  {(m as any).fee_rate && (
                    <span className="flex items-center gap-1">
                      <Icon name="percent" size={10} className="w-2.5 h-2.5" />
                      Fee: {(((m as any).fee_rate ?? 0) * 100).toFixed(1)}%
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      {globalDebateBadge && (
                        <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${globalDebateBadge.color}`} title={globalDebateBadge.tooltip}>
                          {globalDebateBadge.label}
                        </span>
                      )}
                    </>
                  );
                ...
                {positionTickers.has(m.ticker) && (
                  <span className="px-2 py-0.5 rounded text-[10px] font-medium text-orange-300 bg-orange-500/20">
                    <Icon name="briefcase" size={10} className="w-2.5 h-2.5 inline mr-0.5" />POSITION
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-3">
                  {(m.outcomes ?? []).map((o) => {
                    ...
                    const evCents = sig ? sig.ev_cents : null;
                    ...
                    return (
                      <div key={o.id} className="text-center">
                        ...
                        <p className={`text-sm font-bold ${(o.price ?? 0) >= 0.5 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {showPositionsSidebar && positionTickers.size > 0 && (
        <div className="w-56 shrink-0 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <Icon name="briefcase" size={14} className="w-3.5 h-3.5 text-orange-400" />
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-slate-900 border-l border-slate-800 overflow-y-auto">
            <div className="sticky top-0 bg-slate-900 border-b border-slate-800 p-4 flex items-center justify-between">
              ...
              >
                <Icon name="x" size={20} className="w-5 h-5" />
              </button>
            </div>
                  ...
                  {(selectedMarket.outcomes ?? []).map((o) => (
                    <div key={o.id} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className={`text-lg font-bold ${(o.price ?? 0) >= 0.5 ? 'text-green-400' : 'text-red-400'}`}>
                            {((o.price ?? 0) * 100).toFixed(1)}¢
                          </p>
                        </div>
                        {o.bid != null && o.ask != null && (
                          <div className="text-xs text-gray-500">
                            <p>Bid: {((o.bid ?? 0) * 100).toFixed(0)}¢</p>
                            <p>Ask: {((o.ask ?? 0) * 100).toFixed(0)}¢</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                return (
                  <div className="bg-slate-800 rounded-lg p-4">
                    ...
                    <p className="text-[10px] text-gray-500 mt-2 text-center">
                      Mid: {mid.toFixed(1)}¢ · Implied: {((o.price ?? 0) * 100).toFixed(1)}¢
                    ...
                    <div className="relative h-16">
                      <svg width="100%" height="100%" viewBox={`0 0 ${raw.length} 100`} preserveAspectRatio="none">
                        ...
                        {raw.map((p, i) => {
                          ...
                          return <rect key={i} x={i * w} y={100 - h} width={w * 0.8} height={h} fill={CHART_COLORS.AMBER} opacity="0.4" />;
                        })}
                        {/* Smoothed line */}
                        {smoothed.length > 1 && (
                          <polyline
