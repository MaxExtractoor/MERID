...
const KalshiVolDashboardView: React.FC = () => {
  ...
  const gridRes         = useApiData<VolDashGridStatus>(API_ENDPOINTS.KALSHI_GRID_STATUS, std);
  ...
  // ── Sentiment/Vol context (from canonical API) ────────────────────────────
  ...
  }>(API_ENDPOINTS.SENTIMENT_VOL_ASSETS, std);
  ...
  const healthColor = health?.status === HEALTH_STATUS.HEALTHY ? 'text-green-400'
    : health?.status === HEALTH_STATUS.DEGRADED ? 'text-yellow-400' : 'text-red-400';
