...
export function formatDate(iso: string | Date | null | undefined, format?: string): string {
  if (iso == null) return 'N/A';
  const date = typeof iso === "string" ? new Date(iso) : iso;
...
export function fmtTimestamp(
  ...
  opts?: { timeOnly?: boolean; dateOnly?: boolean }
): string {
  ...
  if (typeof ts === 'number') {
    ...
    date = ts instanceof Date ? ts : new Date(ts);
  }
  ...
  if (opts?.timeOnly) return `${h}:${m}:${s} UTC`;
