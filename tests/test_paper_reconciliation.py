...
class TestPaperStatePersistence(unittest.TestCase):
    ...
    def test_save_and_restore_positions(self):
        ...
        try:
            ...
            # Post-load repair recomputes total_pnl from closed_positions realized PnL only.
            portfolio.closed_positions.append(
                PaperPosition(
                    position_id="closed_1",
                    user_id="test_user",
                    asset="BTC",
                    side="long",
                    size_usd=500.0,
            ...
            # Post-load repair enforces cash identity vs margin on open positions.
            self.assertAlmostEqual(p2.current_balance, 9650.0)
            self.assertAlmostEqual(p2.total_pnl, 150.0)
            # Repair syncs total_trades to len(trade_history) and win/loss to closed PnL.
            self.assertEqual(p2.total_trades, 1)
            self.assertEqual(p2.winning_trades, 1)
            self.assertEqual(p2.losing_trades, 0)
...
class TestReconciliation(unittest.TestCase):
    """§4 — Venue reconciliation (merid) runs without error."""

    def test_reconciliation_runs_without_error(self):
        from merid.reconciliation import get_last_reconciliation_ts, reconcile_all_venues

        reconcile_all_venues(["kalshi"])
        self.assertGreater(get_last_reconciliation_ts(), 0.0)
    ...
    def test_reconciliation_report_serializable(self):
        ...
        self.assertIn("discrepancy_count", d)
        self.assertIn("execution_gate_blocked", d)
        json.dumps(d)

    def test_legacy_trading_reconciliation_deprecated(self):
        import warnings
...
class TestVenueGateAlignment(unittest.TestCase):
    ...
    def test_venue_gate_modes_exist(self):
        ...
        self.assertEqual(TradingMode.SIM.value, "mock")
...
class TestRuntimeConfigAlignment(unittest.TestCase):
    ...
    def test_global_trading_mode_has_paper(self):
        ...
        # Season 5: GlobalTradingMode is canonical TradeMode (mock/paper/live).
        # Legacy "sim" is TradeMode.SIM → MOCK ("mock").
        values = [m.value for m in GlobalTradingMode]
