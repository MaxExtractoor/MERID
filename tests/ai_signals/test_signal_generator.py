...
# ── signal_quality confidence cap ─────────────────────────────────────

class TestSignalQualityCap:
    """Verify signal_quality-based confidence ceiling in generate_signal()."""

    def _apply_cap(self, raw: SignalConfidence, signal_quality: str) -> SignalConfidence:
        """Mirror the cap logic from signal_generator.py for unit-testing."""
        result = raw
        if signal_quality == "medium" and raw.value > SignalConfidence.HIGH.value:
            result = SignalConfidence.HIGH
        elif signal_quality == "low" and raw.value > SignalConfidence.MEDIUM.value:
            result = SignalConfidence.MEDIUM
        return result

    def test_high_quality_no_cap(self):
        """'high' signal_quality: VERY_HIGH passes through unchanged."""
        assert self._apply_cap(SignalConfidence.VERY_HIGH, "high") == SignalConfidence.VERY_HIGH

    def test_high_quality_all_levels_pass(self):
        """'high' quality never reduces any confidence level."""
        for level in SignalConfidence:
            assert self._apply_cap(level, "high") == level

    def test_medium_quality_caps_very_high(self):
        """'medium' quality caps VERY_HIGH → HIGH."""
        assert self._apply_cap(SignalConfidence.VERY_HIGH, "medium") == SignalConfidence.HIGH

    def test_medium_quality_leaves_high_unchanged(self):
        """'medium' quality leaves HIGH unchanged (HIGH is the ceiling)."""
        assert self._apply_cap(SignalConfidence.HIGH, "medium") == SignalConfidence.HIGH

    def test_medium_quality_leaves_medium_unchanged(self):
        assert self._apply_cap(SignalConfidence.MEDIUM, "medium") == SignalConfidence.MEDIUM

    def test_medium_quality_leaves_low_unchanged(self):
        assert self._apply_cap(SignalConfidence.LOW, "medium") == SignalConfidence.LOW

    def test_low_quality_caps_very_high(self):
        """'low' quality caps VERY_HIGH → MEDIUM."""
        assert self._apply_cap(SignalConfidence.VERY_HIGH, "low") == SignalConfidence.MEDIUM

    def test_low_quality_caps_high(self):
        """'low' quality caps HIGH → MEDIUM."""
        assert self._apply_cap(SignalConfidence.HIGH, "low") == SignalConfidence.MEDIUM

    def test_low_quality_leaves_medium_unchanged(self):
        assert self._apply_cap(SignalConfidence.MEDIUM, "low") == SignalConfidence.MEDIUM

    def test_low_quality_leaves_low_unchanged(self):
        assert self._apply_cap(SignalConfidence.LOW, "low") == SignalConfidence.LOW

    def test_low_quality_leaves_very_low_unchanged(self):
        assert self._apply_cap(SignalConfidence.VERY_LOW, "low") == SignalConfidence.VERY_LOW

    def test_asset_universe_high_signal_btc(self):
        """BTC has signal_quality='high' — no cap applied."""
        from data.asset_universe import ASSET_UNIVERSE
        btc = ASSET_UNIVERSE["BTC"]
        assert btc.signal_quality == "high"
        assert self._apply_cap(SignalConfidence.VERY_HIGH, btc.signal_quality) == SignalConfidence.VERY_HIGH

    def test_asset_universe_low_signal_algo(self):
        """ALGO has signal_quality='low' — HIGH capped to MEDIUM."""
        from data.asset_universe import ASSET_UNIVERSE
        algo = ASSET_UNIVERSE.get("ALGO")
        assert algo is not None
        assert algo.signal_quality == "low"
        assert self._apply_cap(SignalConfidence.HIGH, algo.signal_quality) == SignalConfidence.MEDIUM

    def test_unknown_symbol_no_cap_applied(self):
        """Unknown symbol falls back gracefully — confidence unchanged (try/except)."""
        from data.asset_universe import ASSET_UNIVERSE
        assert ASSET_UNIVERSE.get("GHOST") is None
        # No asset found → cap logic skipped → original confidence returned
        cap = self._apply_cap(SignalConfidence.VERY_HIGH, "high")  # default for unknown
        assert cap == SignalConfidence.VERY_HIGH
