...
class TestReconciliationUnified:
    ...
    def test_has_critical_discrepancies_uses_lock(self):
        src = (PROJECT_ROOT / "merid/reconciliation.py").read_text(encoding="utf-8")
        # Find has_critical_discrepancies function body
    ...
    def test_frontend_constant_exists(self):
        ...
        assert "RECONCILIATION_UNIFIED" in src
        assert "/api/v1/reconciliation/unified-status" in src
...
class TestCrossModuleWiring:
    ...
    def test_operator_dashboard_imports_data_health(self):
        src = (PROJECT_ROOT / "web/react/src/views/OperatorDashboard.tsx").read_text(encoding="utf-8")
        assert "import DataHealthCard" in src
...
class TestReconciliationModules:
    """Verify both reconciliation modules are importable and functional."""

    def test_trading_reconciliation_importable(self):
        from trading.reconciliation import run_reconciliation, get_last_report
        assert callable(run_reconciliation)
        assert callable(get_last_report)
    ...
    def test_trading_reconciliation_run(self):
...
class TestErrorBarUsage:
    ...
    def test_errorbar_component_exists(self):
        ...
        assert "export default function ErrorBar" in text
