...
class TestRiskContractFields:
    ...
    @patch('merid.prediction.kalshi_tools._get_client')
    @patch('merid.prediction.kalshi_tools.get_venue_gate')
    def test_kalshi_place_order_includes_risk_contract_fields(self, mock_gate, mock_client, mock_route):
        """Test that _kalshi_place_order includes risk contract fields in OrderIntent for crypto 15m markets."""
        from merid.prediction.kalshi_tools import _kalshi_place_order
        from merid.event_venues.kalshi.order_router import OrderResult, TradingMode
        from unittest.mock import AsyncMock
        ...
        # Mock route_order_async to capture the OrderIntent
        ...
        mock_route.side_effect = capture_intent
        
        # Call _kalshi_place_order for a crypto 15m market
        import asyncio
        result = asyncio.run(_kalshi_place_order(
            ticker="KXBTC15M-26JUL081400-00",
            side="yes",
        ...
        # Verify risk contract fields were set in OrderIntent
        assert captured_intent is not None, "OrderIntent was not captured"
        assert captured_intent.window_resolution_id is not None, "window_resolution_id should be set"
        assert captured_intent.exit_policy_id is not None, "exit_policy_id should be set"
        assert captured_intent.risk_tier is not None, "risk_tier should be set"
        assert captured_intent.max_hold_seconds is not None, "max_hold_seconds should be set"
        
        # Verify default values are reasonable
        assert captured_intent.window_resolution_id == "15m", f"Expected window_resolution_id='15m', got {captured_intent.window_resolution_id}"
        assert captured_intent.exit_policy_id == "tp_sl_15m", f"Expected exit_policy_id='tp_sl_15m', got {captured_intent.exit_policy_id}"
        assert captured_intent.risk_tier == "conservative", f"Expected risk_tier='conservative', got {captured_intent.risk_tier}"
        assert captured_intent.max_hold_seconds == 900, f"Expected max_hold_seconds=900, got {captured_intent.max_hold_seconds}"
