...
class TestKalshiFeeCalc(unittest.TestCase):
    ...
    def test_fee_tier1_small(self):
        ...
        # price=50, payout=50, 7% of 50 = 3.5 → ceil = 4¢ per contract
        # 10 contracts → 40¢
        fee = self.fee(50, 10)
        self.assertEqual(fee, 40)

    def test_fee_tier1_min_floor(self):
        """Min fee is 2¢ per contract even if 7% is less."""
        # price=95, payout=5, 7% of 5 = 0.35 → ceil = 1, but min is 2
        fee = self.fee(95, 10)
        self.assertEqual(fee, 20)  # 2¢ × 10

    def test_fee_tier2_medium(self):
        """100-999 contracts: 5% of payout."""
        # price=50, payout=50, 5% of 50 = 2.5 → ceil = 3¢ per contract
        fee = self.fee(50, 100)
        self.assertEqual(fee, 300)  # 3¢ × 100

    def test_fee_tier3_large(self):
        """1000+ contracts: 3% of payout."""
        # price=50, payout=50, 3% of 50 = 1.5 → ceil = 2¢ per contract
        fee = self.fee(50, 1000)
        self.assertEqual(fee, 2000)  # 2¢ × 1000
    ...
    def test_fee_high_price_low_payout(self):
        ...
        # 7% of 10 = 0.7 → ceil = 1, min 2 → 2¢ per contract
        fee = self.fee(90, 50)
        self.assertEqual(fee, 100)  # 2¢ × 50

    def test_fee_low_price_high_payout(self):
        """Low price (10¢), high payout (90¢)."""
        # 7% of 90 = 6.3 → ceil = 7¢ per contract
        fee = self.fee(10, 50)
        self.assertEqual(fee, 350)  # 7¢ × 50
...
class TestKalshiRiskManager(unittest.TestCase):
    ...
    def test_daily_loss_triggers_kill_switch(self):
        self.risk._state.daily_pnl_usd = -600  # exceeds $500 max
        ok, reason = self.risk.check_order("BTC-TEST", "crypto", 10, 50)
        self.assertFalse(ok)
        self.assertTrue(self.risk.kill_switch_active)
    ...
    def test_drawdown_unwind_triggers_kill_switch(self):
        self.risk._state.peak_equity_usd = 10000
...
class TestMarketCatalog(unittest.TestCase):
    ...
    def test_detect_timeframe_from_expiry(self):
        ...
        # 60 days → yearly
        self.assertEqual(self.catalog._detect_timeframe("test", now + timedelta(days=60), now), "yearly")
...
class TestKalshiApiWiring(unittest.TestCase):
    ...
    def test_import(self):
        self.assertIn("from web.api.kalshi_api import router as kalshi_api_router", self.source)
...
class TestFrontendConstants(unittest.TestCase):
    ...
    def test_kalshi_ws(self):
        self.assertIn("KALSHI_WS", self.source)
...
class TestDynamicPositionSizes(unittest.TestCase):
    ...
    def test_fee_check_filters_bad_trades(self):
        """Very high price (99¢) with tiny payout should be filtered by fee check."""
        markets = [{"ticker": "A", "edge_pct": 1.0, "price_cents": 99}]
        sizes = self.dps(markets, 100)
        # 30% of $100 = $30 budget, at 99¢ = 30 contracts
        # payout = 1¢ × 30 = 30¢, fee = 2¢ × 30 = 60¢ → net negative → filtered
        self.assertEqual(sizes.get("A", 0), 0)
...
class TestClientFixes(unittest.TestCase):
    ...
    def test_rsa_message_format(self):
        """RSA signs: timestamp_ms + METHOD + path + body."""
        self.assertIn("ts_ms + method.upper() + path + body", self.source)
    ...
    def test_time_in_force(self):
        ...
        self.assertIn('"gtc"', self.source)
        self.assertIn('"ioc"', self.source)
        self.assertIn('"fok"', self.source)
...
class TestOrderEndpoint(unittest.TestCase):
    ...
    def test_mode_parameter(self):
        self.assertIn('mode: str = "paper"', self.source)
...
class TestErrorTypes(unittest.TestCase):
    ...
    def test_session_error_in_source(self):
        ...
        self.assertIn("service unavailable", source.lower())
...
class TestOrderRouter(unittest.TestCase):
    ...
    def test_route_order_async_live_rejects_when_live_disabled(self):
        ...
        with patch("merid.event_venues.kalshi.order_router.get_venue_gate", return_value=gate):
    ...
    def test_route_order_async_live_uses_kalshi_client(self):
        ...
        with patch("merid.event_venues.kalshi.order_router.get_venue_gate", return_value=gate), patch(
            "merid.event_venues.kalshi.client.get_kalshi_client", return_value=client
        ):
            result = asyncio.run(route_order_async(intent))
...
class TestSidebarConfigAPI(unittest.TestCase):
    ...
    def test_six_sections(self):
        ...
        self.assertEqual(len(SIDEBAR_SECTIONS), 6)
        labels = [s["label"] for s in SIDEBAR_SECTIONS]
        self.assertIn("Trading", labels)
        self.assertIn("Kalshi Suite", labels)
        self.assertIn("Prediction Markets", labels)
        self.assertIn("Agents & Swarms", labels)
        self.assertIn("Risk & Analytics", labels)
        self.assertIn("System", labels)

    def test_kalshi_suite_items(self):
        from web.api.sidebar_config import SIDEBAR_SECTIONS
        kalshi = [s for s in SIDEBAR_SECTIONS if s["id"] == "kalshi-suite"][0]
        hrefs = [i["href"] for i in kalshi["items"]]
        self.assertIn("kalshi-dashboard", hrefs)
        self.assertIn("kalshi-grid", hrefs)
...
class TestSidebarWiring(unittest.TestCase):
    ...
    def test_main_includes_sidebar_router(self):
        source = (ROOT / "web" / "main.py").read_text(encoding="utf-8")
        self.assertIn("include_router(sidebar_config_router)", source)
...
class TestSidebarComponent(unittest.TestCase):
    ...
    def test_has_trading_section(self):
        self.assertIn("const trading", self.source)
        self.assertIn("'Trading'", self.source)

    def test_has_kalshi_suite_section(self):
        self.assertIn("const kalshiSuite", self.source)
        self.assertIn("'Kalshi Suite'", self.source)

    def test_has_prediction_markets_section(self):
        self.assertIn("const predictionMarkets", self.source)
        self.assertIn("'Prediction Markets'", self.source)

    def test_has_agents_swarms_section(self):
        self.assertIn("const agentsSwarms", self.source)
        self.assertIn("'Agents & Swarms'", self.source)

    def test_has_risk_analytics_section(self):
        self.assertIn("const riskAnalytics", self.source)
        self.assertIn("'Risk & Analytics'", self.source)
    ...
    def test_consolidated_sports(self):
        self.assertIn("Sports & Props", self.source)

    def test_consolidated_positions(self):
        self.assertIn("Positions & Orders", self.source)

    def test_consolidated_analytics(self):
        self.assertIn("Analytics & Backtests", self.source)
