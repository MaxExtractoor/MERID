...
def _clear_persisted_kill_switch():
    """Clear any persisted kill switch state before tests."""
    ks_file = Path(os.environ.get("MERID_RISK_KS_FILE", "data/risk_kill_switch.json"))
    if ks_file.exists():
        # Write an inactive state to prevent interference from previous runs
        ks_file.write_text(json.dumps({"active": False, "reason": None, "details": None}))
...
class TestDailyLossChaos:
    ...
    def test_recovery_after_reset(self, controller):
        ...
        # First trigger kill with a large loss
        controller.record_pnl(-150.0)
        assert controller.can_trade() is False
        assert controller._kill_reason == KillSwitchReason.DAILY_LOSS
        ...
        assert controller._global_kill is False
        assert controller._kill_reason is None
...
class TestCallbackChaos:
    ...
    def clear_ks_state(self):
        """Clear persisted kill switch state before each test."""
        _clear_persisted_kill_switch()
        yield
