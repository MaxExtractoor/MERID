    ...
    with respx.mock(assert_all_called=False) as mock:
        ...
        mock.get("https://api.elections.kalshi.com/trade-api/v2/markets").mock(
            return_value=Response(200, json={
                "markets": [
                    {
                        "ticker": "BTC-25DEC-T100000",
                        "event_ticker": "BTC-25DEC",
        ...
        # Market detail mock
        mock.get("https://api.elections.kalshi.com/trade-api/v2/markets/BTC-25DEC-T100000").mock(
            return_value=Response(200, json={
                "market": {
                    "ticker": "BTC-25DEC-T100000",
                    "yes_bid": 54,
...
async def test_kalshi_unified_adapter_integration(mock_kalshi_api):
    ...
    assert "BTC-25DEC-T100000" in symbols
    
    quote = await adapter.get_quote("BTC-25DEC-T100000")
...
async def test_agent_grid_archetypes_integration(mock_kalshi_api):
    ...
    if not any(s["action"] == "quote" for s in signals):
        print(f"Signals: {signals}")
        print(f"Agent active tickers: {agent.state.active_tickers}")
        
    assert any(s["action"] == "quote" for s in signals)
