...
def test_ui_summary_positions_structure(mock_dependencies):
    ...
    # Add mock position
    mock_dependencies["adapter"].get_positions = AsyncMock(return_value=[
        MagicMock(
            symbol="BTC-TEST",
            quantity=10,
            average_entry_price=55.0,
            unrealized_pnl=50.0,
        )
    ])
...
def test_ui_summary_graceful_adapter_failure(mock_dependencies):
    ...
    # Make adapter fail
    mock_dependencies["adapter"].get_positions = AsyncMock(
...
def test_ui_summary_graceful_reconciliation_failure(mock_dependencies):
    ...
    # Make reconciler fail
    mock_dependencies["reconciler"].reconcile = MagicMock(
