...
class TestTelegramLog:
    ...
    def test_filters_by_source(self, notif_client):
        ...
        assert "_stub" not in data
        assert data["total"] == 1
        assert data["messages"][0]["text"] == "Alert sent"
...
class TestTradeNotificationIntegration:
    ...
    def test_order_fill_emits_notification(self, notif_client):
        ...
        engine.fee_bps = dict(engine.DEFAULT_FEE_BPS)
        ...
        with patch("web.api.missing_endpoints.get_paper_engine", return_value=engine), \
             ...
             patch("trading.paper_trading._save_paper_state"), \
             patch("trading.paper_trading._get_risk_controller", return_value=None):
