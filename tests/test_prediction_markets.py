...
class TestKalshiStrategy:
    ...
    def test_no_action_low_volume(self):
        snap = self._make_snapshot(volume=Decimal("10"))
        sig = self.strategy.evaluate(snap)
        assert sig.action == SignalAction.NO_ACTION
        assert "Volume" in sig.reason

    def test_no_action_low_oi(self):
        snap = self._make_snapshot(oi=Decimal("1"))
        sig = self.strategy.evaluate(snap)
        assert sig.action == SignalAction.NO_ACTION
