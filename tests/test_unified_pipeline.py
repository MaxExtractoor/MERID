...
class TestModeManager:
    ...
    def test_check_can_trade_sim_blocked(self):
        cfg = [VenueConfig(venue="testv", domain="crypto", mode=TradingMode.MOCK, enabled=True)]
        mm = ModeManager(configs=cfg)
        with pytest.raises(ModeManager.ModeBlockedError, match="MOCK"):
...
class TestTradeRouter:
    ...
    async def test_mode_blocked(self):
        ...
        result = await router.submit(p)
        assert result.status == ProposalStatus.RISK_REJECTED
        reason = str(result.risk_check_result)
        assert "MOCK" in reason or "SIM" in reason
