...
class TestDomainAssessment:
    ...
    def test_crypto_has_instruments(self):
        # crypto domain was removed from DOMAIN_CONFIGS (Kalshi-only platform)
        domains = _assess_domains(rings_pass=True)
        names = [d.domain for d in domains]
        assert "crypto" not in names  # removed domain

    def test_equity_has_reconciliation(self):
        # equity domain was removed from DOMAIN_CONFIGS (Kalshi-only platform)
        domains = _assess_domains(rings_pass=True)
        names = [d.domain for d in domains]
        assert "equity" not in names  # removed domain
...
class TestReportIntegration:
    ...
    def test_generate_report_completes(self):
        ...
        # elapsed_s is 0 when MERID_PROMOTION_REPORT_FAST=true (fast-path skips ring execution)
    ...
    def test_report_has_domains(self):
        report = generate_promotion_report(gauntlet_cycles=5)
        assert len(report.domains) >= 1  # only "prediction" domain in Kalshi-only config
...
class TestPromotionChecklist:
    ...
    def test_checklist_has_domain_steps(self):
        ...
        assert len(domain_steps) >= 1  # only "prediction" domain in Kalshi-only config
