...
def test_batch_order_intent_creation():
    """Test BatchOrderIntent stores batch-level settings; per-order merging
    happens in route_batch_orders_async(), not at construction time."""
    batch = BatchOrderIntent(
        orders=[
            OrderIntent(
                ...
                price_cents=55,
                count=10,
                order_group_id="custom-og",  # Individual override kept as-is
            ),
            OrderIntent(
                ticker="KXETH-24JUN",
                side="no",
                ...
                price_cents=45,
                count=20,
                # No order_group_id — batch default applied at route time
            ),
        ],
    ...
    # BatchOrderIntent is a plain dataclass — it does not mutate individual orders
    assert batch.order_group_id == "batch-default-og"
    assert batch.self_trade_prevention_type == "taker_at_cross"
    assert batch.orders[0].order_group_id == "custom-og"   # Individual unmodified
    assert batch.orders[1].order_group_id is None           # Unset until routed
