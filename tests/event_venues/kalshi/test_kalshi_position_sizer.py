...
class TestFeeCalc:
    def test_small_order_fee(self):
        # 10 contracts at 55c → payout=45, rate=0.07, per=ceil(45*0.07)=4, min 2
        fee = kalshi_fee_cents(55, 10)
        assert fee == 40  # 4 * 10

    def test_medium_order_fee(self):
        # 200 contracts at 55c → rate=0.05, per=ceil(45*0.05)=3, min 2
        fee = kalshi_fee_cents(55, 200)
        assert fee == 600  # 3 * 200

    def test_large_order_fee(self):
        # 1500 contracts at 55c → rate=0.03, per=ceil(45*0.03)=2, min 2
        fee = kalshi_fee_cents(55, 1500)
        assert fee == 3000  # 2 * 1500
