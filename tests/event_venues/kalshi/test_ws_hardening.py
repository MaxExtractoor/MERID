...
from merid.event_venues.kalshi.ws import KalshiWebSocket, _AUTH_ERROR_CODES, _WARN_ERROR_CODES
...
class TestErrorMessageHandling:
    ...
    def test_fatal_codes_set(self):
        assert "auth_failed" in _AUTH_ERROR_CODES
        assert "invalid_token" in _AUTH_ERROR_CODES
...
class TestOrderbookCaching:
    ...
    def test_delta_without_snapshot_forwarded(self, ws):
...
class TestBackpressure:
    def test_queue_has_bounded_size(self, ws):
        assert ws._msg_queue.maxsize == 131072  # Increased from 65536 to handle burst traffic
