...
def _clear_kalshi_api_host(monkeypatch):
    """Prevent KALSHI_API_HOST from overriding test URLs (env + settings)."""
    monkeypatch.delenv("KALSHI_API_HOST", raising=False)
    try:
        from merid.settings import settings as _s
        monkeypatch.setattr(_s, "KALSHI_API_HOST", None)
    except Exception:
        pass
...
class Test429WithRetryAfterHeader:
    ...
    async def test_429_with_retry_after_waits_exact_duration(self):
        ...
        # Should wait ~5 seconds (Retry-After value) — allow headroom for CI/async overhead
        assert 4.5 <= elapsed <= 6.5, f"Expected ~5s wait, got {elapsed:.2f}s"
    ...
    async def test_multiple_sequential_429s_cumulative_backoff(self):
        ...
        assert 4.0 <= elapsed <= 7.0, f"Expected ~5s cumulative wait, got {elapsed:.2f}s"
        assert result.success
...
class Test429WithoutRetryAfterHeader:
    ...
    async def test_429_without_retry_after_uses_exponential_backoff(self):
        ...
        assert 0.8 <= elapsed <= 2.5, f"Expected ~1s backoff, got {elapsed:.2f}s"
        assert result.success
        assert route.call_count == 2
    ...
    async def test_exponential_backoff_sequence(self):
        ...
        # Should wait ~7 seconds total (1 + 2 + 4), but with jitter [1.0, 2.0) max is ~14s
        assert 6.0 <= elapsed <= 15.0, f"Expected ~7s cumulative (max ~14s with jitter), got {elapsed:.2f}s"
        assert result.success
...
class TestSelfThrottling:
    ...
    async def test_token_bucket_throttles_burst_requests(self):
        ...
        assert 0.3 <= elapsed <= 1.5, f"Expected ~0.5s for 30 requests (20 initial + 10 throttled), got {elapsed:.2f}s"
...
class TestRateLimitIntegration:
    ...
    async def test_client_uses_token_bucket_for_all_requests(self):
        ...
        client._ensure_async_network_resources()
        ...
        # Verify tokens were consumed (bucket should be near empty after burst)
        final_tokens = client._rate_limiter.read_tokens_available
        tokens_consumed = initial_tokens - final_tokens
        
        # Bucket starts with 20 tokens, 25 requests = at least 20 tokens consumed
        # Some tokens may refill during execution, but we should have consumed significantly
        assert tokens_consumed >= 15, f"Expected significant token consumption, consumed {tokens_consumed}"
        
        # Timing assertion is secondary - just verify not instant (would indicate no throttling)
        # With 25 requests and 20 token bucket, some throttling should occur
        assert elapsed >= 0.05, f"Should have some throttling, elapsed: {elapsed:.2f}s"
