...
def _isolated_deployment_state(tmp_path, monkeypatch):
    """Avoid loading repo data/deployment_state.json — tests need a clean controller."""
    monkeypatch.setenv("MERID_DEPLOYMENT_STATE", str(tmp_path / "deployment_state.json"))
...
def _relaxed_config(**overrides):
    defaults = dict(
        ...
        require_backtest_lift_gate=False,
    )
    defaults.update(overrides)
    return DeploymentConfig(**defaults)


def _shadow_then_live(ctrl: DeploymentController, agent_name: str) -> None:
    assert ctrl.promote_to_shadow(agent_name)[0] is True
    assert ctrl.promote_to_live(agent_name)[0] is True
...
class TestPromoteToLive:
    def test_promote_paper_to_live(self):
        ctrl = DeploymentController(_relaxed_config())
        ctrl.register_agent("BTC_HOURLY")
        assert ctrl.promote_to_shadow("BTC_HOURLY")[0] is True
    ...
    def test_already_live_rejected(self):
        ctrl = DeploymentController(_relaxed_config())
        ctrl.register_agent("BTC_HOURLY")
        _shadow_then_live(ctrl, "BTC_HOURLY")
        ok, reason = ctrl.promote_to_live("BTC_HOURLY")
        assert ok is False
