...
from merid.risk import kill_switches as _kill_mod
...
def _order_manager_allow_submit(monkeypatch):
    """Submit path checks global kill switch and venue gate; keep unit tests deterministic."""
    monkeypatch.setattr(_kill_mod.risk_controller, "can_trade", lambda: True)
    try:
        from merid.prediction import venue_gate as _vg

        _gate = MagicMock()
        _gate.should_simulate_fill = MagicMock(return_value=False)
        _gate.mode = MagicMock(value="live")
        monkeypatch.setattr(_vg, "get_venue_gate", lambda: _gate)
    except ImportError:
        pass
