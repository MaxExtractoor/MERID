...
def _ensure_paper_trade_mode() -> None:
    """unittest classes do not always receive pytest autouse fixtures; force PAPER.

    ExecutionGuard promotion checks only run when ``is_live``; stale LIVE mode
    from earlier tests would otherwise mask CQI / cooldown assertions.

    Reset + ``set_trade_mode(PAPER)`` is safe: ``trading.trade_mode`` uses an
    ``RLock`` so ``get_trade_mode()`` can initialize while ``set_trade_mode``
    holds the lock (needed after ``_reset_for_tests()``).

    Also disarms ``risk_controller`` persistence / circuit-breaker latch so
    ``pre_trade_check`` is not blocked by operator-machine state under
    ``data/risk_kill_switch.json``.
    """
    from merid.risk.kill_switches import risk_controller
    from trading.trade_mode import TradeMode, _reset_for_tests, set_trade_mode

    risk_controller.reset("test_hardening")
    _reset_for_tests()
    set_trade_mode(TradeMode.PAPER, reason="test_hardening")
...
class TestWalletBalancesEndpoint(unittest.TestCase):
    ...
    def test_guard_caps_add_informational_transactions(self):
        ...
        from merid.execution_guard import ExecutionGuard, DomainCap
        guard = ExecutionGuard()
        guard.deactivate_kill_switch()
        # Ensure crypto domain cap exists (guard only has 'prediction' by default)
        guard._domain_caps["crypto"] = DomainCap(
            domain="crypto", max_daily_notional_usd=10000, max_single_trade_usd=1000
        )
        guard.update_cqi("crypto", 0.8)
        # Pre-populate promotion eligible domains to bypass promotion ring enforcement
        # This test is focused on cap tracking, not promotion semantics
