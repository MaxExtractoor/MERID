...
sys.modules['hardening'] = MagicMock()
...
class TestMarketsEndpoint:
    ...
    def test_get_markets_with_kalshi_data(self, client, mock_prediction_aggregator):
        ...
        with patch('web.api.predictions.get_prediction_market_aggregator', return_value=mock_prediction_aggregator):
            # Trigger a refresh
            import asyncio
...
class TestKalshiIntegration:
    ...
    async def test_fetch_kalshi_markets_fallback_on_error(self):
        ...
        with patch('web.api.predictions.get_prediction_market_aggregator', side_effect=Exception("Connection failed")):
            await fetch_kalshi_markets()
            
            # Should fall back to mock data
            assert len(_markets_cache) > 0
            # Should contain mock market IDs
            market_ids = [m["id"] for m in _markets_cache]
            assert "bitcoin-price-2024" in market_ids or len(_markets_cache) > 0
