...
class TestWebSocketEndpoints:
    ...
    def test_ws_trades_endpoint_exists(self, mock_dependencies):
        """Test that /ws/trades endpoint exists and accepts connections."""
        from web.api.ws_dedicated_streams import router
        
        # Verify the router has the trades endpoint
        routes = [route.path for route in router.routes]
        assert "/ws/trades" in routes or "/trades" in routes
    
    def test_ws_prices_endpoint_exists(self, mock_dependencies):
        """Test that /ws/prices endpoint exists and accepts connections."""
        from web.api.ws_dedicated_streams import router
        
        # Verify the router has the prices endpoint
        routes = [route.path for route in router.routes]
        assert "/ws/prices" in routes or "/prices" in routes
