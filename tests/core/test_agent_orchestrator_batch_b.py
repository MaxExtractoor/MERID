...
class TestAgentOrchestratorLifecycle:
    ...
    async def test_start_creates_background_tasks(self, orchestrator, mocker):
        """Test start sets running=True and runs orchestration loop.

        news_monitor.start_monitoring and price_feed.start_streaming are intentionally
        NOT called inside start() — they are started by OrchestratorAgentManager and
        main.py lifespan respectively to avoid double-start.
        """
        ...
        orchestrator._orchestration_loop.assert_called_once()
