...
class TestFreshStartPaperResetPreservesKillSwitch(unittest.TestCase):
    ...
    def test_fresh_start_zeroes_paper_but_preserves_kill_switch(self):
        ...
        # ── Arrange: trigger the kill switch ──
...
class TestExecutionBlockedUntilFirstReconciliation(unittest.TestCase):
    """On fresh start, has_critical_discrepancies() returns True until recon runs."""

    def test_execution_blocked_until_first_reconciliation_on_fresh_start(self):
        import trading.reconciliation as recon_mod

        # Save originals
        orig_completed = recon_mod._has_ever_completed
        orig_report = recon_mod._last_report

        try:
            # Simulate fresh-start state: never completed, no report
            recon_mod._has_ever_completed = False
            recon_mod._last_report = None

            with mock.patch("core.fresh_start.is_fresh_start", return_value=True):
                # Before first reconciliation → blocked
                self.assertTrue(
                    recon_mod.has_critical_discrepancies(),
                    "Execution must be blocked before first reconciliation on fresh start",
                )

            # Simulate first reconciliation completing OK
            recon_mod._has_ever_completed = True
            recon_mod._last_report = recon_mod.ReconciliationReport(all_ok=True)

            with mock.patch("core.fresh_start.is_fresh_start", return_value=True):
                # After first reconciliation → unblocked
                self.assertFalse(
                    recon_mod.has_critical_discrepancies(),
                    "Execution must be unblocked after first clean reconciliation",
                )

            # Simulate a reconciliation with deltas
            recon_mod._last_report = recon_mod.ReconciliationReport(all_ok=False)

            with mock.patch("core.fresh_start.is_fresh_start", return_value=True):
                self.assertTrue(
                    recon_mod.has_critical_discrepancies(),
                    "Execution must be blocked when reconciliation has deltas",
                )
        finally:
            recon_mod._has_ever_completed = orig_completed
            recon_mod._last_report = orig_report
