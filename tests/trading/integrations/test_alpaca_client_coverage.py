...
class TestGetAlpacaClient:
    ...
    def test_kalshi_only_returns_none(self, monkeypatch):
        monkeypatch.setattr(merid_settings, "KALSHI_ONLY", True, raising=False)
        get_alpaca_client.cache_clear()
        assert get_alpaca_client() is None
        assert fetch_account_snapshot() == {}
