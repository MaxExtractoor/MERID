...
class TestSetMode:
    ...
    def test_set_mode_with_reason(self):
        controller = TradingModeController()
        result = controller.set_mode("hybrid", changed_by="admin", reason="Testing")
        assert result is True
        assert controller.mode == TradingMode.HYBRID
...
class TestCanExecuteLive:
    ...
    def test_autonomous_symbol_not_allowed(self):
        controller = TradingModeController()
        controller.set_mode("autonomous")
        can_execute, reason = controller.can_execute_live("DOGE/USDT", 100.0)
        assert can_execute is False
    ...
    def test_autonomous_daily_trade_limit(self):
        controller = TradingModeController()
        controller.set_mode("autonomous")
        controller._daily_stats["trades"] = 50
        can_execute, reason = controller.can_execute_live("BTC/USDT", 100.0)
        assert can_execute is False
    ...
    def test_autonomous_daily_volume_limit(self):
        controller = TradingModeController()
        controller.set_mode("autonomous")
        controller._daily_stats["volume_usd"] = 9900.0
    ...
    def test_autonomous_resets_daily_stats(self):
        controller = TradingModeController()
        controller.set_mode("autonomous")
        controller._daily_stats["last_reset"] = time.time() - 90000  # > 24h ago
        controller._daily_stats["trades"] = 100
        
        can_execute, reason = controller.can_execute_live("BTC/USDT", 100.0)
        
        assert controller._daily_stats["trades"] == 0
