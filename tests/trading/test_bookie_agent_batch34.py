...
class TestBettingPool:
    ...
    def sample_pool(self):
        return BettingPool(market_id="block:100", house_cut_pct=5.0)
...
class TestBookieAgent:
    ...
    def test_place_bet_success(self, agent):
        ...
        bet = agent.place_bet("user_1", block_index=100, prediction="approved", stake_amount=100.0)
    ...
    def test_place_bet_invalid_amount(self, agent):
        ...
        with pytest.raises(ValueError, match="Stake amount must be"):
            agent.place_bet("user_1", block_index=100, prediction="approved", stake_amount=0.5)  # Below minimum

    def test_place_bet_no_pool(self, agent):
        with pytest.raises(ValueError, match="No betting pool exists"):
            agent.place_bet("user_1", block_index=100, prediction="approved", stake_amount=100.0)
    ...
    def test_settle_pool_high_confidence(self, agent):
        agent.deposit("user_1", 1000.0)
        agent.create_betting_pool(100)
        agent.place_bet("user_1", block_index=100, prediction="high_confidence", stake_amount=100.0)
        agent.lock_pool_for_mining(100)
    ...
    def test_settle_pool_rejected(self, agent):
        agent.deposit("user_1", 1000.0)
        agent.create_betting_pool(100)
        agent.place_bet("user_1", block_index=100, prediction="rejected", stake_amount=100.0)
        agent.lock_pool_for_mining(100)
    ...
    def test_settle_pool_low_confidence(self, agent):
        agent.deposit("user_1", 1000.0)
        agent.create_betting_pool(100)
        agent.place_bet("user_1", block_index=100, prediction="low_confidence", stake_amount=100.0)
        agent.lock_pool_for_mining(100)
    ...
    def test_reward_agents(self, agent):
        ...
        agent.reward_agents(block_index=100, agent_votes=agent_votes, reward_pool_pct=10.0)
    ...
    def test_reward_agents_pool_not_settled(self, agent):
        ...
        agent.reward_agents(block_index=100, agent_votes=agent_votes)  # Should not raise, just log warning
        assert agent.agent_rewards == {}

    def test_reward_agents_no_pool(self, agent):
        agent_votes = {"agent_1": True}
        agent.reward_agents(block_index=100, agent_votes=agent_votes)  # Should return early
        assert agent.agent_rewards == {}
    ...
    def test_get_pool_stats_existing(self, agent):
        ...
        stats = agent.get_pool_stats(block_index=100)
    ...
    def test_calculate_odds_empty_pool(self, agent):
        pool = BettingPool(market_id="block:100")
