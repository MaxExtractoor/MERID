...
class TestEntryPriceBandFix:
    ...
    def test_price_range_in_profile(self):
        ...
        # Check max_price_cents is 75 (aligned with DEEP_OTM_EXPENSIVE_CENTS)
    ...
    def test_price_range_wider_than_previous(self):
        """Test that price_range is 10-75c (wider minimum, aligned maximum)."""
        with open("config/profiles/kalshi_crypto_15m_v2.yaml", "r", encoding="utf-8") as f:
            profile = yaml.safe_load(f)
        
        price_range = profile["price_range"]
        
        # Range should be 10-75c (aligned with DEEP_OTM_EXPENSIVE_CENTS)
        assert price_range["min_price_cents"] == 10, \
    ...
    def test_agent_grid_uses_profile_price_range(self):
        ...
        # Check that code has fallback to 10-75c (updated to 75c max)
        ...
        assert "ENTRY_MAX_PRICE_CENTS = 75" in code or "ENTRY_MAX_PRICE_CENTS =  75" in code, \
            "agent_grid_15m.py should have fallback to 75c maximum"
