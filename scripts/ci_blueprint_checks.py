...
def check_paper_config() -> bool:
    """Check 1: paper_config loads, prediction domain active, risk config present."""
    try:
        from merid.paper_config import get_paper_config, DOMAIN_CONFIGS
        cfg = get_paper_config()
        domains = cfg.active_domain_names()

        if len(domains) < 1:
            return _fail("paper_config", "No active domains configured")
        if "prediction" not in domains:
            return _fail("paper_config", "prediction domain missing or disabled")

        # Prediction domain must have a matching engine
        pred = DOMAIN_CONFIGS.get("prediction")
        if pred is None or pred.matching_engine is None:
            return _fail("paper_config", "prediction domain has no matching engine")
        if not pred.matching_engine.enabled:
            return _fail("paper_config", "prediction matching engine is disabled")
        ...
        return _ok("paper_config", f"{len(domains)} domain(s): {domains}")
