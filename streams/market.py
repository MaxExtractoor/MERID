...
class MarketStreamConfig(StreamConfig):
    ...
    @classmethod
    def from_asset_universe(cls, **kwargs) -> "MarketStreamConfig":
        """Build config with swarm-eligible symbols from the top-50 asset universe."""
        from data.asset_universe import get_swarm_eligible
        symbols = [a.symbol for a in get_swarm_eligible()]
...
class MarketStream(BaseStream):
    ...
    async def _fetch_data(self) -> Optional[object]:
        ...
        """
        if (self._ticker_task is None or self._ticker_task.done()) and self._market_config.enable_tickers:
            self._ticker_task = asyncio.create_task(self._ticker_loop())
        
        if (self._funding_task is None or self._funding_task.done()) and self._market_config.enable_funding:
...
_market_stream_lock = threading.Lock()
