...
class KalshiAPIRobustnessLayer:
    ...
    def _increment_request_count(self, endpoint_name: str) -> None:
        ...
        # E4b: get-mutate-set must be atomic; ThreadSafeState.get returns a
        # reference to the stored dict so mutating then re-setting is safe.
        counts = self._state.get("request_counts", {})
        counts[endpoint_name] = counts.get(endpoint_name, 0) + 1
        self._state.set("request_counts", counts)
    ...
    def _increment_error_count(self, endpoint_name: str) -> None:
        ...
        counts[endpoint_name] = counts.get(endpoint_name, 0) + 1
        self._state.set("error_counts", counts)
    ...
    def _record_health(
        ...
        health = self._state.get("endpoint_health", {})  # noqa: E4b
        ...
        self._state.set("endpoint_health", health)  # E4b: write mutated dict back
...
_kalshi_api_robustness_lock = threading.Lock()
...
def get_kalshi_api_robustness() -> KalshiAPIRobustnessLayer:
    ...
    if _kalshi_api_robustness is None:  # E4b: double-checked locking
        with _kalshi_api_robustness_lock:
