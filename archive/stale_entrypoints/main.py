...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _active_tickers = []
        try:
            _active_tickers = [m.market.market_id for m in _get_cat().get_all_markets()[:50]]
        except Exception as _cat_err:
            logger.debug("WS bridge ticker bootstrap from catalog failed (will subscribe later): %s", _cat_err)
        asyncio.create_task(ws_bridge.start(_active_tickers or None))
        logger.info("✅ Kalshi WS bridge started (%d tickers)", len(_active_tickers))
    except Exception as e:
