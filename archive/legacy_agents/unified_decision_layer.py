...
class DecisionAggregator:
    ...
    def aggregate(
        ...
        # T-010: Enforce minimum quorum — no single-agent decisions
        MIN_QUORUM = int(os.environ.get("MERID_MIN_CONSENSUS_QUORUM", "3"))
        if len(contributions) < MIN_QUORUM:
            logger.warning(
                "Insufficient quorum: %d/%d agents contributed. Returning NO_ACTION.",
                len(contributions), MIN_QUORUM,
            )
            return UnifiedDecision(
                decision_id=f"quorum_fail_{int(time.time())}",
                ...
                reasoning_summary=f"Quorum not met: {len(contributions)}/{MIN_QUORUM} agents",
                timestamp=time.time(),
                metadata=context,
            )
