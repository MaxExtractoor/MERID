...
class Phase6CombinedExecutor:
    ...
    def execute_tasks(self):
        ...
        for idx, task in enumerate(tasks, 1):
            ...
            # Build notes safely
            latency_str = f"{latency_improvement*100:.1f}" if latency_improvement is not None else "N/A"
            reliability_str = f"{reliability_improvement*100:.1f}" if reliability_improvement is not None else "N/A"
            strategy_str = f"{strategy_accuracy*100:.1f}" if strategy_accuracy is not None else "N/A"
            ...
            roi_calculations = {
                ...
                "notes": (
                    ...
                    f"strategy_accuracy={strategy_str}% | "
                    f"latency_improvement={latency_str}% | "
                    f"reliability_improvement={reliability_str}%"
                ),
