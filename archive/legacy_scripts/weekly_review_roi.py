#!/usr/bin/env python3
"""
Weekly Review ROI Wrapper
Run core ROI queries and emit markdown summary
"""
...
logger = get_logger("swarm.weekly_review_roi")

class WeeklyReviewROI:
    """Weekly review ROI analysis and reporting"""
    ...
    def generate_markdown_report(self, summary: Dict[str, Any]) -> str:
        ...
        if overview["total_hours_saved"] >= 10:
            ...
            status_indicators.append("⚠️ Volume below target")
        
        if cost["avg_roi_score"] >= 85:
            status_indicators.append("✅ ROI healthy")
        else:
            status_indicators.append("⚠️ ROI needs improvement")
...
def run_weekly_review(weeks_back: int = 1, output_file: str = "weekly_roi_review.md"):
    """Run weekly review and save markdown report"""
    print("=" * 70)
    print("WEEKLY ROI REVIEW")
    print("=" * 70)
    ...
    try:
        # Generate summary
        reviewer = WeeklyReviewROI()
        summary = reviewer.run_weekly_summary(weeks_back)
        
        # Generate markdown report
        markdown = reviewer.generate_markdown_report(summary)
