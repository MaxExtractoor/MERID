...
This module contains direct HTTP API calls that bypass the canonical order router
and risk guard pipeline. It has been hardened to prevent accidental activation.

If you need maker bot functionality, use: KalshiContinuousTrader with Strategy.MARKET_MAKER
See: maker_bot_advanced.py.DISABLED for details.
"""

# PRODUCTION HARDENING: Runtime guard against accidental activation
import os

# Allow bypass only in explicit dev/test environments with override
_ALLOW_MAKER_BOT_ADVANCED = os.getenv("MERID_ALLOW_MAKER_BOT_ADVANCED", "").lower() in ("1", "true", "yes")

if not _ALLOW_MAKER_BOT_ADVANCED:
    raise RuntimeError(
        "[PRODUCTION HARDENING] maker_bot_advanced is DISABLED. "
        "This module contains direct HTTP bypasses to Kalshi API that circumvent "
        "the canonical order router (route_order_async), Top-3 batch gate, "
        "GlobalRiskGuard (1-2% cap), PreTradeGate (deduplication), and lease registry. "
        "To use maker functionality, use KalshiContinuousTrader with Strategy.MARKET_MAKER "
        "which routes through the hardened order_router. "
        "If you absolutely need this module (not recommended), set MERID_ALLOW_MAKER_BOT_ADVANCED=1 "
        "and understand you are bypassing production risk controls."
    )
...
class KalshiAutoReplaceEngine:
    ...
    def submit_order_from_decision(self, ticker: str, decision: ExecutionDecision) -> Optional[str]:
        ...
        try:
            ...
            # CRITICAL: GlobalExecutionGuard check — prevents 7% allocation bypass
            _guard = get_global_execution_guard()
            _allowed, _reason = _guard.check_order(
                ticker=ticker,
                contracts=int(decision.size),
                price_cents=int(decision.suggested_price * 100),
                source="maker_bot_advanced",
                asset=None,
            )
            if not _allowed:
                logger.error(
                    "[MAKER_BOT_BLOCKED] GlobalExecutionGuard rejected: %s | ticker=%s",
                    _reason, ticker
                )
                return None
