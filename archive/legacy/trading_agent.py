...
class KalshiTradingAgent:
    ...
    async def run_cycle(self, tick: int) -> None:
        """
        Run one decision cycle (called by 15m loop).
        
        This is a public method for external cycle control (e.g., by AgentGrid.run_cycle).
        It delegates to the internal _run_cycle method.
        """
    ...
    async def _run_cycle_body(
        ...
        for market in active_markets:
            ...
            try:
                ...
                # Defensive: _get_mood_context may not exist in all agent implementations
                mood_context = None
                if hasattr(self, '_get_mood_context'):
                    try:
    ...
    async def _resolve_markets(self) -> None:
        ...
        try:
            ...
            self.logger.debug("[RESOLVE-DEBUG] agent=%s series_tickers=%s", self.config.name, _series)
            if _series:
                from merid.event_venues.kalshi.market_catalog import get_market_catalog
                catalog = get_market_catalog()
                catalog_size = len(catalog.get_all_markets())
                self.logger.debug("[RESOLVE-DEBUG] agent=%s catalog_size=%d before refresh", self.config.name, catalog_size)
                ...
                for cm in catalog.get_all_markets():
                    ...
                    # Debug: log first few markets to see what we're matching against
                    if len(all_markets) < 3:
                        self.logger.debug(
                            "[RESOLVE-DEBUG] market=%s mkt_series=%s mkt_event=%s mkt_id=%s",
                            cm.market.market_id, mkt_series, mkt_event, mkt_id
                        )
    ...
    def _build_snapshot(self, market: EventMarket, now: datetime) -> MarketSnapshot:
        ...
        # INFO: Log asset resolution for spot cache mismatch diagnosis
        self.logger.info(
            "[SPOT-CACHE-DEBUG] market_id=%s resolved_asset=%s _asset_for_spot=%s",
            market.market_id, _resolved_asset, _asset_for_spot
        )
        strike = None
        ...
        if is_crypto_15m:
            try:
                ...
                if self._btc15m_risk is None:
                    ...
                    # No fallback to paper balance - unified bankroll must be available
                    if _init_equity <= 0:
                        self.logger.error("[crypto15m_risk] Unified bankroll unavailable - cannot initialize without live Kalshi bankroll")
                        raise ValueError("Live bankroll required for initialization - no fallback to paper balance")
                ...
                if _cur_equity <= 0:
                    self.logger.error("[crypto15m_risk] Unified bankroll unavailable for update - cannot proceed without live Kalshi bankroll")
                    raise ValueError("Live bankroll required for update - no fallback to paper balance")
                try:
