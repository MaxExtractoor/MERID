...
class SignalStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        _default_db = os.path.join(_project_root, "data", "signals.db")
        self._db_path = db_path or os.environ.get("MERID_SIGNAL_DB", _default_db)
    ...
    def _init_db(self):
        ...
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS arb_plans (
                id TEXT PRIMARY KEY,
                signal_id TEXT REFERENCES arb_signals(id) ON DELETE SET NULL,
                domain TEXT NOT NULL,
                arb_type TEXT NOT NULL,
                symbol TEXT NOT NULL,
    ...
    def store_signal(self, signal_dict: Dict[str, Any]):
        """Store a Kalshi trading signal (edge, liquidity, volume, risk)."""
        conn = self._conn()
        try:
            conn.execute(
                """INSERT OR REPLACE INTO signal_features
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    signal_dict.get("signal_id", f"sig-{time.time()}"),
                    signal_dict.get("ticker", signal_dict.get("symbol", "")),
                    signal_dict.get("signal_type", "kalshi"),
                    json.dumps(signal_dict),
                    signal_dict.get("source", "kalshi_signals"),
                    1.0,
                    time.time(),
                    time.time(),
                ),
    ...
    def store_feature_snapshots_batch(self, feature_dicts: List[Dict[str, Any]]):
        """Batch-insert feature snapshots in a single transaction (much faster)."""
        ...
        try:
            ...
            rows = [
                (
                    f"sf-{d.get('symbol','')}-{d.get('domain','')}",
                    d.get("symbol", ""),
                    d.get("domain", ""),
                    json.dumps(d.get("features", {})),
                    d.get("source", "synthetic"),
                    d.get("avg_freshness", 1.0),
                    now,
                    now,
                )
    ...
    def store_arb_signals_batch(self, signal_dicts: List[Dict[str, Any]]) -> None:
        """Batch-insert arb signals in a single transaction (avoids per-signal open/commit/close)."""
        if not signal_dicts:
            return
        conn = self._conn()
        try:
            rows = [
                (
                    ...
                    sd.get("arb_type", ""),
                    sd.get("symbol", ""),
                    json.dumps(sd.get("venues", [])),
                    sd.get("gross_edge_bps", 0),
                    sd.get("net_edge_bps", 0),
                    sd.get("edge_usd", 0),
                    sd.get("ttl_seconds", 120),
                    sd.get("status", "active"),
                    sd.get("detected_at", time.time()),
                )
