...
def get_dynamic_beta(
    ...
    Returns:
        Beta value (float). Returns 1.0 if insufficient data.
    ...
    try:
        ...
        if beta_estimate.sample_count >= min_observations:
            logger.debug(
                "[DYNAMIC-BETA] %s/%s: beta=%.2f (n=%d, r2=%.2f)",
                asset, timeframe, beta_estimate.beta, beta_estimate.sample_count, beta_estimate.r_squared
            )
            return beta_estimate.beta

        # Not enough data, return 1.0 (no stack fallback)
        logger.debug(
            "[DYNAMIC-BETA] %s/%s: insufficient data (n=%d), returning 1.0",
            asset, timeframe, beta_estimate.sample_count
        )
        return 1.0
