...
# This module is archived and superseded by LeanAgentGrid15m in agent_grid_15m.py.
...
if is_kalshi_crypto_15m_v2():
    raise RuntimeError(
        ...
        "Use merid.prediction.agent_grid_15m.LeanAgentGrid15m instead. "
        f"Module: {__name__}"
    )
...
# ═══════════════════════════════════════════════════════════════════════
# LEAN 15m KALSHI STACK: Gate legacy subsystems under profile flag
# For kalshi_crypto_15m_v2, disable reflection, paper session, and social broadcaster
# to prevent synchronous I/O and background threads from blocking the event loop
# ═══════════════════════════════════════════════════════════════════════
PROFILE = os.getenv("MERID_PROFILE", "").lower()
LEAN_15M_PROFILE = PROFILE == "kalshi_crypto_15m_v2"
ENABLE_REFLECTION = not LEAN_15M_PROFILE
ENABLE_PAPER_SESSION = not LEAN_15M_PROFILE
ENABLE_SOCIAL_BROADCASTER = not LEAN_15M_PROFILE
ENABLE_ALERT_MANAGER = not LEAN_15M_PROFILE
ENABLE_AUTO_PROMOTER = not LEAN_15M_PROFILE

# Debug: log profile detection
print(f"[AGENT-GRID-PROFILE-DEBUG] PROFILE={PROFILE} LEAN_15M_PROFILE={LEAN_15M_PROFILE}", file=sys.stderr)
...
class AgentGrid:
    ...
    def __init__(self, config: Optional[AgentGridConfig] = None):
        # Profile guard: Prevent legacy AgentGrid from being instantiated for kalshi_crypto_15m_v2
        if LEAN_15M_PROFILE:
            raise RuntimeError(
                "Legacy AgentGrid must not be used for MERID_PROFILE=kalshi_crypto_15m_v2. "
                "Use merid.prediction.agent_grid_15m.LeanAgentGrid15m instead to ensure zero legacy dependencies."
            )
        ...
        # Social broadcaster (log-only event consumer) - DISABLED for 15m profile
        if ENABLE_SOCIAL_BROADCASTER:
            self._broadcaster = get_social_broadcaster()
            logger.info("[AGENT-GRID] KalshiSocialBroadcaster enabled")
        else:
            self._broadcaster = None
            logger.info("[AGENT-GRID] SocialBroadcaster DISABLED for profile kalshi_crypto_15m_v2")

        # Paper session tracker - DISABLED for 15m profile
        if ENABLE_PAPER_SESSION:
            self._paper_session = get_paper_session()
            logger.info("[AGENT-GRID] PaperSession enabled")
        else:
            self._paper_session = None
            logger.info("[AGENT-GRID] PaperSession DISABLED for 15m profile")
        ...
        # Alert Manager with Twitter/Telegram sinks - DISABLED for 15m profile
        if ENABLE_ALERT_MANAGER:
            from merid.prediction.alerts import get_alert_manager
            self._alert_manager = get_alert_manager()
            self._register_alert_sinks()
            logger.info("[AGENT-GRID] AlertManager enabled")
        else:
            self._alert_manager = None
            logger.info("[AGENT-GRID] AlertManager DISABLED for 15m profile")

        # AutoPromoter — promotion pipeline with operator confirmation - DISABLED for 15m profile
        if ENABLE_AUTO_PROMOTER:
            ...
            logger.info("[AGENT-GRID] AutoPromoter enabled")
        else:
            self._auto_promoter = None
            logger.info("[AGENT-GRID] AutoPromoter DISABLED for 15m profile")
        ...
        # Log lean-only invariant for 15m profile
        logger.info(
            "[AGENT-GRID] 15m profile lean invariant: reflection=%s paper=%s social=%s alerts=%s promoter=%s",
            ENABLE_REFLECTION,
            ENABLE_PAPER_SESSION,
            ENABLE_SOCIAL_BROADCASTER,
            ENABLE_ALERT_MANAGER,
            ENABLE_AUTO_PROMOTER,
        )
    ...
    async def start(self) -> None:
        ...
        async with self._start_lock:
            ...
            try:
                ...
                # SKIP PM RUNTIME FOR 15m PROFILE
                if LEAN_15M_PROFILE:
                    logger.info("[AGENT-GRID] PM runtime preconditions SKIPPED for kalshi_crypto_15m_v2 (lean 15m stack)")
                else:
    ...
    async def _start_grid_services(self) -> None:
        ...
        # L6: Register all agents with DeploymentController (starts each in PAPER mode) - DISABLED for 15m profile
        if not LEAN_15M_PROFILE:
            try:
            ...
            logger.info("[AGENT-GRID] DeploymentController registration SKIPPED for kalshi_crypto_15m_v2 (lean 15m stack)")
        ...
        # LEAN 15m KALSHI STACK: Pre-initialize signal model singletons at startup
        # This prevents lazy initialization during the first cycle which can cause event-loop lag
        if LEAN_15M_PROFILE:
            logger.info("[AGENT-GRID] Pre-initializing signal model singletons for kalshi_crypto_15m_v2...")
            try:
                ...
                _ = get_momentum_ranker()
                logger.info("  ✓ CrossSectionalMomentumRanker singleton initialized")
                
                _ = get_unified_regime_classifier()
                logger.info("  ✓ UnifiedRegimeClassifier singleton initialized")
                
                _ = get_btc_anchor_gate()
                logger.info("  ✓ BtcAnchorGate singleton initialized")
                
                _ = get_btc_anchored_model()
                logger.info("  ✓ BtcAnchoredMoveModel singleton initialized")
                
                logger.info("[AGENT-GRID] All signal model singletons pre-initialized at startup")
            except Exception as _singleton_exc:
                logger.warning("[AGENT-GRID] Failed to pre-initialize signal model singletons: %s", _singleton_exc)
        ...
        # Start social broadcaster - DISABLED for 15m profile
        if ENABLE_SOCIAL_BROADCASTER and self._broadcaster:
            ...
            logger.info("[AGENT-GRID] SocialBroadcaster start() skipped for 15m profile")

        # Auto-start paper session for PnL tracking - DISABLED for 15m profile
        if ENABLE_PAPER_SESSION and self._paper_session:
            ...
            logger.info("[AGENT-GRID] PaperSession start() skipped for 15m profile")

        # Ensure ReflectionSystem is initialized and persistence is running
        # before agents start producing fills (idempotent — singleton pattern) - DISABLED for 15m profile
        if ENABLE_REFLECTION:
            ...
            logger.info("[AGENT-GRID] ReflectionSystem init skipped for 15m profile")
        ...
        # Start reconciliation loop (auto-fix critical discrepancies) - DISABLED for 15m profile
        if ENABLE_REFLECTION and not LEAN_15M_PROFILE:
            ...
            logger.info("[AGENT-GRID] Reconciliation loop SKIPPED for kalshi_crypto_15m_v2 (lean 15m stack)")
    ...
    async def stop(self) -> None:
        ...
        # Stop social broadcaster - DISABLED for 15m profile
        if ENABLE_SOCIAL_BROADCASTER and self._broadcaster:
            await self._broadcaster.stop()
        else:
            logger.debug("[AGENT-GRID] SocialBroadcaster stop() skipped for 15m profile")

        # Stop sentiment service - REMOVED for 15m stack
        if self._sentiment:
            await self._sentiment.stop()

        # Stop market mood bus - REMOVED for 15m stack
        if self._mood_bus:
            await self._mood_bus.stop()

        # Stop insight pipeline - REMOVED for 15m stack
        if self._insight_pipeline:
            await self._insight_pipeline.stop()

        # Flush ReflectionSystem persistence buffer on shutdown - DISABLED for 15m profile
        if ENABLE_REFLECTION:
            try:
                from agents.reflection.integration import get_reflection_system
            ...
            logger.debug("[AGENT-GRID] ReflectionSystem shutdown skipped for 15m profile")
    ...
    async def run_cycle(self, tick: int) -> None:
        """
        Run one trading cycle across all agents.
        
        This is called by the 15m loop to step all trading agents.
        Each agent's run_cycle method generates signals and places orders.
        """
        logger.info("[AGENT-GRID] ENTER run_cycle tick=%d n_agents=%d", tick, len(self._agents))
        
        # Run agents in parallel to avoid blocking if one agent takes a long time
        tasks = []
        for agent in self._agents:
            if hasattr(agent, 'run_cycle'):
                logger.info("[AGENT-GRID] Scheduling agent.run_cycle tick=%d agent=%s", tick, agent.config.name)
                tasks.append(self._run_agent_cycle(agent, tick))
            else:
                logger.warning("[AGENT-GRID] Agent %s has no run_cycle method", agent.config.name)
        
        if tasks:
            logger.info("[AGENT-GRID] Waiting for %d agent cycles to complete", len(tasks))
            await asyncio.gather(*tasks, return_exceptions=True)
            logger.info("[AGENT-GRID] All agent cycles completed")
        
        logger.info("[AGENT-GRID] EXIT run_cycle tick=%d", tick)
    
    async def _run_agent_cycle(self, agent: Any, tick: int) -> None:
        """Run a single agent cycle with error handling."""
        try:
            logger.info("[AGENT-GRID] ENTER agent.run_cycle tick=%d agent=%s", tick, agent.config.name)
            result = await agent.run_cycle(tick)
            logger.info("[AGENT-GRID] EXIT agent.run_cycle tick=%d agent=%s result=%s", tick, agent.config.name, result)
        except Exception as exc:
            logger.error(
                "[AGENT-GRID] Agent %s failed in cycle %d: %s",
                agent.config.name,
                tick,
                exc,
                exc_info=True,
