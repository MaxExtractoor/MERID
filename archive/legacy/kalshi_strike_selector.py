...
class KalshiStrikeSelector:
    ...
    def evaluate(
        ...
        if not asset_in_ticker(ticker, asset_upper):
            logger.warning(
                "[STRIKE_SELECTOR_ASSET_MISMATCH] ticker=%s asset=%s tf=%s spot=%s strike=%s - asset validation failed",
        ...
        if spot is None or spot <= 0:
            logger.warning(
                "[STRIKE_SELECTOR_MISSING_SPOT] ticker=%s asset=%s tf=%s spot=%s strike=%s - missing or invalid spot",
                ticker, asset_upper, tf_lower, spot, strike
            )
            return self._reject(ticker, asset_upper, tf_lower, spot, strike,
        ...
        if strike is None:
            if self._config.allow_directional_passthrough:
                ...
                if spot is not None and spot > 0:
                    logger.info(
                        "[STRIKE_SELECTOR_DIRECTIONAL_PASS] ticker=%s asset=%s tf=%s spot=%s - directional market passed through",
                        ticker, asset_upper, tf_lower, spot
                    )
                    return StrikeSelectionResult(
                        ticker=ticker,
                        asset=asset_upper,
                    ...
                    logger.warning(
                        "[STRIKE_SELECTOR_DIRECTIONAL_NO_SPOT] ticker=%s asset=%s tf=%s spot=%s - directional market but no spot",
                        ticker, asset_upper, tf_lower, spot
                    )
                    return self._reject(ticker, asset_upper, tf_lower, spot, strike,
                                        RejectionReason.DIRECTIONAL_NO_SPOT)
            logger.warning(
                "[STRIKE_SELECTOR_MISSING_STRIKE] ticker=%s asset=%s tf=%s spot=%s - missing strike and directional passthrough disabled",
                ticker, asset_upper, tf_lower, spot
            )
            return self._reject(ticker, asset_upper, tf_lower, spot, strike,
