...
The UI is frozen at **18 routed views** grouped into 5 sidebar sections. The canonical list lives in `web/react/src/types/views.ts`. Any new view requires updating that file, `App.tsx`, `Sidebar.tsx`, and `CommandPalette.tsx`.

| Section | Views |
|---------|-------|
| Trading | overview, kalshi-terminal, kalshi-dashboard, kalshi-all-markets, kalshi-portfolio, positions, orders |
| Swarm Intelligence | kalshi-grid, swarm-consensus, kalshi-performance, calibration-dashboard, lane-control |
| Analytics | kalshi-sentiment, kalshi-vol-dashboard, kalshi-risk-screen |
| Operator | operator, kill-switch |
| System | logs, settings |

- Do not add new views without updating all four files above
- Do not reintroduce legacy stub views — dead views belong in git history, not the filesystem
- All new Kalshi features must wire through existing views
- Shared components live in `web/react/src/components/`
- Sub-components used only within a single view may live in `views/` (e.g. `OperatorStatusBar.tsx`)
